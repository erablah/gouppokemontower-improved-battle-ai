  ...
  proc { |handled|
    ...
    $player.able_party.each do |pkmn|
      ...
      if pkmn.hp == 1 && !Settings::POISON_FAINT_IN_FIELD
        pkmn.status = :NONE
        pbMessage(_INTL("\\j[{1},은,는] 독을 버텨냈다.\\n독이 사라졌다!", pkmn.name))
        next
      elsif pkmn.hp == 0
        pkmn.changeHappiness("faint")
        pkmn.status = :NONE
        pbMessage(_INTL("\\j[{1},은,는] 기절하고 말았다...", pkmn.name))
      end
...
def pbItemBall(item, quantity = 1)
  ...
  if $bag.add(item, quantity)   # If item can be picked up
    ...
    if item == :DNASPLICERS
      pbMessage("\\me[#{meName}]" + _INTL("\\j[\\c[1]{1}\\c[0],을,를] 주웠다!", itemname) + "\\wtnp[40]")
    elsif item.is_machine?   # TM or HM
      if quantity > 1
        pbMessage("\\me[Machine get]" + _INTL("\\j[\\c[1]{2} {3}\\c[0],을,를] {1}개 주웠다!",
                                              quantity, itemname, GameData::Move.get(move).name) + "\\wtnp[70]")
      else
        pbMessage("\\me[Machine get]" + _INTL("\\j[\\c[1]{1} {2}\\c[0],을,를] 주웠다!",
                                              itemname, GameData::Move.get(move).name) + "\\wtnp[70]")
      end
    elsif quantity > 1
      pbMessage("\\me[#{meName}]" + _INTL("\\j[\\c[1]{2}\\c[0],을,를] {1}개 주웠다!", quantity, itemname) + "\\wtnp[40]")
  ...
  if item.is_machine?   # TM or HM
    if quantity > 1
      pbMessage(_INTL("\\j[\\c[1]{2} {3}\\c[0],을,를] {1}개 주웠다!", quantity, itemname, GameData::Move.get(move).name))
    else
      pbMessage(_INTL("\\j[\\c[1]{1} {2}\\c[0],을,를] 주웠다!", itemname, GameData::Move.get(move).name))
    end
  elsif quantity > 1
    pbMessage(_INTL("\\j[\\c[1]{2}\\c[0],을,를] {1}개 주웠다!", quantity, itemname))
  elsif itemname.starts_with_vowel?
    pbMessage(_INTL("\\j[\\c[1]{1}\\c[0],을,를] 주웠다!", itemname))
  else
    pbMessage(_INTL("\\j[\\c[1]{1}\\c[0],을,를] 주웠다!", itemname))
  end
...
def pbReceiveItem(item, quantity = 1)
  ...
  meName = (item.is_key_item?) ? "Key item get" : "Item get"
  if item == :DNASPLICERS
    pbMessage("\\me[#{meName}]" + _INTL("\\j[\\c[1]{1}\\c[0],을,를] 받았다!", itemname) + "\\wtnp[40]")
  elsif item.is_machine?   # TM or HM
    if quantity > 1
      pbMessage("\\me[Machine get]" + _INTL("\\j[\\c[1]{2} {3}\\c[0],을,를] {1}개 받았다!",
                                            quantity, itemname, GameData::Move.get(move).name) + "\\wtnp[70]")
    else
      pbMessage("\\me[Machine get]" + _INTL("\\j[\\c[1]{1} {2}\\c[0],을,를] 받았다!",
                                            itemname, GameData::Move.get(move).name) + "\\wtnp[70]")
    end
  elsif quantity > 1
    pbMessage("\\me[#{meName}]" + _INTL("\\j[\\c[1]{2}\\c[0],을,를] {1}개 받았다!", quantity, itemname) + "\\wtnp[40]")
  elsif itemname.starts_with_vowel?
    pbMessage("\\me[#{meName}]" + _INTL("\\j[\\c[1]{1}\\c[0],을,를]] 받았다!", itemname) + "\\wtnp[40]")
  else
    pbMessage("\\me[#{meName}]" + _INTL("\\j[\\c[1]{1}\\c[0],을,를] 받았다!", itemname) + "\\wtnp[40]")
...
def pbBuyPrize(item, quantity = 1)
  ...
  pbMessage("\\CN" + _INTL("\\j[{1},을,를] 가방의\\n<icon=bagPocket{2}>\\c[1]{3}\\c[0] 포켓에 넣었다.",
