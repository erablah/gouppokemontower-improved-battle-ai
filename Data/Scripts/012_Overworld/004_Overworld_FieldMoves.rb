...
def pbCut
  move = :CUT
  has_cut_tm = $bag.has?(:HM01)
  movefinder = $player.get_pokemon_with_move(move) unless has_cut_tm

  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_CUT, false) || (!$DEBUG && !has_cut_tm && !movefinder)
  ...
  if pbConfirmMessage(_INTL("This tree looks like it can be cut down!\nWould you like to cut it?"))
    $stats.cut_count += 1
    user_name = has_cut_tm ? $player.name : movefinder&.name || $player.name
    pbMessage(_INTL("\\j[{1},은,는] {2}를 사용했다!", user_name, GameData::Move.get(move).name))
...
HiddenMoveHandlers::UseMove.add(:CUT, proc { |move, pokemon|
  if !pbHiddenMoveAnimation(pokemon)
    pbMessage(_INTL("\\j[{1},은,는] {2}를 사용했다!", pokemon.name, GameData::Move.get(move).name))
  end
  $stats.cut_count += 1
  facingEvent = $game_player.pbFacingEvent
...
HiddenMoveHandlers::ConfirmUseMove.add(:DIG, proc { |move, pkmn|
  ...
  next pbConfirmMessage(_INTL("여기서 빠져나가 \\j[{1},으로,로] 돌아갈까?", mapname))
})
...
def pbDive
  ...
  if pbConfirmMessage(_INTL("The sea is deep here. Would you like to use Dive?"))
    ...
    pbMessage(_INTL("\\j[{1},은,는] {2}를 사용했다!", speciesname, GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    pbFadeOutIn do
...
def pbFlyToNewLocation(move = :FLY)
  ...
  return false if !$bag.has?(:HM02)
  name = $player.name
  pbMessage(_INTL("\\j[{1},은,는] {2}를 사용했다!", name, GameData::Move.get(move).name))
  $stats.fly_count += 1
  pbFadeOutIn do
    pbSEPlay("Fly")
...
HiddenMoveHandlers::CanUseMove.add(:FLY, proc { |move, _pkmn, showmsg|
  next pbCanFly?(showmsg)
})

HiddenMoveHandlers::UseMove.add(:FLY, proc { |move, _pkmn|
  if $game_temp.fly_destination.nil?
    pbMessage(_INTL("You can't use that here."))
    next false
  end
  pbFlyToNewLocation(move)
  next true
})
...
def pbRockSmash
  move = :ROCKSMASH
  has_rocksmash_tm = $bag.has?(:HM07)
  movefinder = $player.get_pokemon_with_move(move) unless has_rocksmash_tm

  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_ROCKSMASH, false) || (!$DEBUG && !has_rocksmash_tm && !movefinder)
  ...
  if pbConfirmMessage(_INTL("This rock seems breakable with a hidden move.\nWould you like to use Rock Smash?"))
    ...
    speciesname = has_rocksmash_tm ? $player.name : movefinder&.name || $player.name
    pbMessage(_INTL("\\j[{1},은,는] {2}를 사용했다!", speciesname, GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    return true
  end
...
HiddenMoveHandlers::UseMove.add(:STRENGTH, proc { |move, pokemon|
  if !pbHiddenMoveAnimation(pokemon)
    pbMessage(_INTL("\\j[{1},은,는] {2}를 사용했다!", pokemon.name, GameData::Move.get(move).name) + "\1")
  end
  pbMessage(_INTL("Strength made it possible to move boulders around!"))
  $PokemonMap.strengthUsed = true
  next true
...
def pbSurf
  allowed_maps = [13, 22]
  return false if !allowed_maps.include?($game_map.map_id)
  return false if !$game_player.can_ride_vehicle_with_follower?
  return false if !$bag.has?(:HM03)

  if pbConfirmMessage(_INTL("물이 잔잔히 흐르고 있다... 파도타기를 사용할까?"))
    pbMessage(_INTL("\\j[{1},은,는] 파도타기를 사용했다!", $player.name))
    pbCancelVehicles
...
# Hidden Move - Surf 사용 가능 여부
HiddenMoveHandlers::CanUseMove.add(:SURF, proc { |move, pkmn, showmsg|
  has_surf_tm = $bag.has?(:HM03)
  next false if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_SURF, showmsg)

  if !has_surf_tm && !pkmn
    pbMessage(_INTL("A Pokémon needs to know Surf or you need to carry the HM.")) if showmsg
    next false
  end
...
HiddenMoveHandlers::UseMove.add(:SURF, proc { |move, pokemon|
  allowed_maps = [13, 22]
  if !allowed_maps.include?($game_map.map_id)
    pbMessage(_INTL("여기서는 파도타기를 사용할 수 없다!"))
    next false
  end
  
  has_surf_tm = $bag.has?(:HM03)
  user_name = has_surf_tm ? $player.name : pokemon&.name || $player.name
...
def pbWaterfall
  return false if !$bag.has?(:HM05)  # HM05가 없으면 작동 불가

  if pbConfirmMessage(_INTL("It's a large waterfall. Would you like to use Waterfall?"))
    $stats.waterfall_count += 1
    pbMessage(_INTL("\\j[{1},은,는] 폭포오르기를 사용했다!", $player.name))
    pbHiddenMoveAnimation(nil)
    pbAscendWaterfall
    return true
  end
