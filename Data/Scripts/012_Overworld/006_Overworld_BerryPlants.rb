...
def pbBerryPlant
  ...
  if berry_plant.grown?
    ...
    when 1   # X planted
      ...
      if berry_name.starts_with_vowel?
        pbMessage(_INTL("\\j[{1},이,가] 심어져 있다.", berry_name))
      else
        pbMessage(_INTL("\\j[{1},이,가] 심어져 있다.", berry_name))
      end
    when 2   # X sprouted
      this_event.turn_down   # Stop the event turning towards the player
      pbMessage(_INTL("\\j[{1},이,가] 싹을 틔웠다.", berry_name))
    when 3   # X taller
      this_event.turn_left   # Stop the event turning towards the player
      pbMessage(_INTL("{1} 나무가 자라고 있다.", berry_name))
    else     # X flowering
      this_event.turn_right   # Stop the event turning towards the player
      if Settings::NEW_BERRY_PLANTS
        pbMessage(_INTL("{1} 나무가 다 자랐다!", berry_name))
      else
        case berry_plant.watering_count
        when 4
          pbMessage(_INTL("{1} 나무가 눈부시게 자라났다!", berry_name))
        when 3
          pbMessage(_INTL("{1} 나무가 매우 아름답게 자라났다!", berry_name))
        when 2
          pbMessage(_INTL("{1} 나무가 예쁘게 자라났다!", berry_name))
        when 1
          pbMessage(_INTL("{1} 나무가 귀엽게 자라났다!", berry_name))
        else
          pbMessage(_INTL("{1} 나무가 자라났다!", berry_name))
        end
      end
    end
    ...
    GameData::BerryPlant::WATERING_CANS.each do |item|
      ...
      break if !pbConfirmMessage(_INTL("\\j[{1},으로,로] 물을 줄까?",
                                       GameData::Item.get(item).name))
      berry_plant.water
      pbMessage("\\se[Water berry plant]" + _INTL("\\j[{1},은,는] 물을 주었다.", $player.name) + "\\wtnp[40]")
      if Settings::NEW_BERRY_PLANTS
  ...
  if Settings::NEW_BERRY_PLANTS
    ...
    if berry_plant.mulch_id
      pbMessage(_INTL("\\j[{1},을,를] 심었다.", GameData::Item.get(berry_plant.mulch_id).name))
    else
      ...
      when 0   # Fertilize
        ...
        if mulch_data.is_mulch?
          ...
          $bag.remove(mulch)
          pbMessage(_INTL("\\j[{1},을,를] 흙 위에 뿌렸다.", mulch_data.name))
        else
...
def pbPickBerry(berry, qty = 1)
  ...
  if qty > 1
    message = _INTL("\\j[\\c[1]{2}\\c[0],이,가] {1}개 열려있다!\n열매를 딸까?", qty, berry_name)
  else
    message = _INTL("\\j[\\c[1]{1}\\c[0],이,가] 1개 열려있다!\n열매를 딸까?", berry_name)
  end
  ...
  $bag.add(berry, qty)
  if qty > 1
    pbMessage("\\me[Berry get]" + _INTL("\\j[\\c[1]{2}\\c[0],을,를] {1}개 땄다!.", qty, berry_name) + "\\wtnp[30]")
  else
    pbMessage("\\me[Berry get]" + _INTL("\\j[\\c[1]{1}\\c[0],을,를] 땄다!.", berry_name) + "\\wtnp[30]")
  end
  pocket = berry.pocket
  pbMessage(_INTL("\\j[{1},을,를] 가방의\\n<icon=bagPocket{2}>\\c[1]{3}\\c[0] 포켓에 넣었다.",
