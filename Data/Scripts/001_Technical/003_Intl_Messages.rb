...
module Translator
  ...
  def find_translatable_text_from_RGSS_script(items, script)
    ...
    # If it crashes, this code runs instead of closing the game
    File.open("encoding_error_log.txt", "w") { |f| 
      f.write("CRASH IN SCRIPT CONTENT:\n\n")
      f.write(script) 
    }
    raise "FOUND IT! Check 'encoding_error_log.txt' in your game folder. Error: #{e.message}"
  end
  end
