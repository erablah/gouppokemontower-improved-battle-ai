...
def pbNickname(pkmn)
  ...
  if pbConfirmMessage(_INTL("Would you like to give a nickname to {1}?", species_name))
    pkmn.name = pbEnterPokemonName(_INTL("{1}의 별명은?", species_name),
...
def pbStorePokemon(pkmn)
  ...
  if $player.party_full?
    ...
    pbMessage(_INTL("\\j[{1},은,는] 박스 \"{2}\"로 보내졌다!", pkmn.name, box_name))
  else
...
def pbAddPokemon(pkmn, level = 1, see_form = true)
  ...
  species_name = pkmn.speciesName
  pbMessage(_INTL("\\j[{1},은,는] \\j[\\c[1]{2}\\c[0],을,를] 얻었다!", $player.name, species_name) + "\\me[Pkmn get]\\wtnp[80]")
  ...
  if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && see_form && !was_owned &&
    ...
    pbMessage(_INTL("{1}의 정보가 도감에 기록되었다.", species_name))
...
def pbAddForeignPokemon(pkmn, level = 1, owner_name = nil, nickname = nil, owner_gender = 0, see_form = true)
  ...
  pkmn.calc_stats
  if owner_name
    pbMessage(_INTL("\\j[{1},은,는] {2}에게서 포켓몬을 받았다.", $player.name, owner_name) + "\\me[Pkmn get]\\wtnp[80]")
  else
    pbMessage(_INTL("\\j[{1},은,는] 포켓몬을 받았다.", $player.name) + "\\me[Pkmn get]\\wtnp[80]")
  end
  was_owned = $player.owned?(pkmn.species)
  $player.pokedex.set_seen(pkmn.species)
  $player.pokedex.set_owned(pkmn.species)
