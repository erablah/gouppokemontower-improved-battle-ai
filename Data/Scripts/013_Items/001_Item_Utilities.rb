...
def pbChangeLevel(pkmn, new_level, scene)
  ...
  if old_level > new_level
    ...
    if scene.is_a?(PokemonPartyScreen)
      scene.pbDisplay(_INTL("\\j[{1},은,는] Lv. {2}이 되었다!", pkmn.name, pkmn.level))
    else
      pbMessage(_INTL("\\j[{1},은,는] Lv. {2}이 되었다!", pkmn.name, pkmn.level))
    end
    total_hp_diff        = pkmn.totalhp - old_total_hp
    attack_diff          = pkmn.attack - old_attack
    defense_diff         = pkmn.defense - old_defense
...
def pbChangeExp(pkmn, new_exp, scene)
  ...
  if pkmn.exp > new_exp   # Loses Exp
    ...
    if scene.is_a?(PokemonPartyScreen)
      scene.pbDisplay(_INTL("\\j[{1},은,는] 경험치를 {2}만큼 잃었다!", pkmn.name, difference))
    else
      pbMessage(_INTL("\\j[{1},은,는] 경험치를 {2}만큼 잃었다!", pkmn.name, difference))
    end
    pkmn.exp = new_exp
    pkmn.calc_stats
    scene.pbRefresh
    ...
    if scene.is_a?(PokemonPartyScreen)
      scene.pbDisplay(_INTL("\\j[{1},은,는] 경험치를 {2}만큼 얻었다!", pkmn.name, difference))
    else
      pbMessage(_INTL("\\j[{1},은,는] 경험치를 {2}만큼 얻었다!", pkmn.name, difference))
    end
    pkmn.exp = new_exp
    pkmn.changeHappiness("vitamin")
    pkmn.calc_stats
...
def pbUseEVRaisingItem(stat, amt_per_use, qty, pkmn, happiness_type, scene, no_ev_cap = false)
  ...
  pbSEPlay("Use item in party")
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 올랐다.", pkmn.name, GameData::Stat.get(stat).name))
  return true
end
...
def pbNatureChangingMint(new_nature, item, pkmn, scene)
  ...
  if !scene.pbConfirm(_INTL("{1}의 성격에 영향을 줄 수 있습니다. 사용하시겠습니까?", pkmn.name))
    return false
  end
  pkmn.nature_for_stats = new_nature
  pkmn.calc_stats
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}의 능력치가 {2}의 효과로 인해 변한 것 같다!",
...
def pbLearnMove(pkmn, move, ignore_if_known = false, by_machine = false, &block)
  ...
  if pkmn.hasMove?(move)
    pbMessage(_INTL("\\j[{1},은,는] 이미 \\j[{2},을,를] 알고 있다.", pkmn_name, move_name), &block) if !ignore_if_known
    return false
  elsif pkmn.numMoves < Pokemon::MAX_MOVES
    pkmn.learn_move(move)
    pbMessage("\\se[]" + _INTL("\\j[{1},은,는] \\j[{2},을,를] 배웠다!", pkmn_name, move_name) + "\\se[Pkmn move learnt]", &block)
    return true
  end
  pbMessage(_INTL("\\j[{1},은,는] \\j[{2},을,를] 배우고 싶지만, 이미 기술을 {3}개 알고 있습니다.",
                  pkmn_name, move_name, pkmn.numMoves.to_word) + "\1", &block)
  if pbConfirmMessage(_INTL("\\j[{1},이,가] \\j[{2},을,를] 배울까요?", pkmn_name, move_name), &block)
    loop do
      ...
      if move_index >= 0
        ...
        pbMessage(_INTL("1, 2, ...\\wt[16] ...\\wt[16] ...\\wt[16] 짜잔!") + "\\se[Battle ball drop]\1", &block)
        pbMessage(_INTL("\\j[{1},은,는] \\j[{2},을,를] 잊었다! \\n그리고..." + "\1", pkmn_name, old_move_name), &block)
        pbMessage("\\se[]" + _INTL("\\j[{1},은,는] \\j[{2},을,를] 배웠다!!", pkmn_name, move_name) + "\\se[Pkmn move learnt]", &block)
        pkmn.changeHappiness("machine") if by_machine
        return true
      elsif pbConfirmMessage(_INTL("\\j[{1},을,를] 포기할까?", move_name), &block)
        pbMessage(_INTL("\\j[{1},은,는] \\j[{2},을,를] 배우지 않았다.", pkmn_name, move_name), &block)
        return false
      end
    end
  else
  ...
  if useType == 1   # Item is usable on a Pokémon
    ...
    pbFadeOutIn do
      ...
      loop do
        ...
        if max_at_once > 1
          qty = screen.scene.pbChooseNumber(JosaProcessor.process(
            _INTL("\\j[{1},을,를] 몇 개 사용할까?", GameData::Item.get(item).portion_name_plural)), max_at_once
          )
        ...
        pbMessage(_INTL("마지막 \\j[{1},을,를] 사용했다.", itm.portion_name)) { screen.pbUpdate }
        break
      end
      screen.pbEndScene
      bagscene&.pbRefresh
...
def pbUseItemOnPokemon(item, pkmn, scene)
  ...
  if itm.is_machine?
    ...
    if pkmn.shadowPokemon?
      ...
      pbMessage(_INTL("\\j[{1},은,는] \\j[{2},을,를] 배울 수 없다.", pkmn.name, movename)) { scene.pbUpdate }
    else
      pbMessage("\\se[PC access]" + _INTL("\\j[{1},을,를] 사용했다.", itm.portion_name) + "\1") { scene.pbUpdate }
      if pbConfirmMessage(_INTL("{2}에게 \\j[{1},을,를] 가르칠까?", movename, pkmn.name)) { scene.pbUpdate }
  ...
  max_at_once = [max_at_once, $bag.quantity(item)].min
  if max_at_once > 1
    qty = scene.scene.pbChooseNumber(JosaProcessor.process(
      _INTL("\\j[{1},을,를] 몇 개 사용할까?", itm.portion_name_plural)), max_at_once
    )
    scene.scene.pbSetHelpText("") if scene.is_a?(PokemonPartyScreen)
  end
  ...
  if ret && itm.consumed_after_use?
    $bag.remove(item, qty)
    if !$bag.has?(item)
      pbMessage(_INTL("마지막 \\j[{1},을,를] 사용했다.", itm.portion_name)) { scene.pbUpdate }
    end
  end
  return ret
end
...
def pbUseItemMessage(item)
  ...
  if itemname.starts_with_vowel?
    pbMessage(_INTL("\\j[{1},을,를] 사용했다.", itemname))
  else
    pbMessage(_INTL("\\j[{1},을,를] 사용했다.", itemname))
  end
end
...
def pbGiveItemToPokemon(item, pkmn, scene, pkmnid = 0)
  ...
  if pkmn.egg?
    ...
    scene.pbDisplay(_INTL("{1}에게 도구를 주려면 메일을 먼저 제거해야 한다.", pkmn.name))
    return false if !pbTakeItemFromPokemon(pkmn, scene)
  end
  if pkmn.hasItem?
    olditemname = pkmn.item.portion_name
    if newitemname.starts_with_vowel?
      scene.pbDisplay(_INTL("\\j[{1},은,는] 이미 \\j[{2},을,를] 지니고 있다.", pkmn.name, olditemname) + "\1")
    else
      scene.pbDisplay(_INTL("\\j[{1},은,는] 이미 \\j[{2},을,를] 지니고 있다.", pkmn.name, olditemname) + "\1")
    end
    if scene.pbConfirm(_INTL("Would you like to switch the two items?"))
      ...
      if !$bag.add(pkmn.item)
        ...
        if pbWriteMail(item, pkmn, pkmnid, scene)
          pkmn.item = item
          scene.pbDisplay(_INTL("{2}에게서 \\j[{1},을,를] 가져온 뒤, \\j[{3},을,를] 줬다.", olditemname, pkmn.name, newitemname))
          return true
        elsif !$bag.add(item)
    ...
    $bag.remove(item)
    pkmn.item = item
    scene.pbDisplay(_INTL("\\j[{1},은,는] 이제 \\j[{2},을,를] 지니고 있다.", pkmn.name, newitemname))
    return true
  end
  return false
end
...
def pbTakeItemFromPokemon(pkmn, scene)
  ret = false
  if !pkmn.hasItem?
    scene.pbDisplay(_INTL("\\j[{1},은,는] 아무것도 지니고 있지 않다.", pkmn.name))
    ...
    if scene.pbConfirm(_INTL("Save the removed mail in your PC?"))
      ...
      $bag.add(pkmn.item)
      scene.pbDisplay(_INTL("{2}에게서 \\j[{1},을,를] 받았다.", pkmn.item.portion_name, pkmn.name))
      pkmn.item = nil
      pkmn.mail = nil
      ret = true
    end
