...
ItemHandlers::UseFromBag.addIf(:move_machines,
  ...
  proc { |item|
    ...
    move = item_data.move
    next 0 if !move
    pbMessage("\\se[PC access]" + _INTL("\\j[{1},을,를] 사용했다.", item_data.name) + "\1")
    next 0 if !pbConfirmMessage(_INTL("포켓몬에게 \\j[{1},을,를] 가르치겠습니까?",
...
ItemHandlers::ConfirmUseInField.add(:ESCAPEROPE, proc { |item|
  ...
  next pbConfirmMessage(_INTL("여기서 빠져나가 \\j[{1},으로,로] 돌아가시겠습니까?", mapname))
})
...
ItemHandlers::UseInField.add(:ITEMFINDER, proc { |item|
  ...
  if offsetX == 0 && offsetY == 0   # Standing on the item, spin around
    ...
    pbWait(0.3)
    pbMessage(_INTL("\\j[{1},이,가] 바로 발밑에서 무언가를 가리키고 있다!", GameData::Item.get(item).name))
    ...
    pbWait(0.3)
    pbMessage(_INTL("어라? \\j[{1},이,가] 반응하고 있다!", GameData::Item.get(item).name) + "\1")
...
ItemHandlers::UseInField.add(:EXPALL, proc { |item|
  ...
  $game_switches[EXP_ALL_SWITCH] = false
...
ItemHandlers::UseInField.add(:EXPALLOFF, proc { |item|
  ...
  $game_switches[EXP_ALL_SWITCH] = true
...
ItemHandlers::UseOnPokemon.add(:AWAKENING, proc { |item, qty, pkmn, scene|
  ...
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("\\j[{1},은,는] 잠에서 깼다.", pkmn.name))
  next true
})
...
ItemHandlers::UseOnPokemon.add(:ANTIDOTE, proc { |item, qty, pkmn, scene|
  ...
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}의 독이 치료되었다!", pkmn.name))
  next true
})
...
ItemHandlers::UseOnPokemon.add(:BURNHEAL, proc { |item, qty, pkmn, scene|
  ...
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}의 화상이 치료되었다!", pkmn.name))
  next true
})
...
ItemHandlers::UseOnPokemon.add(:PARALYZEHEAL, proc { |item, qty, pkmn, scene|
  ...
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}의 마비가 치료되었다!", pkmn.name))
  next true
})
...
ItemHandlers::UseOnPokemon.add(:ICEHEAL, proc { |item, qty, pkmn, scene|
  ...
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("\\j[{1},은,는] 해동되었다!", pkmn.name))
  next true
})
...
ItemHandlers::UseOnPokemon.add(:FULLHEAL, proc { |item, qty, pkmn, scene|
  ...
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("\\j[{1},은,는] 건강해졌다.", pkmn.name))
  next true
})
...
ItemHandlers::UseOnPokemon.add(:POMEGBERRY, proc { |item, qty, pkmn, scene|
  next pbRaiseHappinessAndLowerEV(
    pkmn, scene, :HP, qty, [
      _INTL("\\j[{1},이,가] 당신을 아주 좋아한다! 기본 HP가 떨어졌다!", pkmn.name),
...
ItemHandlers::UseOnPokemon.add(:KELPSYBERRY, proc { |item, qty, pkmn, scene|
  next pbRaiseHappinessAndLowerEV(
    pkmn, scene, :ATTACK, qty, [
      _INTL("\\j[{1},이,가] 당신을 아주 좋아한다! 기본 공격이 떨어졌다!", pkmn.name),
...
ItemHandlers::UseOnPokemon.add(:QUALOTBERRY, proc { |item, qty, pkmn, scene|
  next pbRaiseHappinessAndLowerEV(
    pkmn, scene, :DEFENSE, qty, [
      _INTL("\\j[{1},이,가] 당신을 아주 좋아한다! 기본 방어가 떨어졌다!", pkmn.name),
...
ItemHandlers::UseOnPokemon.add(:HONDEWBERRY, proc { |item, qty, pkmn, scene|
  next pbRaiseHappinessAndLowerEV(
    pkmn, scene, :SPECIAL_ATTACK, qty, [
      _INTL("\\j[{1},이,가] 당신을 아주 좋아한다! 기본 특수공격이 떨어졌다!", pkmn.name),
...
ItemHandlers::UseOnPokemon.add(:GREPABERRY, proc { |item, qty, pkmn, scene|
  next pbRaiseHappinessAndLowerEV(
    pkmn, scene, :SPECIAL_DEFENSE, qty, [
      _INTL("\\j[{1},이,가] 당신을 아주 좋아한다! 기본 특수방어가 떨어졌다!", pkmn.name),
...
ItemHandlers::UseOnPokemon.add(:TAMATOBERRY, proc { |item, qty, pkmn, scene|
  next pbRaiseHappinessAndLowerEV(
    pkmn, scene, :SPEED, qty, [
      _INTL("\\j[{1},이,가] 당신을 아주 좋아한다! 기본 스피드가 떨어졌다!", pkmn.name),
...
ItemHandlers::UseOnPokemon.add(:ABILITYCAPSULE, proc { |item, qty, pkmn, scene|
  if scene.pbConfirm(_INTL("Do you want to change {1}'s Ability?", pkmn.name))
    ...
    pkmn.ability = nil
    scene.pbRefresh
    scene.pbDisplay(_INTL("{1}의 특성이 바뀌었다! 새로운 특성은 \\j[{2},이다,다]!", pkmn.name, newabilname))
    next true
  end
  next false
})
...
ItemHandlers::UseOnPokemon.add(:ABILITYPATCH, proc { |item, qty, pkmn, scene|
  if scene.pbConfirm(_INTL("Do you want to change {1}'s Ability?", pkmn.name))
    ...
    pkmn.ability = nil
    scene.pbRefresh
    scene.pbDisplay(_INTL("{1}의 특성이 바뀌었다! 새로운 특성은 \\j[{2},이다,다]!", pkmn.name, new_ability_name))
    next true
  end
  next false
})
...
ItemHandlers::UseOnPokemon.add(:ROTOMCATALOG, proc { |item, qty, pkmn, scene|
  ...
  new_form = scene.pbShowCommands(_INTL("어떤 모습으로 변화시킬까?"), choices, pkmn.form)
  if new_form == pkmn.form
    ...
    pkmn.setForm(new_form) do
      scene.pbRefresh
      scene.pbDisplay(_INTL("{1} 모습이 변했다!", pkmn.name))
    end
    next true
  end
  next false
...
ItemHandlers::UseOnPokemon.add(:ZYGARDECUBE, proc { |item, qty, pkmn, scene|
  ...
  when 1   # Change ability
    ...
    pkmn.ability = nil
    scene.pbRefresh
    scene.pbDisplay(_INTL("{1}의 특성이 바뀌었다! 새로운 특성은 \\j[{2},이다,다]!", pkmn.name, pkmn.ability.name))
    next true
  end
  next false
})
...
ItemHandlers::UseOnPokemon.add(:BOTTLECAP, proc { |item, qty, pkmn, scene|
  if scene.nil?
    next false
  end
  if pkmn.fainted?
    scene.pbDisplay(_INTL("기절한 포켓몬에게는 사용할 수 없다.", pkmn.name))
    next false
  end

  stats = [:HP, :ATTACK, :DEFENSE, :SPECIAL_ATTACK, :SPECIAL_DEFENSE, :SPEED]
  stat_names = ["HP", "공격", "방어", "특수공격", "특수방어", "스피드"]

  cmd = nil
  begin
    if scene.respond_to?(:pbChooseListCommand)
      cmd = scene.pbChooseListCommand(
        _INTL("{1}의 어떤 능력치를 올릴까요?", pkmn.name),
        stat_names,
        nil
      )
    else
    ...
    scene.pbDisplay("선택 UI를 불러올 수 없습니다.")
    next false
  end

  if cmd.is_a?(Integer) && cmd >= 0 && cmd < stats.length
    pkmn.iv[stats[cmd]] = 31
    pkmn.calc_stats
    scene.pbDisplay(_INTL("{1}의 {2}의 IV가 최고치가 되었다!", pkmn.name, stat_names[cmd]))
    next true
  end
...
ItemHandlers::UseOnPokemon.add(:GOLDBOTTLECAP, proc { |item, qty, pkmn, scene|
  if scene.nil?
    next false
  end
  if pkmn.fainted?
  ...
  pkmn.calc_stats
  scene.pbDisplay(_INTL("{1}의 모든 IV가 최고치가 되었다!", pkmn.name))
  next true
})



ItemHandlers::UseOnPokemon.add(:ADAPTIVELEVELCANDY, proc { |item, qty, pkmn, scene|
  # 파티 평균 레벨 계산
  party = $player.party
  total_level = 0
  num_pokemon = 0
  party.each do |poke|
    next if poke.nil? || poke.fainted?
    total_level += poke.level
    num_pokemon += 1
  end
  avg_level = (num_pokemon > 0) ? (total_level.to_f / num_pokemon).floor : 1
  target_level = [avg_level - 2, 1].max

  if pkmn.level >= target_level
    scene.pbDisplay(_INTL("{1}는 이미 평균보다 충분히 강하다!", pkmn.name))
    next false
  end

  pkmn.exp = pkmn.growth_rate.minimum_exp_for_level(target_level)
  pkmn.calc_stats
  scene.pbDisplay(_INTL("{1}의 레벨이 {2}이 되었다!", pkmn.name, target_level))
  next true
})


ItemHandlers::UseOnPokemon.add(:SHINYCANDY, proc { |item, qty, pkmn, scene|
  if pkmn.shiny?
    scene.pbDisplay(_INTL("{1}는 이미 빛나는 포켓몬이다!", pkmn.name))
    next false
  end

  pkmn.shiny = true
  scene.pbDisplay(_INTL("{1}의 몸이 반짝이기 시작했다!", pkmn.name))
  next true
})
...
#===============================================================================
# 아이템 핸들러: JAZZTAPE (디버그 메뉴 기반 최종 수정)
#===============================================================================
ItemHandlers::UseOnPokemon.add(:JAZZTAPE, proc { |item, pkmn, scene|
  next false
})

ItemHandlers::UseInField.add(:JAZZTAPE, proc { |item, pkmn|
  # 1. 효과음 재생
  pbPlayDecisionSE
  pbMEFade(0.3)
  pbMEPlay("Jazz Tape")
  
  # 2. 전체 회복 로직 실행 (디버그 메뉴 스크립트와 동일한 방식으로 개별 회복)
  $player.party.each do |p|
    # 알인 경우 회복을 건너뜁니다.
    next if p.egg?
    
    # 2-A. HP 회복 (디버그 메뉴 스크립트 참고)
    p.heal_HP
    
    # 2-B. 상태이상 회복 (디버그 메뉴 스크립트 참고)
    p.heal_status
    
    # 2-C. PP 회복 (HP/상태와 별도로 구현)
    p.moves.each do |m|
      m.pp = m.total_pp # 모든 기술의 PP를 최대치로 설정
    end
  end
  
  # 3. 메시지 출력
  Kernel.pbMessage(_INTL("재즈 음악을 들으며 포켓몬들이 치유되었다."))
  
  # 4. 아이템 소모
  next 1 # 아이템을 1개 소모하고 사용 성공
})