...
module PokeTetra
  ...
  # pass the parameters to pbStartScene.
    def pbStartScreen(opponentName,minLevel,maxLevel,
      ...
      if count<HAND_SIZE
        @scene.pbDisplayPaused(_INTL("카드가 부족합니다."))
        @scene.pbEndScene
        return 0
      end
      ...
      blockPlacement
      @scene.pbDisplay(_INTL("선공 플레이어를 고르는 중..."))
      @scene.pbUpdateScore
      playerTurn=false
      if rand(2)==0
        @scene.pbDisplay(_INTL("\\j[{1},이,가] 선공입니다.",@playerName))
        playerTurn=true
      else
        @scene.pbDisplay(_INTL("\\j[{1},이,가] 선공입니다.",@opponentName))
        playerTurn=false
      end
      for i in 0...PLAYABLE_CARDS
        ...
        if playerTurn
          ...
          # Opponent's turn
          @scene.pbDisplay(_INTL("\\j[{1},이,가] 카드를 두고 있습니다...",@opponentName))    
          scores=[]
          ...
          if scores.length==0
            @scene.pbDisplay(_INTL("\\j[{1},은,는] 움직일 수가 없다...",@opponentName))
      ...
      if playerCount==opponentCount
        @scene.pbDisplayPaused(_INTL("무승부입니다."))
        result=3
        if !USE_PARTY
          case @trade
          when 1
        ...
        @scene.pbDisplayPaused(_INTL("\\j[{1},이,가] {2}에게 승리하였습니다.",
        ...
        @scene.pbDisplayPaused(
          _INTL("\\j[{1},이,가] {2}에게 패배하였습니다.",@playerName,@opponentName)
        )
        result=2
        if !USE_PARTY
          case @trade
...
pbFadeOutInWithMusic(99999){
    ...
    result = screen.pbStartScreen(name,minLevel,maxLevel,rules,oppdeck,prize)
    return result 
}
end
