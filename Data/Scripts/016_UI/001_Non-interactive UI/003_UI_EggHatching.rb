...
class PokemonEggHatch_Scene
  ...
  def pbMain
    ...
    @pokemon.name = nil
    pbMessage("\\se[]" + _INTL("\\j[{1},이,가] 알에서 부화했다!", @pokemon.name) + "\\wt[80]") { update }
    ...
    if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && !was_owned &&
      ...
      pbMessage(_INTL("{1}의 정보가 도감에 기록되었다.", @pokemon.name)) { update }
    ...
    if $PokemonSystem.givenicknames == 0 &&
      ...
      nickname = pbEnterPokemonName(_INTL("{1}의 별명은?", @pokemon.name),
...
def pbHatch(pokemon)
  ...
  if !pbHatchAnimation(pokemon)
    ...
    pbMessage(_INTL("...") + "\1")
    pbMessage(_INTL("... .... .....") + "\1")
    pbMessage(_INTL("\\j[{1},이,가] 알에서 부화했다!!", speciesname))
    was_owned = $player.owned?(pokemon.species)
    $player.pokedex.register(pokemon)
    $player.pokedex.set_owned(pokemon.species)
    $player.pokedex.set_seen_egg(pokemon.species)
    ...
    if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && !was_owned &&
       $player.has_pokedex && $player.pokedex.species_in_unlocked_dex?(pokemon.species)
      pbMessage(_INTL("{1}의 정보가 도감에 기록되었다.", speciesname))
      $player.pokedex.register_last_seen(pokemon)
      pbFadeOutIn do
        scene = PokemonPokedexInfo_Scene.new
        screen = PokemonPokedexInfoScreen.new(scene)
    ...
    if $PokemonSystem.givenicknames == 0 &&
      ...
      nickname = pbEnterPokemonName(_INTL("{1}의 별명은?", speciesname),
...
#===============================================================================
# Faster Egg Hatching (tuned)
#===============================================================================
BASE_HATCH_SPEED = 2   # ← 기본 부화 속도 (1 = 기본, 2 = 2배, 3 = 매우 빠름)
...
EventHandlers.add(:on_player_step_taken, :hatch_eggs,
  proc {
    $player.party.each do |egg|
      ...
      speed = BASE_HATCH_SPEED

      # 불꽃몸 / 마그마아머 / 스팀엔진 등 FasterEggHatching 특성
