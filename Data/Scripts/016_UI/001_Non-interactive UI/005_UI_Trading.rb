...
class PokemonTrade_Scene
  ...
  def pbTrade
    ...
    pbScene1
    pbMessageDisplay(@sprites["msgwindow"],
                     _INTL("{1}의 \\j[{2},과,와]\n{3}의 \\j[{4},을,를] 교환했다.", @trader1, speciesname1, @trader2, speciesname2) + "\1") { pbUpdate }
    pbMessageDisplay(@sprites["msgwindow"],
                     _INTL("{1}에게서 \\j[{2},을,를] 받았다!", @trader2, speciesname2)) { pbUpdate }
    pbScene2
    pbBGMStop
    ...
    pbMessageDisplay(@sprites["msgwindow"],
                     _INTL("\\j[{1},을,를] 잘 부탁해!", speciesname2)) { pbUpdate }
    ...
    if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && !was_owned &&
      ...
      pbMessageDisplay(@sprites["msgwindow"],
                       _INTL("{1}의 정보가 도감에 기록되었다.", speciesname2)) { pbUpdate }
