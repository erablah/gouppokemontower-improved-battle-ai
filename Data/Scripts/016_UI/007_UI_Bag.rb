...
class Window_PokemonBag < Window_DrawableCommand
  ...
  def drawItem(index, _count, rect)
    textpos = []
    rect = Rect.new(rect.x + 16, rect.y + 18, rect.width - 16, rect.height)
    thispocket = @bag.pockets[@pocket]
    if index == self.itemCount - 1
      textpos.push([_INTL("CLOSE BAG"), rect.x, rect.y + 8, :left, self.baseColor, self.shadowColor])
    else
      ...
      textpos.push(
        [@adapter.getDisplayName(item), rect.x, rect.y + 8, :left, baseColor, shadowColor]
      )
      ...
      if item_data.is_important?
        if @bag.registered?(item)
          pbDrawImagePositions(
            self.contents,
            [[_INTL("Graphics/UI/Bag/icon_register"), rect.x + rect.width - 72, rect.y + 9, 0, 0, -1, 24]]
          )
          showing_register_icon = true
        elsif pbCanRegisterItem?(item)
          pbDrawImagePositions(
            self.contents,
            [[_INTL("Graphics/UI/Bag/icon_register"), rect.x + rect.width - 72, rect.y + 6, 0, 24, -1, 24]]
          )
          showing_register_icon = true
        end
      end
      if item_data.show_quantity? && !showing_register_icon
        ...
        textpos.push([qtytext, xQty, rect.y + 8, :left, baseColor, shadowColor])
      end
    end
...
class PokemonBag_Scene
  ...
  def pbStartScene(bag, choosing = false, filterproc = nil, resetpocket = true)
    ...
    @sprites["itemtext"] = Window_UnformattedTextPokemon.newWithSize(
      "", 72, 295, Graphics.width - 72 - 24, 128, @viewport
    )
  ...
  def pbRefreshIndexChanged
    ...
    if showslider
     ...
     if itemlist.item
    desc = GameData::Item.get(itemlist.item).description

    if desc.length > 47 || desc.count("\n") >= 2
      @sprites["itemtext"].y = 280
    else
      @sprites["itemtext"].y = 295
    end

    @sprites["itemtext"].text = desc
  else
    @sprites["itemtext"].y = 295
...
class PokemonBagScreen
  ...
  def pbStartScreen
    ...
    loop do
      ...
      itemname = itm.name
      command = @scene.pbShowCommands(JosaProcessor.process(_INTL("\\j[{1},을,를] 선택했습니다.", itemname)), commands)
      if cmdRead >= 0 && command == cmdRead   # Read mail
        ...
        if $player.pokemon_count == 0
          ...
          @scene.pbDisplay(_INTL("\\j[{1},은,는] 지닐 수 없습니다.", itm.portion_name))
        else
          pbFadeOutIn do
        ...
        if qty > 1
          helptext = _INTL("\\j[{1},을,를] 몇 개 버릴까요?", itm.portion_name_plural)
          qty = @scene.pbChooseNumber(helptext, qty)
        end
        if qty > 0
          itemname = (qty > 1) ? itm.portion_name_plural : itm.portion_name
          if pbConfirm(_INTL("\\j[{2},을,를] {1}개 버릴까요?", qty, itemname))
            pbDisplay(_INTL("\\j[{2},을,를] {1}개 버렸다.", qty, itemname))
  ...
  def pbWithdrawItemScreen
    ...
    loop do
      ...
      if @bag.can_add?(item, qty)
        ...
        pbDisplay(_INTL("\\j[{2},을,를] {1}개 꺼냈다.", dispqty, itemname))
      else
    ...
    loop do
      ...
      if qty > 0
        if storage.can_add?(item, qty)
          ...
          dispqty  = (itm.is_important?) ? 1 : qty
          itemname = (dispqty > 1) ? itm.portion_name_plural : itm.portion_name
          pbDisplay(_INTL("\\j[{2},을,를] {1}개 맡겼다.", dispqty, itemname))
        else
  ...
  def pbTossItemScreen
    ...
    loop do
      ...
      if qty > 1
        qty = @scene.pbChooseNumber(_INTL("\\j[{1},을,를] 몇 개 버릴까요?", itemnameplural), qty)
      end
      next if qty <= 0
      itemname = itemnameplural if qty > 1
      next if !pbConfirm(_INTL("\\j[{2},을,를] {1}개 버릴까요?", qty, itemname))
      if !storage.remove(item, qty)
        raise "Can't delete items from storage"
      end
      @scene.pbRefresh
