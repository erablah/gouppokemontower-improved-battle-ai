...
class PokemonSummary_Scene
  ...
  def drawPage(page)
    ...
    textpos = [
      ...
      [_INTL("도구"), 66, 324, :left, base, shadow]
    ]
    # Write the held item's name
    if @pokemon.hasItem?
      textpos.push([@pokemon.item.name, 16, 358, :left, Color.new(64, 64, 64), Color.new(176, 176, 176)])
    else
      textpos.push([_INTL("없음"), 16, 358, :left, Color.new(192, 200, 208), Color.new(208, 216, 224)])
    end
  ...
  def drawPageOneEgg
    ...
    if mapname && mapname != ""
      ...
      memo += black_text_tag + _INTL("{1}에게서 받은 신비한 포켓몬의 알.", mapname) + "\n"
    else
  ...
  def drawPageTwo
    ...
    mettext = [
      ...
      "",
      _INTL("Lv.{1}에 운명적 만남.", @pokemon.obtain_level)
  ...
  def drawPageThree
    ...
    if ability
  textpos.push([
    ability.name, 362, 290,
    :left, Color.new(64, 64, 64), Color.new(176, 176, 176)
  ])

  overlay.font.size = 16

  # 드디어! 정확한 drawTextEx 시그니처 (8개 인자)
  drawTextEx(
    overlay,                   # bitmap
    224, 322, 282, 64,         # x, y, width, height
    ability.description.to_s, # text
    Color.new(64, 64, 64),     # base color
    Color.new(176, 176, 176)   # shadow color
  )
  ...
  def pbOptions
    Kernel.pbMessage("pbOptions called!") 
    return false 
  end
