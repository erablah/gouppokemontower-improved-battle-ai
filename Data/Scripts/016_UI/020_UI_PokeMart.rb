  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if GameData::Item.get(item).is_important?
        next if !pbConfirm(_INTL("\\j[{1},은,는] 다해서 {2}원입니다.\n구매하시겠어요?",
        ...
        quantity = @scene.pbChooseNumber(
          _INTL("\\j[{1},을,를] 몇 개 드릴까요?", itemnameplural), item, maxafford
        )
        next if quantity == 0
        price *= quantity
        if quantity > 1
          next if !pbConfirm(_INTL("{2} {1}개는 다해서 {3}원입니다.\n구매하시겠어요?",
  ...
  def pbSellScreen
    ...
    loop do
      ...
      if !@adapter.canSell?(item)
        pbDisplayPaused(_INTL("죄송하지만 \\j[{1},은,는] 구매하고 있지 않습니다.", itemnameplural))
        next
      end
      ...
      if qty > 1
        qty = @scene.pbChooseNumber(
          _INTL("\\j[{1},을,를] 몇 개 파시겠습니까?", itemnameplural), item, qty
        )
      end
      if qty == 0
        @scene.pbHideMoney
      ...
      price *= qty
      if pbConfirm(_INTL("다해서 {1}원입니다.\n판매하실건가요?", price.to_s_formatted))
        ...
        pbDisplayPaused(_INTL("\\j[{1},을,를] 팔고 {2}원을 받았다.",
