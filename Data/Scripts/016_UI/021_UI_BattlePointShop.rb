...
class Window_BattlePointShop < Window_DrawableCommand
  ...
  def drawItem(index, count, rect)
    ...
    if index == count - 1
      ...
      textpos.push([itemname, rect.x, ypos + 6, :left, self.baseColor, self.shadowColor])
      textpos.push([qty, xQty, ypos + 6, :left, self.baseColor, self.shadowColor])
    end
...
class BattlePointShop_Scene
  ...
  def pbRefresh
    ...
    @sprites["battlepointwindow"].text = _INTL("배틀포인트:\n<r>{1}", @adapter.getBPString)
  end
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if @adapter.getBP < price
        pbDisplayPaused(_INTL("충분한 배틀포인트가 없으신 것 같군요."))
        next
      end
      if GameData::Item.get(item).is_important?
        next if !pbConfirm(_INTL("\\j[{1},으로,로] 교환하시겠어요?\n총 {2} BP입니다.",
        ...
        quantity = @scene.pbChooseNumber(
          _INTL("\\j[{1},을,를] 몇 개 교환하시겠어요?", itemnameplural), item, maxafford
        )
        next if quantity == 0
        price *= quantity
        if quantity > 1
          next if !pbConfirm(_INTL("\\j[{2},을,를] {1}개 교환하시겠어요?\n총 {3} BP입니다.",
      ...
      if @adapter.getBP < price
        pbDisplayPaused(_INTL("죄송하지만, 충분한 배틀포인트가 없으신 것 같군요."))
        next
      end
      added = 0
      quantity.times do
      ...
      if added == quantity
        ...
        pbDisplayPaused(_INTL("여기 있습니다! 감사합니다.")) { pbSEPlay("Mart buy item") }
      else
        added.times do
        ...
        pbDisplayPaused(_INTL("가방에 공간이 없는 것 같군요."))
      end
    end
    @scene.pbEndScene
  end
...
def pbBattlePointShop(stock, speech = nil)
  ...
  if speech.nil?
    pbMessage(_INTL("안녕하세요! 배틀포인트 교환을 도와드리고 있습니다."))
    pbMessage(_INTL("어떤 아이템으로 교환하시겠어요?"))
  else
    pbMessage(speech)
  end
  ...
  screen.pbBuyScreen
  pbMessage(_INTL("방문해주셔서 감사합니다."))
  pbMessage(_INTL("배틀포인트를 모으면 다시 방문해주세요."))
