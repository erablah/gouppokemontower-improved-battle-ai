...
class Window_Pokedex < Window_DrawableCommand
  ...
  def drawItem(index, _count, rect)
    ...
    pbDrawShadowText(self.contents, rect.x + 84, rect.y + 12, rect.width, rect.height,
...
#    end
  ...
  def pbRefresh
    ...
    textpos = [
      [dexname, Graphics.width / 2, 12, :center, Color.new(248, 248, 248), Color.black]
    ]
    textpos.push([GameData::Species.get(iconspecies).name, 112, 60, :center, base, shadow]) if iconspecies
    if @searchResults
      textpos.push([_INTL("Search results"), 112, 316, :center, base, shadow])
      textpos.push([@dexlist.length.to_s, 112, 348, :center, base, shadow])
    else
      textpos.push([_INTL("Seen:"), 42, 312, :left, base, shadow])
      textpos.push([$player.pokedex.seen_count(pbGetPokedexRegion).to_s, 182, 316, :right, base, shadow])
      textpos.push([_INTL("Owned:"), 42, 348, :left, base, shadow])
      textpos.push([$player.pokedex.owned_count(pbGetPokedexRegion).to_s, 182, 348, :right, base, shadow])
    end
    # Draw all text
  ...
  def pbRefreshDexSearch(params, _index)
    ...
    # Write various bits of text
    textpos = [
      [_INTL("Search Mode"), Graphics.width / 2, 12, :center, base, shadow],
      [_INTL("Order"), 136, 66, :center, base, shadow],
      [_INTL("Name"), 58, 124, :center, base, shadow],
      [_INTL("Type"), 58, 176, :center, base, shadow],
      [_INTL("Height"), 58, 228, :center, base, shadow],
      [_INTL("Weight"), 58, 280, :center, base, shadow],
      [_INTL("Color"), 326, 124, :center, base, shadow],
      [_INTL("Shape"), 454, 176, :center, base, shadow],
      [_INTL("Reset"), 80, 348, :center, base, shadow, 1],
      [_INTL("Start"), Graphics.width / 2, 348, :center, base, shadow, :outline],
      [_INTL("Cancel"), Graphics.width - 80, 348, :center, base, shadow, :outline]
    ]
    # Write order, name and color parameters
    textpos.push([@orderCommands[params[0]], 344, 68, :center, base, shadow, :outline])
    textpos.push([(params[1] < 0) ? "----" : @nameCommands[params[1]], 176, 126, :center, base, shadow, :outline])
    textpos.push([(params[8] < 0) ? "----" : @colorCommands[params[8]].name, 444, 126, :center, base, shadow, :outline])
    # Draw type icons
    if params[2] >= 0
      ...
      textpos.push(["----", 176, 178, :center, base, shadow, :outline])
    end
    if params[3] >= 0
      ...
      textpos.push(["----", 304, 178, :center, base, shadow, :outline])
    end
    ...
    if System.user_language[3..4] == "US"   # If the user is in the United States
      ...
      textpos.push([sprintf("%d'%02d''", ht1 / 12, ht1 % 12), 166, 230, :center, base, shadow, :outline])
      textpos.push([sprintf("%d'%02d''", ht2 / 12, ht2 % 12), 294, 230, :center, base, shadow, :outline])
      textpos.push([sprintf("%.1f", wt1 / 10.0), 166, 282, :center, base, shadow, :outline])
      textpos.push([sprintf("%.1f", wt2 / 10.0), 294, 282, :center, base, shadow, :outline])
      hwoffset = true
    else
      textpos.push([sprintf("%.1f", ht1 / 10.0), 166, 230, :center, base, shadow, :outline])
      textpos.push([sprintf("%.1f", ht2 / 10.0), 294, 230, :center, base, shadow, :outline])
      textpos.push([sprintf("%.1f", wt1 / 10.0), 166, 282, :center, base, shadow, :outline])
      textpos.push([sprintf("%.1f", wt2 / 10.0), 294, 282, :center, base, shadow, :outline])
    end
  ...
  def pbRefreshDexSearchParam(mode, cmds, sel, _index)
    ...
    when 2   # Type icons
      2.times do |i|
        if !sel[i] || sel[i] < 0
          textpos.push(["----", 298 + (128 * i), 68, :center, base, shadow, :outline])
        else
    ...
    when 3   # Height range
      ...
      textpos.push([txt1, 286, 68, :center, base, shadow, :outline])
      textpos.push([txt2, 414, 68, :center, base, shadow, :outline])
    ...
    when 5   # Color
      if sel[0] < 0
        textpos.push(["----", 362, 68, :center, base, shadow, :outline])
      else
        textpos.push([cmds[sel[0]].name, 362, 68, :center, base, shadow, :outline])
      end
    when 6   # Shape icon
      if sel[0] >= 0
      ...
      if sel[0] < 0
        text = ["----", "-", "----", "", "", "----", ""][mode]
        textpos.push([text, 362, 68, :center, base, shadow, :outline])
      else
        textpos.push([cmds[sel[0]], 362, 68, :center, base, shadow, :outline])
      end
    end
    ...
    if [3, 4].include?(mode)   # Height, weight
      ...
      textpos.push([txt1, xpos1 + halfwidth, ypos1 + 2, :center, base])
      textpos.push([txt2, xpos2 + halfwidth, ypos2 + 2, :center, base])
    else
    ...
    when 0, 1   # Order, name
      cmds.length.times do |i|
        ...
        textpos.push([cmds[i], x, y  + 2, :center, base, shadow, :outline])
      end
      if mode != 0
        textpos.push([(mode == 1) ? "-" : "----",
                      xstart + halfwidth + ((cols - 1) * xgap),
                      ystart + 16 + ((cmds.length / cols).floor * ygap),
    ...
    when 5   # Color
      cmds.length.times do |i|
        x = xstart + halfwidth + ((i % cols) * xgap)
        y = ystart + 14 + ((i / cols).floor * ygap)
        textpos.push([cmds[i].name, x, y + 2, :center, base, shadow, :outline])
      end
      textpos.push(["----",
                    xstart + halfwidth + ((cols - 1) * xgap),
                    ystart + 16 + ((cmds.length / cols).floor * ygap),
