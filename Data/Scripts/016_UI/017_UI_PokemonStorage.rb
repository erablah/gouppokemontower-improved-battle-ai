      ...
      loop do
        ...
        if selected.nil?
          ...
          if @scene.quickswap
            ...
            cmdDebug    = -1
            if heldpoke
              helptext = JosaProcessor.process(_INTL("\\j[{1},을,를] 선택했습니다.", heldpoke.name))
              commands[cmdMove = commands.length] = (pokemon) ? _INTL("Shift") : _INTL("Place")
            elsif pokemon
              helptext = JosaProcessor.process(_INTL("\\j[{1},을,를] 선택했습니다.", pokemon.name))
...
command = pbShowCommands(
  JosaProcessor.process(_INTL("\\j[{1},을,를] 선택했습니다.", pokemon.name)),
  [_INTL("Withdraw"),
   _INTL("Summary"),
   _INTL("Mark"),
   _INTL("Release"),
    ...
    when 2   # Deposit
      ...
      loop do
        ...
        if selected == -3   # Close box
          ...
          next if !pokemon
          command = pbShowCommands(JosaProcessor.process(_INTL("\\j[{1},을,를] 선택했습니다.", pokemon.name)),
                                   [_INTL("Store"),
                                    _INTL("Summary"),
                                    _INTL("Mark"),
                                    _INTL("Release"),
  ...
  def pbStore(selected, heldpoke)
    ...
    if pbAbleCount <= 1 && pbAble?(@storage[box, index]) && !heldpoke
      ...
      pbDisplay(_INTL("\\j[{1},은,는] 박스에 들어가고 싶지 않은 것 같다!", heldpoke.name))
    elsif !heldpoke && @storage[box, index].cannot_store
      pbDisplay(_INTL("\\j[{1},은,는] 박스에 들어가고 싶지 않은 것 같다!", @storage[box, index].name))
    else
      loop do
  ...
  def pbPlace(selected)
    ...
    if @storage[box, index]
      ...
      if index >= @storage.maxPokemon(box)
        ...
        pbDisplay(_INTL("\\j[{1},은,는] 박스에 들어가고 싶지 않은 것 같다!", @heldpkmn.name))
        return
      end
    end
  ...
  def pbRelease(selected, heldpoke)
    ...
    if pokemon.egg?
      ...
      pbDisplay(_INTL("\\j[{1},은,는] 당신 곁을 떠나려 하지 않습니다!", pokemon.name))
      return false
    end
    ...
    if command == 1
      ...
      @scene.pbRefresh
      pbDisplay(JosaProcessor.process(_INTL("\\j[{1},을,를] 놓아주었습니다.", pkmnname)))
  ...
  def pbItem(selected, heldpoke)
    ...
    if pokemon.item
      ...
      if pbConfirm(JosaProcessor.process(_INTL("\\j[{1},을,를] 가져올까?", itemname)))
        if $bag.add(pokemon.item)
          pbDisplay(JosaProcessor.process(_INTL("\\j[{1},을,를] 가져왔다.", itemname)))
          pokemon.item = nil
          @scene.pbHardRefresh
        else
          pbDisplay(JosaProcessor.process(_INTL("\\j[{1},을,를] 보관할 수 없다.", itemname)))
        end
      end
    else
  ...
  def pbChoosePokemon(_party = nil)
    ...
    loop do
      ...
      if selected.nil?
        ...
        helptext = _INTL(JosaProcessor.process("\\j[{1},을,를] 선택했습니다.", pokemon.name))
        command = pbShowCommands(helptext, commands)
        case command
        when 0   # Select
          if pokemon
