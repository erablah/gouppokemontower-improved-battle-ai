...
class PokemonPartyPanel < Sprite
  ...
  def draw_name
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[@pokemon.name, 97, 26, :left, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
  end
  ...
  def draw_hp
    ...
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[hp_text, 224, 69, :right, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
    # HP bar
    if @pokemon.able?
...
class PokemonPartyScreen
  ...
  def pbPokemonMultipleEntryScreenEx(ruleset)
    ...
    loop do
      ...
      command = @scene.pbShowCommands(JosaProcessor.process(_INTL("\\j[{1},은,는] 무엇을 할까?", pkmn.name)), commands) if pkmn
        ...
        if [:MILKDRINK, :SOFTBOILED].include?(move.id)
          ...
          loop do
            ...
            if party_idx == old_party_idx
              pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 스스로에게 쓸 수 없습니다!", pkmn.name, movename))
            elsif newpkmn.egg?
              pbDisplay(_INTL("\\j[{1},은,는] 알에게 쓸 수 없습니다!", movename))
            elsif newpkmn.fainted? || newpkmn.hp == newpkmn.totalhp
              pbDisplay(_INTL("\\j[{1},은,는] 그 포켓몬에게 쓸 수 없습니다.", movename))
            else
              pkmn.hp -= amt
...
MenuHandlers.add(:party_menu_item, :move, {
  ...
  "effect"    => proc { |screen, party, party_idx|
    ...
    screen.scene.pbSetHelpText(JosaProcessor.process(_INTL("\\j[{1},을,를] 어디로 옮길까?", itemname)))
    old_party_idx = party_idx
    moved = false
    loop do
      ...
      if newpkmn.egg?
        ...
        screen.pbRefresh
        screen.pbDisplay(_INTL("{1}에게 \\j[{2},을,를] 지니게 했습니다.", newpkmn.name, portionitemname))
        moved = true
        break
      elsif newpkmn.item.is_mail?
        screen.pbDisplay(_INTL("{1}에게 도구를 주려면 메일을 먼저 제거해야 합니다.", newpkmn.name))
        next
      end
      ...
      if newitemname.starts_with_vowel?
        screen.pbDisplay(_INTL("\\j[{1},은,는] 이미 \\j[{2},을,를] 지니고 있습니다.", newpkmn.name, newitemname) + "\1")
      else
        screen.pbDisplay(_INTL("\\j[{1},은,는] 이미 \\j[{2},을,를] 지니고 있습니다.", newpkmn.name, newitemname) + "\1")
      end
      ...
      screen.scene.pbClearSwitching
      screen.pbRefresh
      screen.pbDisplay(_INTL("{1}에게 \\j[{2},을,를] 지니게 했습니다.", newpkmn.name, portionitemname) + "\1")
      screen.pbDisplay(_INTL("{1}에게 \\j[{2},을,를] 지니게 했습니다.", pkmn.name, newitemname))
      moved = true
      break
    end
...
MenuHandlers.add(:party_menu, :nickname, {
  ...
  "condition" => proc { |screen, party, party_idx|
    pkmn = party[party_idx]
    # 알이 아니고, 외국 포켓몬이 아니며, MECHANICS_GENERATION 9 이상일 때만 활성화 (기존 Summary의 조건 참고)
    next !pkmn.egg? && Settings::MECHANICS_GENERATION >= 9
  },
  "effect"    => proc { |screen, party, party_idx|
    pkmn = party[party_idx]
    new_nickname = pbEnterPokemonName(_INTL("{1}'s nickname?", pkmn.name), 0, Pokemon::MAX_NAME_SIZE, "", pkmn, true)
    if new_nickname && new_nickname != pkmn.name
      pkmn.name = new_nickname
      # 이름이 바뀌면 화면을 갱신합니다.
      screen.pbRefreshSingle(party_idx)
      screen.pbDisplay(_INTL("{1}의 별명이 {2}로 변경되었습니다.", pkmn.name, new_nickname))
    end
  }
})

# 새로 추가된 :moves 핸들러 (Order 70)
MenuHandlers.add(:party_menu, :remember, {
  "name"      => _INTL("기술 떠올리기"),
  "order"     => 70,
  "condition" => proc { |screen, party, party_idx|
    pkmn = party[party_idx]
    # 알이 아니고, 기술을 다시 배울 수 있으며, MECHANICS_GENERATION 9 이상일 때만 활성화 (기존 Summary의 조건 참고)
    next !pkmn.egg? && pkmn.can_relearn_move? && Settings::MECHANICS_GENERATION >= 9
  },
  "effect"    => proc { |screen, party, party_idx|
    pkmn = party[party_idx]
    
    # 기술을 다시 배우는 화면을 호출합니다.
    pbRelearnMoveScreen(pkmn)
    
    # 기술을 변경했거나 배웠을 수 있으므로 화면을 갱신합니다.
    screen.pbRefreshSingle(party_idx)
    screen.scene.pbClearSwitching
  }
})
