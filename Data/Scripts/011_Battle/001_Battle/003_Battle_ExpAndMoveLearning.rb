class Battle
  ...
  def pbGainExp
    ...
    expAll = $game_switches[EXP_ALL_SWITCH]
    p1 = pbParty(0)
    @battlers.each do |b|
  ...
  def pbGainExpOne(idxParty, defeatedBattler, numPartic, expShare, expAll, showMessages = true)
    ...
    exp = (exp * 1.3).floor if Settings::MORE_EXP_FROM_TRAINER_POKEMON && trainerBattle?
    ...
    exp = exp * 2.5 / 2 if $bag.has?(:EXPCHARM)
    ...
    if showMessages
      if isOutsider
        pbDisplayPaused(_INTL("\\j[{1},이,가] {2}의 많은 경험치를 얻었다!", pkmn.name, expGained))
      else
        pbDisplayPaused(_INTL("\\j[{1},이,가] {2}의 경험치를 얻었다!", pkmn.name, expGained))
      end
    end
    curLevel = pkmn.level
    ...
    loop do   # For each level gained in turn...
      ...
      battler&.pbUpdate(false)
      @scene.pbRefreshOne(battler.index) if battler
      pbDisplayPaused(_INTL("\\j[{1},은,는] Lv. {2}이 되었다!", pkmn.name, curLevel)) { pbSEPlay("Pkmn level up") }
  ...
  def pbLearnMove(idxParty, newMove)
    ...
    if pkmn.numMoves < Pokemon::MAX_MOVES
      ...
      pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 배웠다!", pkmnName, moveName)) { pbSEPlay("Pkmn move learnt") }
      if battler
    ...
    pbDisplayPaused(_INTL("\\j[{1},은,는] \\j[{2},을,를] 배우고 싶다...",
                          pkmnName, moveName, pkmn.numMoves.to_word))
    if pbDisplayConfirm(_INTL("\\j[{1},이,가] \\j[{2},을,를] 배울까요?", pkmnName, moveName))
      loop do
        ...
        if forgetMove >= 0
          ...
          pbDisplayPaused(_INTL("\\j[{1},은,는] \\j[{2},을,를] 잊었다! 그리고...", pkmnName, oldMoveName))
          pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 배웠다!", pkmnName, moveName)) { pbSEPlay("Pkmn move learnt") }
          battler&.pbCheckFormOnMovesetChange
          break
        elsif pbDisplayConfirm(_INTL("\\j[{1},을,를] 배우지 않을까요??", moveName))
          pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 배우지 않았다.", pkmnName, moveName))
          break
        end
      end
    else
