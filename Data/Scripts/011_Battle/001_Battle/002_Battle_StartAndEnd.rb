class Battle
  ...
  def pbStartBattleSendOut(sendOuts)
    ...
    [1, 0].each do |side|
      ...
      if side == 0
        ...
        when 3
          msg += _INTL("나와라! {1}, {2}, {3}!", @battlers[sent[0]].name,
  ...
  def pbGainMoney
    ...
    if trainerBattle?
      ...
      tMoney *= 1.5 if @field.effects[PBEffects::AmuletCoin]
      tMoney *= 1.5 if @field.effects[PBEffects::HappyHour]
    ...
    if @field.effects[PBEffects::PayDay] > 0
      @field.effects[PBEffects::PayDay] *= 1.5 if @field.effects[PBEffects::AmuletCoin]
      @field.effects[PBEffects::PayDay] *= 1.5 if @field.effects[PBEffects::HappyHour]
      oldMoney = pbPlayer.money
...
def pbLoseMoney
  ...
  return if pbPlayer.money <= 0

  oldMoney = pbPlayer.money
  lostMoney = oldMoney / 2
  pbPlayer.money -= lostMoney
  $stats.battle_money_lost += lostMoney

  if trainerBattle?
    pbDisplayPaused(_INTL("You gave ${1} to the winner...", lostMoney.to_s_formatted))
  else
    pbDisplayPaused(_INTL("You panicked and dropped ${1}...", lostMoney.to_s_formatted))
  end
end
