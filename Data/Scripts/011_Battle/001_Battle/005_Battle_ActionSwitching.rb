class Battle
  ...
  def pbCanSwitchIn?(idxBattler, idxParty, partyScene = nil)
    ...
    if party[idxParty].fainted?
      partyScene&.pbDisplay(_INTL("\\j[{1},은,는] 기절해서 나갈 수 없다!", party[idxParty].name))
      return false
    end
    if pbFindBattler(idxParty, idxBattler)
      partyScene&.pbDisplay(_INTL("\\j[{1},은,는] 이미 배틀에 나가 있다!", party[idxParty].name))
      return false
    end
    return true
  end
  ...
  def pbCanSwitchOut?(idxBattler, partyScene = nil)
    ...
    if battler.trappedInBattle?
      partyScene&.pbDisplay(_INTL("\\j[{1},은,는] 교체할 수 없다!", battler.pbThis))
      return false
    end
    ...
    allOtherSideBattlers(idxBattler).each do |b|
      ...
      if Battle::AbilityEffects.triggerTrappingByTarget(b.ability, battler, b, self)
        partyScene&.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 교체를 막았다!", b.pbThis, b.abilityName))
        return false
      end
    end
    allOtherSideBattlers(idxBattler).each do |b|
      ...
      if Battle::ItemEffects.triggerTrappingByTarget(b.item, battler, b, self)
        partyScene&.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 교체를 막았다!", b.pbThis, b.itemName))
        return false
      end
    end
    return true
  ...
  def pbCanSwitch?(idxBattler, idxParty = -1, partyScene = nil)
    ...
    allSameSideBattlers(idxBattler).each do |b|
      ...
      partyScene&.pbDisplay(_INTL("\\j[{1},은,는] 이미 선택했다.",
      ...
      @battlers.each do |b|
        ...
        if !pbOwnedByPlayer?(idxBattler)   # Opponent/ally is switching in
          ...
          if @internalBattle && @switchStyle && trainerBattle? && pbSideSize(0) == 1 &&
            ...
            if pbDisplayConfirm(_INTL("\\j[{1},은,는] \\j[{2},을,를] 내보내려는 것 같다. 포켓몬을 교체할까?",
  ...
  def pbEffectsOnBattlerEnteringPosition(battler)
    ...
    if position.effects[PBEffects::LunarDance]
      ...
      if battler.canHeal? || battler.status != :NONE || !full_pp
        ...
        pbDisplay(_INTL("\\j{1},은,는] 초승달춤을 추고 쓰러졌다!", battler.pbThis))
        battler.pbRecoverHP(battler.totalhp)
        battler.pbCureStatus(false)
  ...
  def pbEntryHazards(battler)
    ...
    if battler_side.effects[PBEffects::Spikes] > 0 && battler.takesIndirectDamage? &&
      ...
      pbDisplay(_INTL("\\j[{1},은,는] 압정에 찔렸다!", battler.pbThis))
      battler.pbItemHPHealCheck
    end
    # Toxic Spikes
    if battler_side.effects[PBEffects::ToxicSpikes] > 0 && !battler.fainted? && !battler.airborne?
      if battler.pbHasType?(:POISON)
        ...
        pbDisplay(_INTL("\\j[{1},은,는] 독압정에 찔렸다!", battler.pbThis))
      elsif battler.pbCanPoison?(nil, false) && !battler.hasActiveItem?(:HEAVYDUTYBOOTS)
        if battler_side.effects[PBEffects::ToxicSpikes] == 2
          battler.pbPoison(nil, _INTL("\\j[{1},은,는] 독압정에 찔려 맹독에 걸렸다!", battler.pbThis), true)
        else
          battler.pbPoison(nil, _INTL("\\j[{1},은,는] 독압정에 찔려 독에 걸렸다!", battler.pbThis))
        end
      end
    end
    # Sticky Web
    if battler_side.effects[PBEffects::StickyWeb] && !battler.fainted? && !battler.airborne? &&
      ...
      pbDisplay(_INTL("\\j[{1},은,는] 끈적끈적네트에 걸렸다!", battler.pbThis))
