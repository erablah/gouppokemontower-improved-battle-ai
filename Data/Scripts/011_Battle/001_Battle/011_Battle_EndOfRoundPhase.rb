class Battle
  ...
  def pbEORWeatherDamage(battler)
    ...
    when :Sandstorm
      ...
      pbDisplay(_INTL("모래바람이 \\j[{1},을,를] 덮쳤다!", battler.pbThis))
      amt = battler.totalhp / 16
    when :Hail
      return if !battler.takesHailDamage?
      pbDisplay(_INTL("싸라기눈이 \\j[{1},을,를] 덮쳤다!", battler.pbThis))
      amt = battler.totalhp / 16
    when :ShadowSky
      return if !battler.takesShadowSkyDamage?
      pbDisplay(_INTL("\\j[{1},은,는] 검은 하늘의 영향으로 데미지를 입었다!", battler.pbThis))
      amt = battler.totalhp / 16
    end
    return if amt < 0
  ...
  def pbEORUseFutureSight(position, position_index)
    ...
    pbDisplay(_INTL("\\j[{1},은,는] {2} 공격을 받았다!", @battlers[position_index].pbThis,
    ...
    2.times do |side|
      ...
      priority.each do |battler|
        ...
        battler.pbTakeEffectDamage(battler.totalhp / 8, false) do |hp_lost|
          pbDisplay(_INTL("\\j[{1},은,는] 파도치는 화염에 데미지를 받았다!", battler.pbThis))
        end
      end
    end
  end
  ...
  def pbEORHealingEffects(priority)
    ...
    priority.each do |battler|
      ...
      hpGain = (hpGain * 1.3).floor if battler.hasActiveItem?(:BIGROOT)
      battler.pbRecoverHP(hpGain)
      pbDisplay(_INTL("\\j[{1},이,가] 뿌리에서 영양분을 흡수했다!", battler.pbThis))
    end
    # Leech Seed
    priority.each do |battler|
      ...
      battler.pbTakeEffectDamage(battler.totalhp / 8) do |hp_lost|
        recipient.pbRecoverHPFromDrain(hp_lost, battler,
                                       _INTL("\\j[{1},이,가] 뿌린 씨를 통해 체력을 흡수했다!", battler.pbThis))
      ...
      if battler.hasActiveAbility?(:POISONHEAL)
        if battler.canHeal?
          ...
          if Scene::USE_ABILITY_SPLASH
            ...
            pbDisplay(_INTL("{1}의 \\j[{2},이,가] 체력을 회복시켰다.", battler.pbThis, battler.abilityName))
          end
  ...
  def pbEOREffectDamage(priority)
    ...
    priority.each do |battler|
      ...
      battler.pbTakeEffectDamage(battler.totalhp / 4) do |hp_lost|
        pbDisplay(_INTL("\\j[{1},은,는] 악몽을 꾸고 있다!", battler.pbThis))
      end
    end
    # Curse
    priority.each do |battler|
      ...
      battler.pbTakeEffectDamage(battler.totalhp / 4) do |hp_lost|
        pbDisplay(_INTL("\\j[{1},은,는] 저주받고 있다!", battler.pbThis))
      end
    end
  end
  ...
  def pbEORTrappingDamage(battler)
    ...
    if battler.effects[PBEffects::Trapping] == 0
      pbDisplay(_INTL("\\j[{1},은,는] {2}에서 풀려났다!", battler.pbThis, move_name))
      return
    end
    ...
    battler.pbTakeEffectDamage(hpLoss, false) do |hp_lost|
      pbDisplay(_INTL("\\j[{1},은,는] {2}의 데미지를 받고 있다!", battler.pbThis, move_name))
    end
  end
  ...
  def pbEOREndBattlerEffects(priority)
    ...
    pbEORCountDownBattlerEffect(priority, PBEffects::Taunt) do |battler|
      pbDisplay(_INTL("\\j[{1},은,는] 도발에서 풀려났다!", battler.pbThis))
    end
    # Encore
    priority.each do |battler|
      ...
      if idxEncoreMove >= 0
        ...
        if battler.effects[PBEffects::Encore] == 0 || battler.moves[idxEncoreMove].pp == 0
          ...
          pbDisplay(_INTL("\\j[{1},은,는] 앙코르 상태가 끝났다!", battler.pbThis))
        end
      else
    ...
    pbEORCountDownBattlerEffect(priority, PBEffects::Disable) do |battler|
      ...
      pbDisplay(_INTL("\\j[{1},은,는] 사슬묶기에서 벗어났다!", battler.pbThis))
    end
    # Magnet Rise
    ...
    pbEORCountDownBattlerEffect(priority, PBEffects::Telekinesis) do |battler|
      pbDisplay(_INTL("\\j[{1},은,는] 텔레키네시스에서 풀려났다!", battler.pbThis))
    end
    # Heal Block
    pbEORCountDownBattlerEffect(priority, PBEffects::HealBlock) do |battler|
      pbDisplay(_INTL("\\j[{1},은,는] 회복봉인에서 풀려났다!", battler.pbThis))
    end
    # Embargo
    pbEORCountDownBattlerEffect(priority, PBEffects::Embargo) do |battler|
      pbDisplay(_INTL("\\j[{1},은,는] 도구를 다시 사용할 수 있게 되었다!", battler.pbThis))
  ...
  def pbEOREndSideEffects(side, priority)
    ...
    pbEORCountDownSideEffect(side, PBEffects::AuroraVeil,
                             _INTL("{1}의 오로라베일이 사라졌다!", @battlers[side].pbTeam))
  end
  ...
  def pbEOREndBattlerSelfEffects(battler)
    ...
    if battler.inHyperMode?
      if pbRandom(100) < 10
        ...
        pbDisplay(_INTL("\\j[{1},은,는] 정신을 차렸다!", battler.pbThis))
      else
        pbDisplay(_INTL("\\j[{1},은,는] 하이퍼 모드 상태다!", battler.pbThis))
      end
    end
    # Uproar
    if battler.effects[PBEffects::Uproar] > 0
      ...
      if battler.effects[PBEffects::Uproar] == 0
        pbDisplay(_INTL("\\j[{1},은,는] 진정했다!", battler.pbThis))
      else
        pbDisplay(_INTL("\\j[{1},은,는] 소란 피우고 있다!", battler.pbThis))
      end
    end
    ...
    if battler.effects[PBEffects::SlowStart] > 0
      ...
      if battler.effects[PBEffects::SlowStart] == 0
        pbDisplay(_INTL("\\j[{1},은,는] 드디어 기운을 차렸다!", battler.pbThis))
      end
    end
  end
  ...
  def pbEORShiftDistantBattlers
    ...
    if !singleBattle?
      ...
      swaps.each do |pair|
        ...
        when 2
          pbDisplay(_INTL("\\j[{1},은,는] 자리를 옮겼다!", @battlers[pair[1]].pbThis))
        when 3
          pbDisplay(_INTL("\\j[{1},은,는] 센터로 이동했다!", @battlers[pair[1]].pbThis))
        end
      end
    end
  end
  ...
  def pbEndOfRoundPhase
    ...
    if Settings::AFFECTION_EFFECTS && @internalBattle
      priority.each do |battler|
        ...
        case old_status
        when :SLEEP
          pbDisplay(_INTL("\\j[{1},은,는] 당신이 걱정하지 않도록 잠에서 일어났다!", battler.pbThis))
        when :POISON
          pbDisplay(_INTL("\\j[{1},은,는] 당신이 걱정하지 않도록 독 상태에서 벗어났다!", battler.pbThis))
        when :BURN
          pbDisplay(_INTL("\\j[{1},은,는] 당신이 걱정하지 않도록 화상 상태에서 벗어났다!", battler.pbThis))
        when :PARALYSIS
          pbDisplay(_INTL("\\j[{1},은,는] 당신이 걱정하지 않도록 마비 상태에서 벗어났다!", battler.pbThis))
        when :FROZEN
          pbDisplay(_INTL("\\j[{1},은,는] 당신이 걱정하지 않도록 얼음을 녹였다!", battler.pbThis))
        end
      end
    end
    ...
    priority.each do |battler|
      ...
      pbDisplay(_INTL("하이퍼 모드의 공격으로 \\j[{1},이,가] 피해를 입었다!", battler.pbThis(true)))
      battler.pbFaint if battler.fainted?
    end
