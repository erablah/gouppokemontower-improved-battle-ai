  ...
  def pbRun(idxBattler, duringBattle = false)
    ...
    if !duringBattle
      ...
      if battler.itemActive? &&
         Battle::ItemEffects.triggerCertainEscapeFromBattle(battler.item, battler)
        pbSEPlay("Battle flee")
        pbDisplayPaused(_INTL("\\j[{1},은,는] \\j[{2},을,를] 사용해 도망쳤다!", battler.pbThis, battler.itemName))
        @decision = 3
        return 1
      end
      ...
      allOtherSideBattlers(idxBattler).each do |b|
        ...
        if Battle::AbilityEffects.triggerTrappingByTarget(b.ability, battler, b, self)
          pbDisplayPaused(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 도망치는 것을 막았다!", b.pbThis, b.abilityName))
          return 0
        end
      end
      allOtherSideBattlers(idxBattler).each do |b|
        ...
        if Battle::ItemEffects.triggerTrappingByTarget(b.item, battler, b, self)
          pbDisplayPaused(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 도망치는 것을 막았다!", b.pbThis, b.itemName))
          return 0
        end
      end
    end
