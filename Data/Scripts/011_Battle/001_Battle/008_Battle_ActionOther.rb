class Battle
  ...
  def pbCall(idxBattler)
    ...
    pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 불렀다!", trainerName, battler.pbThis(true)))
    pbDisplay(_INTL("{1}!", battler.name))
    if battler.shadowPokemon?
      if battler.inHyperMode?
        battler.pokemon.hyper_mode = false
        battler.pokemon.change_heart_gauge("call")
        pbDisplay(_INTL("\\j[{1},은,는] 트레이너의 부름에 정신을 차렸다!", battler.pbThis))
      else
  ...
  def pbMegaEvolve(idxBattler)
    ...
    when 1   # Rayquaza
      ...
      pbDisplay(_INTL("{1}의 \\j[{2},이,가] {3}의 {4}에 반응하고 있다!",
    ...
    pbDisplay(_INTL("\\j[{1},은,는] 메가진화하여 \\j[{2},이,가] 되었다!", battler.pbThis, megaName))
