...
Battle::AbilityEffects::OnHPDroppedBelowHalf.add(:EMERGENCYEXIT,
  proc { |ability, battler, move_user, battle|
    ...
    if battle.wildBattle?
      ...
      battle.pbDisplay(_INTL("\\j[{1},은,는] 도망쳤다!", battler.pbThis))
    ...
    battle.pbDisplay(_INTL("\\j[{1},은,는] {2}에게 돌아갔다!",
  ...
  proc { |ability, battler, user, status|
    ...
    when :POISON
      if user.pbCanPoisonSynchronize?(battler)
        ...
        msg = nil
        if !Battle::Scene::USE_ABILITY_SPLASH
          msg = _INTL("{1}의 \\j[{2},이,가] \\j[{3},을,를] 중독시켰다!", battler.pbThis, battler.abilityName, user.pbThis(true))
        end
    ...
    when :BURN
      if user.pbCanBurnSynchronize?(battler)
        ...
        msg = nil
        if !Battle::Scene::USE_ABILITY_SPLASH
          msg = _INTL("{1}의 \\j[{2},이,가] {3}에게 화상을 입혔다!", battler.pbThis, battler.abilityName, user.pbThis(true))
        end
        user.pbBurn(nil, msg)
        battler.battle.pbHideAbilitySplash(battler)
      end
    when :PARALYSIS
      if user.pbCanParalyzeSynchronize?(battler)
        ...
        msg = nil
        if !Battle::Scene::USE_ABILITY_SPLASH
          msg = _INTL("{1}의 \\j[{2},이,가] \\j[{3},을,를] 마비시켰다! 움직이지 못할 수도 있다!",
...
Battle::AbilityEffects::StatusCure.add(:IMMUNITY,
  proc { |ability, battler|
    ...
    if !Battle::Scene::USE_ABILITY_SPLASH
      battler.battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 독을 치료했다!", battler.pbThis, battler.abilityName))
    end
    battler.battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::StatusCure.add(:INSOMNIA,
  proc { |ability, battler|
    ...
    battler.pbCureStatus(Battle::Scene::USE_ABILITY_SPLASH)
    if !Battle::Scene::USE_ABILITY_SPLASH
      battler.battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 잠에서 깨웠다!", battler.pbThis, battler.abilityName))
    end
    battler.battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::StatusCure.add(:LIMBER,
  proc { |ability, battler|
    ...
    battler.pbCureStatus(Battle::Scene::USE_ABILITY_SPLASH)
    if !Battle::Scene::USE_ABILITY_SPLASH
      battler.battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 마비를 치료했다!", battler.pbThis, battler.abilityName))
    end
    battler.battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::StatusCure.add(:MAGMAARMOR,
  proc { |ability, battler|
    ...
    battler.pbCureStatus(Battle::Scene::USE_ABILITY_SPLASH)
    if !Battle::Scene::USE_ABILITY_SPLASH
      battler.battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 얼음을 녹였다!", battler.pbThis, battler.abilityName))
    end
    battler.battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::StatusCure.add(:OBLIVIOUS,
  proc { |ability, battler|
    ...
    if battler.effects[PBEffects::Attract] >= 0
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        battler.battle.pbDisplay(_INTL("\\j[{1},은,는] 헤롱헤롱 상태에서 벗어났다!", battler.pbThis))
      else
        battler.battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 헤롱헤롱 상태를 벗어나게 했다!",
    ...
    if battler.effects[PBEffects::Taunt] > 0 && Settings::MECHANICS_GENERATION >= 6
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        battler.battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 도발 상태를 풀어주었다!",
           battler.pbThis, battler.abilityName))
      end
    end
    battler.battle.pbHideAbilitySplash(battler)
...
Battle::AbilityEffects::StatusCure.add(:OWNTEMPO,
  proc { |ability, battler|
    ...
    if Battle::Scene::USE_ABILITY_SPLASH
      battler.battle.pbDisplay(_INTL("\\j[{1},은,는] 혼란에서 벗어났다!", battler.pbThis))
    else
      battler.battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 혼란을 벗어나게 했다!",
         battler.pbThis, battler.abilityName))
    end
    battler.battle.pbHideAbilitySplash(battler)
  }
...
Battle::AbilityEffects::StatusCure.add(:WATERVEIL,
  proc { |ability, battler|
    ...
    battler.pbCureStatus(Battle::Scene::USE_ABILITY_SPLASH)
    if !Battle::Scene::USE_ABILITY_SPLASH
      battler.battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 화상을 치료했다!", battler.pbThis, battler.abilityName))
    end
    battler.battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::StatLossImmunity.add(:BIGPECKS,
  proc { |ability, battler, stat, battle, showMessages|
    ...
    if showMessages
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("{1}의 \\j[{2},은,는] 떨어지지 않는다!", battler.pbThis, GameData::Stat.get(stat).name))
      else
        battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] \\j[{3},이,가] 떨어지는 걸 막았다!", battler.pbThis,
  ...
  proc { |ability, battler, stat, battle, showMessages|
    if showMessages
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 능력치 감소를 막았다!", battler.pbThis, battler.abilityName))
      end
      battle.pbHideAbilitySplash(battler)
    end
    next true
  ...
  proc { |ability, bearer, battler, stat, battle, showMessages|
    ...
    if showMessages
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] {3}의 능력치 감소를 막았다!",
...
Battle::AbilityEffects::PriorityBracketUse.add(:QUICKDRAW,
  proc { |ability, battler, battle|
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("\\j[{1},이,가] \\j[{2},을,를] 빠르게 행동하게 해주었다!", battler.abilityName, battler.pbThis(true)))
    battle.pbHideAbilitySplash(battler)
  }
)
  ...
  proc { |ability, user, target, move, type, battle, show_message|
    ...
    if show_message
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] {3}의 효과를 낮췄다!",
    ...
    if show_message
      ...
      if !target.effects[PBEffects::FlashFire]
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          battle.pbDisplay(_INTL("{1}의 \\j[{2},으로,로] 인해 불꽃 타입 기술의 위력이 올랐다!",
  ...
  proc { |ability, user, target, move, type, battle, show_message|
    ...
    if show_message
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] \\j[{3},을,를] 막았다!", target.pbThis, target.abilityName, move.name))
      end
      battle.pbHideAbilitySplash(target)
    end
    next true
...
Battle::AbilityEffects::MoveImmunity.add(:TELEPATHY,
  proc { |ability, user, target, move, type, battle, show_message|
    ...
    if show_message
      battle.pbShowAbilitySplash(target)
      if Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("\\j[{1},은,는] 아군의 공격을 피했다!", target.pbThis(true)))
      else
        battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 아군의 공격을 피했다!",
  ...
  proc { |ability, user, target, move, type, battle, show_message|
    ...
    if show_message
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 피해를 막았다!!", target.pbThis, target.abilityName))
      end
      battle.pbHideAbilitySplash(target)
    end
    next true
  ...
  proc { |ability, user, target, move, battle|
    ...
    if !battle.moldBreaker
      ...
      if dampBattler
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 사용할 수 없다!", target.pbThis, target.abilityName))
        else
          battle.pbDisplay(_INTL("\\j[{1},은,는] {3}의 \\j[{4},으로,로] 인해 \\j[{2},을,를] 사용할 수 없다!",
    ...
    if user.takesIndirectDamage?(Battle::Scene::USE_ABILITY_SPLASH) &&
      ...
      battle.pbDisplay(_INTL("\\j[{1},은,는] 여파에 휘말렸다!", user.pbThis))
    end
    battle.pbHideAbilitySplash(target)
  }
)
...
Battle::AbilityEffects::OnBeingHit.add(:ANGERPOINT,
  proc { |ability, user, target, move, battle|
    ...
    if Battle::Scene::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 최대치까지 올랐다!", target.pbThis, GameData::Stat.get(:ATTACK).name))
    else
      battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] \\j[{3},을,를] 최대치로 올렸다!",
...
Battle::AbilityEffects::OnBeingHit.add(:CURSEDBODY,
  proc { |ability, user, target, move, battle|
    ...
    if !move.pbMoveFailedAromaVeil?(target, user, Battle::Scene::USE_ABILITY_SPLASH)
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("{1}의 \\j[{2},은,는] 봉인되었다!", user.pbThis, regularMove.name))
      else
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] {3}의 \\j[{4},으로,로] 인해 봉인되었다!",
...
Battle::AbilityEffects::OnBeingHit.add(:CUTECHARM,
  proc { |ability, user, target, move, battle|
    ...
    if user.pbCanAttract?(target, Battle::Scene::USE_ABILITY_SPLASH) &&
      ...
      msg = nil
      if !Battle::Scene::USE_ABILITY_SPLASH
        msg = _INTL("{1}의 \\j[{2},이,가] \\j[{3},을,를] 헤롱헤롱하게 만들었다!", target.pbThis,
    ...
    if user.affectedByPowder?(Battle::Scene::USE_ABILITY_SPLASH) &&
      ...
      when 0
        if user.pbCanSleep?(target, Battle::Scene::USE_ABILITY_SPLASH)
          msg = nil
          if !Battle::Scene::USE_ABILITY_SPLASH
            msg = _INTL("{1}의 \\j[{2},이,가] \\j[{3},을,를] 잠들게 만들었다!", target.pbThis,
               target.abilityName, user.pbThis(true))
          end
          user.pbSleep(msg)
        end
      when 1
        if user.pbCanPoison?(target, Battle::Scene::USE_ABILITY_SPLASH)
          msg = nil
          if !Battle::Scene::USE_ABILITY_SPLASH
            msg = _INTL("{1}의 \\j[{2},이,가] \\j[{3},을,를) 독 상태로 만들었다!", target.pbThis,
               target.abilityName, user.pbThis(true))
          end
...
Battle::AbilityEffects::OnBeingHit.add(:INNARDSOUT,
  proc { |ability, user, target, move, battle|
    ...
    if user.takesIndirectDamage?(Battle::Scene::USE_ABILITY_SPLASH)
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("\\j[{1},은,는] 상처를 입었다!", user.pbThis))
      else
        battle.pbDisplay(_INTL("\\j[{1},은,는] {2}의 \\j[{3},으로,로] 인해 상처를 입었다!", user.pbThis,
           target.pbThis(true), target.abilityName))
      end
    end
    battle.pbHideAbilitySplash(target)
...
Battle::AbilityEffects::OnBeingHit.add(:MUMMY,
  proc { |ability, user, target, move, battle|
    ...
    if user.affectedByContactEffect?(Battle::Scene::USE_ABILITY_SPLASH)
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("{1}의 특성이 \\j[{2},으로,로] 바뀌었다!", user.pbThis, user.abilityName))
      else
        battle.pbDisplay(_INTL("{1}의 특성이 \\j[{3},으로,로] 인해 \\j[{2},으로,로] 바뀌었다!",
  ...
  proc { |ability, user, target, move, battle|
    ...
    if user.affectedByContactEffect?(Battle::Scene::USE_ABILITY_SPLASH)
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        battle.pbDisplay(_INTL("{1}의 \\j[{2},으로,로] 인해 양쪽 포켓몬 모두 3턴 후 쓰러진다!",
           target.pbThis(true), target.abilityName))
      end
    end
    battle.pbHideAbilitySplash(target)
...
Battle::AbilityEffects::OnBeingHit.add(:POISONPOINT,
  proc { |ability, user, target, move, battle|
    ...
    if user.pbCanPoison?(target, Battle::Scene::USE_ABILITY_SPLASH) &&
      ...
      msg = nil
      if !Battle::Scene::USE_ABILITY_SPLASH
        msg = _INTL("{1}의 \\j[{2},이,가] \\j[{3},을,를] 중독시켰다!", target.pbThis, target.abilityName, user.pbThis(true))
      end
      user.pbPoison(target, msg)
    end
    battle.pbHideAbilitySplash(target)
...
Battle::AbilityEffects::OnBeingHit.add(:WANDERINGSPIRIT,
  proc { |ability, user, target, move, battle|
    ...
    if user.affectedByContactEffect?(Battle::Scene::USE_ABILITY_SPLASH)
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] \\j[{2},과,와] 특성을 바꾸었다!", target.pbThis, user.pbThis(true))))
      else
        battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 자신의 {2} 특성과 {3}의 {4} 특성을 바꾸었다!",
...
Battle::AbilityEffects::OnDealingHit.add(:POISONTOUCH,
  proc { |ability, user, target, move, battle|
    ...
    if target.hasActiveAbility?(:SHIELDDUST) && !battle.moldBreaker
      ...
      msg = nil
      if !Battle::Scene::USE_ABILITY_SPLASH
        msg = _INTL("{1}의 \\j[{2},이,가] \\j[{3},을,를] 중독시켰다!", user.pbThis, user.abilityName, target.pbThis(true))
      end
...
Battle::AbilityEffects::OnEndOfUsingMove.add(:MAGICIAN,
  proc { |ability, user, targets, move, battle|
    ...
    targets.each do |b|
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("\\j[{1},이,가] {2}의 \\j[{3},을,를] 훔쳤다!", user.pbThis,
           b.pbThis(true), user.itemName))
      else
        battle.pbDisplay(_INTL("\\j[{1},이,가] \\j[{4},으로,로] {2}의 \\j[{3},을,를] 훔쳤다!", user.pbThis,
...
Battle::AbilityEffects::AfterMoveUseFromTarget.add(:COLORCHANGE,
  proc { |ability, target, user, move, switched_battlers, battle|
    ...
    battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{3},으로,로] 인해 타입이 \\j[{2},으로,로] 바뀌었다!",
...
Battle::AbilityEffects::AfterMoveUseFromTarget.add(:PICKPOCKET,
  proc { |ability, target, user, move, switched_battlers, battle|
    ...
    battle.pbDisplay(_INTL("\\j[{1},은,는] {2}의 \\j[{3},을,를] 소매치기했다!", target.pbThis,
  ...
  proc { |ability, weather, battler, battle|
    ...
    when :Sun, :HarshSun
      if battler.takesIndirectDamage?
        ...
        battle.pbDisplay(_INTL("\\j[{1},은,는] 강한 햇살에 데미지를 입었다!", battler.pbThis))
        battle.pbHideAbilitySplash(battler)
    ...
    when :Rain, :HeavyRain
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 체력을 회복시켰다.", battler.pbThis, battler.abilityName))
      end
      battle.pbHideAbilitySplash(battler)
    end
  }
...
Battle::AbilityEffects::EndOfRoundWeather.add(:ICEFACE,
  proc { |ability, weather, battler, battle|
    ...
    battler.pbChangeForm(0, _INTL("{1}의 모습이 변했다!", battler.pbThis))
    battle.pbHideAbilitySplash(battler)
  }
)
  ...
  proc { |ability, battler, battle|
    ...
    battler.allAllies.each do |b|
      ...
      if !Battle::Scene::USE_ABILITY_SPLASH
        case oldStatus
        when :SLEEP
          battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 파트너를 깨웠다!", battler.pbThis, battler.abilityName))
        when :POISON
          battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 파트너의 독을 치료했다!", battler.pbThis, battler.abilityName))
        when :BURN
          battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 파트너의 화상을 치료했다!", battler.pbThis, battler.abilityName))
        when :PARALYSIS
          battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 파트너의 마비를 치료했다!", battler.pbThis, battler.abilityName))
        when :FROZEN
          battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 파트너의 얼음을 녹였다!", battler.pbThis, battler.abilityName))
        end
      end
      battle.pbHideAbilitySplash(battler)
    end
...
Battle::AbilityEffects::EndOfRoundHealing.add(:HYDRATION,
  proc { |ability, battler, battle|
    ...
    if !Battle::Scene::USE_ABILITY_SPLASH
      case oldStatus
      when :SLEEP
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 잠을 깨웠다!", battler.pbThis, battler.abilityName))
      when :POISON
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 독을 치료했다!", battler.pbThis, battler.abilityName))
      when :BURN
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 화상을 치료했다!", battler.pbThis, battler.abilityName))
      when :PARALYSIS
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 마비를 치료했다!", battler.pbThis, battler.abilityName))
      when :FROZEN
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 얼음을 녹였다!", battler.pbThis, battler.abilityName))
      end
    end
    battle.pbHideAbilitySplash(battler)
  }
  ...
  proc { |ability, battler, battle|
    battle.allOtherSideBattlers(battler.index).each do |b|
      ...
      b.pbTakeEffectDamage(b.totalhp / 8) do |hp_lost|
        if Battle::Scene::USE_ABILITY_SPLASH
          battle.pbDisplay(_INTL("\\j[{1},은,는] 괴로워하고 있다!", b.pbThis))
        else
          battle.pbDisplay(_INTL("\\j[{1},은,는] {2}의 \\j[{3},으로,로] 인해 괴로워하고 있다!",
...
Battle::AbilityEffects::EndOfRoundGainItem.add(:BALLFETCH,
  proc { |ability, battler, battle|
    ...
    battle.pbDisplay(_INTL("\\j[{1},이,가] 던졌던 \\j[{2},을,를] 다시 주워왔다!", battler.pbThis, battler.itemName))
    battle.pbHideAbilitySplash(battler)
...
Battle::AbilityEffects::EndOfRoundGainItem.add(:HARVEST,
  proc { |ability, battler, battle|
    ...
    battler.setInitialItem(battler.item) if !battler.initialItem
    battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 수확했다!", battler.pbThis, battler.itemName))
    battle.pbHideAbilitySplash(battler)
    battler.pbHeldItemTriggerCheck
  }
)
...
Battle::AbilityEffects::EndOfRoundGainItem.add(:PICKUP,
  proc { |ability, battler, battle|
    ...
    battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 발견했다!", battler.pbThis, battler.itemName))
    battle.pbHideAbilitySplash(battler)
    battler.pbHeldItemTriggerCheck
  }
)
...
Battle::AbilityEffects::OnSwitchIn.add(:AIRLOCK,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    if !Battle::Scene::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("{1}의 {2}!", battler.pbThis, battler.abilityName))
    end
...
Battle::AbilityEffects::OnSwitchIn.add(:ANTICIPATION,
  proc { |ability, battler, battle, switch_in|
    ...
    if found
      battle.pbShowAbilitySplash(battler)
      battle.pbDisplay(_INTL("\\j[{1},은,는] 몸을 떨었다!", battler.pbThis))
      battle.pbHideAbilitySplash(battler)
    end
  }
)
...
Battle::AbilityEffects::OnSwitchIn.add(:ASONECHILLINGNEIGH,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("\\j[{1},은,는] 두 가지 특성을 가지고 있다!", battler.pbThis))
    battle.pbHideAbilitySplash(battler)
    battler.ability_id = :UNNERVE
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("\\j[{1},은,는] 긴장해서 나무열매를 먹지 못한다!", battler.pbOpposingTeam))
    battle.pbHideAbilitySplash(battler)
...
Battle::AbilityEffects::OnSwitchIn.add(:AURABREAK,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("\\j[{1},은,는] 다른 포켓몬의 오라를 반전시켰다!", battler.pbThis))
    battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::OnSwitchIn.add(:COMATOSE,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("\\j[{1},은,는] 비몽사몽하다!", battler.pbThis))
    battle.pbHideAbilitySplash(battler)
  }
)
  ...
  proc { |ability, battler, battle, switch_in|
    ...
    battler.allAllies.each do |b|
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        battle.pbDisplay(_INTL("{1}의 능력치 변화가 {2}의 \\j[{3},으로,로] 사라졌다!",
           b.pbThis, battler.pbThis(true), battler.abilityName))
      end
    end
    battle.pbHideAbilitySplash(battler)
...
Battle::AbilityEffects::OnSwitchIn.add(:DARKAURA,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("\\j[{1},은,는] 다크 오라를 뿜어내고 있다!", battler.pbThis))
    battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::OnSwitchIn.add(:FAIRYAURA,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("\\j[{1},은,는] 페어리 오라를 뿜어내고 있다!", battler.pbThis))
    battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::OnSwitchIn.add(:FOREWARN,
  proc { |ability, battler, battle, switch_in|
    ...
    if forewarnMoves.length > 0
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 감지했다!",
          battler.pbThis, forewarnMoveName))
      else
        battle.pbDisplay(_INTL("{1}의 예지력으로 \\j[{2},을,를] 감지했다!",
          battler.pbThis, forewarnMoveName))
      end
      battle.pbHideAbilitySplash(battler)
    end
  ...
  proc { |ability, battler, battle, switch_in|
    ...
    if foes.length > 0
      ...
      if Settings::MECHANICS_GENERATION >= 6
        foes.each do |b|
          battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 수색해 \\j[{3},을,를] 발견했다!",
             battler.pbThis, b.pbThis(true), b.itemName))
        end
      else
        foe = foes[battle.pbRandom(foes.length)]
        battle.pbDisplay(_INTL("\\j[{1},은,는] 상대를 수색해 \\j[{2},을,를] 발견했다!",
...
Battle::AbilityEffects::OnSwitchIn.add(:MOLDBREAKER,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("\\j[{1},은,는] 틀을 깨뜨렸다!", battler.pbThis))
    battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::OnSwitchIn.add(:PASTELVEIL,
  proc { |ability, battler, battle, switch_in|
    ...
    battler.allAllies.each do |b|
      ...
      b.pbCureStatus(Battle::Scene::USE_ABILITY_SPLASH)
      if !Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] {3}의 독을 치료했다!",
...
Battle::AbilityEffects::OnSwitchIn.add(:PRESSURE,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("\\j[{1},은,는] 프레셔를 내뿜고 있다!", battler.pbThis))
    battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::OnSwitchIn.add(:SCREENCLEANER,
  proc { |ability, battler, battle, switch_in|
    ...
    if battler.pbOpposingSide.effects[PBEffects::AuroraVeil] > 0
      battler.pbOpposingSide.effects[PBEffects::AuroraVeil] = 0
      battle.pbDisplay(_INTL("{1}의 오로라베일이 사라졌다!", battler.pbOpposingTeam))
    end
    ...
    if battler.pbOwnSide.effects[PBEffects::AuroraVeil] > 0
      ...
      battle.pbDisplay(_INTL("{1}의 오로라베일이 사라졌다!", battler.pbTeam))
    end
...
Battle::AbilityEffects::OnSwitchIn.add(:SLOWSTART,
  proc { |ability, battler, battle, switch_in|
    ...
    if Battle::Scene::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("\\j[{1},은,는] 기운을 나지 않는다!", battler.pbThis))
    else
      battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 인해 기운을 나지 않는다!",
         battler.pbThis, battler.abilityName))
    end
    battle.pbHideAbilitySplash(battler)
  }
...
Battle::AbilityEffects::OnSwitchIn.add(:TERAVOLT,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("\\j[{1},은,는] 격렬한 오라를 뿜어내고 있다!", battler.pbThis))
    battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::OnSwitchIn.add(:TURBOBLAZE,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("\\j[{1},은,는] 타오르는 오라를 뿜어내고 있다!", battler.pbThis))
    battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::ChangeOnBattlerFainting.add(:POWEROFALCHEMY,
  proc { |ability, battler, fainted, battle|
    ...
    battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 지배당했다!", fainted.pbThis, fainted.abilityName))
    battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::OnTerrainChange.add(:MIMICRY,
  proc { |ability, battler, battle, ability_changed|
    if battle.field.terrain == :None
      ...
      battle.pbDisplay(_INTL("\\j[{1},은,는] 원래 타입으로 돌아왔다!", battler.pbThis))
      battle.pbHideAbilitySplash(battler)
    else
      ...
      if new_type
        ...
        battle.pbDisplay(_INTL("\\j[{1},은,는] {2} 타입으로 변했다!", battler.pbThis, new_type_name))
        battle.pbHideAbilitySplash(battler)
      end
    end
  }
...
Battle::AbilityEffects::DamageCalcFromUser.add(:AMPLIFIEDENERGY,
  proc { |ability, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 4 / 3.0 if !move.contactMove?
  }
)
