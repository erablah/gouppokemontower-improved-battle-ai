module Battle::CatchAndStoreMixin
  ...
  def pbStorePokemon(pkmn)
    ...
    if !pkmn.shadowPokemon?
      if $PokemonSystem.givenicknames == 0 &&
        ...
        nickname = @scene.pbNameEntry(_INTL("{1}의 별명은?", pkmn.speciesName), pkmn)
        pkmn.name = nickname
      end
    end
    # Store the Pokémon
    if pbPlayer.party_full? && (@sendToBoxes == 0 || @sendToBoxes == 2)   # Ask/must add to party
      ...
      loop do
        cmd = pbShowCommands(_INTL("\\j[{1},을,를] 어디로 보낼까요?", pkmn.name), cmds, 99)
        ...
        when 0   # Add to your party
          ...
          pbDisplayPaused(_INTL("\\j[{1},은,는] 박스 \"{2}\"로 보내졌습니다.", send_pkmn.name, box_name))
    ...
    if stored_box < 0
      pbDisplayPaused(_INTL("\\j[{1},이,가] 파티에 추가되었습니다.", pkmn.name))
      @initialItems[0][pbPlayer.party.length - 1] = pkmn.item_id if @initialItems
      return
    end
    # Messages saying the Pokémon was stored in a PC box
    box_name = @peer.pbBoxName(stored_box)
    pbDisplayPaused(_INTL("\\j[{1},은,는] 박스 {2}로 보내졌습니다!", pkmn.name, box_name))
  end
    ...
    @caughtPokemon.each do |pkmn|
      ...
      if !pbPlayer.owned?(pkmn.species)
        ...
        if $player.has_pokedex && $player.pokedex.species_in_unlocked_dex?(pkmn.species)
          pbDisplayPaused(_INTL("{1}의 정보가 도감에 기록되었습니다.", pkmn.name))
  ...
  def pbThrowPokeBall(idxBattler, ball, catch_rate = nil, showPlayer = false)
    ...
    if battler.fainted?
      if itemName.starts_with_vowel?
        pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 던졌다!", pbPlayer.name, itemName))
      else
        pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 던졌다!", pbPlayer.name, itemName))
      end
    ...
    if itemName.starts_with_vowel?
      pbDisplayBrief(_INTL("\\j[{1},은,는] \\j[{2},을,를] 던졌다!", pbPlayer.name, itemName))
    else
      pbDisplayBrief(_INTL("\\j[{1},은,는] \\j[{2},을,를] 던졌다!", pbPlayer.name, itemName))
    end
    ...
    when 3
      ...
      Battle::PokeBallEffects.onFailCatch(ball, self, battler)
    when 4
      pbDisplayBrief(_INTL("좋았어! \\j[{1},을,를] 잡았다!", pkmn.name))
  ...
  def pbCaptureCalc(pkmn, battler, catch_rate, ball)
    ...
    catch_rate = (catch_rate * 1.1).clamp(1, 255)
