...
Battle::ItemEffects::HPHeal.add(:AGUAVBERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :SPECIAL_DEFENSE,
       _INTL("{1}에게 \\j[{2},은,는] 너무 썼다!", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::HPHeal.add(:BERRYJUICE,
  proc { |item, battler, battle, forced|
    ...
    if forced
      ...
      battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 체력을 회복했다!", battler.pbThis, itemName))
    end
    next true
  }
)
...
Battle::ItemEffects::HPHeal.add(:FIGYBERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :ATTACK,
       _INTL("{1}에게 \\j[{2},은,는] 너무 매웠다!", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::HPHeal.add(:IAPAPABERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :DEFENSE,
       _INTL("{1}에게 \\j[{2},은,는] 너무 셨다!", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::HPHeal.add(:LANSATBERRY,
  proc { |item, battler, battle, forced|
    ...
    itemName = GameData::Item.get(item).name
    if forced
      battle.pbDisplay(_INTL("\\j[{1},은,는] {2} 덕분에 기운이 넘친다!", battler.pbThis, itemName))
    else
      battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 기운을 끌어올렸다!", battler.pbThis, itemName))
    end
    next true
  }
)
...
Battle::ItemEffects::HPHeal.add(:MAGOBERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :SPEED,
       _INTL("{1}에게 \\j[{2},은,는] 너무 달았다!", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::HPHeal.add(:MICLEBERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      ...
      battle.pbDisplay(_INTL("\\j[{1},은,는] 다음 기술의 명중률을 높였다!", battler.pbThis))
    else
      battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 다음 기술의 명중률을 높였다!",
...
Battle::ItemEffects::HPHeal.add(:ORANBERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      ...
      battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 약간의 HP를 회복했다!", battler.pbThis, itemName))
    end
    next true
  }
)
...
Battle::ItemEffects::HPHeal.add(:SITRUSBERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      ...
      battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] HP를 회복했다!", battler.pbThis, itemName))
    end
    next true
  }
)
...
Battle::ItemEffects::HPHeal.add(:WIKIBERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :SPECIAL_ATTACK,
       _INTL("{1}에게 \\j[{2},은,는] 너무 텁텁했다!", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::StatusCure.add(:ASPEARBERRY,
  proc { |item, battler, battle, forced|
    ...
    battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 얼음을 녹였다!", battler.pbThis, itemName)) if !forced
    next true
  }
)
...
Battle::ItemEffects::StatusCure.add(:CHERIBERRY,
  proc { |item, battler, battle, forced|
    ...
    battle.pbCommonAnimation("EatBerry", battler) if !forced
    battler.pbCureStatus(forced)
    battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 마비를 치료했다!", battler.pbThis, itemName)) if !forced
    next true
  }
)
...
Battle::ItemEffects::StatusCure.add(:CHESTOBERRY,
  proc { |item, battler, battle, forced|
    ...
    battle.pbCommonAnimation("EatBerry", battler) if !forced
    battler.pbCureStatus(forced)
    battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 잠을 깨웠다!", battler.pbThis, itemName)) if !forced
    next true
  }
)
...
Battle::ItemEffects::StatusCure.add(:LUMBERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 혼란에서 벗어나게 했다!", battler.pbThis)) if oldConfusion
    else
      case oldStatus
      when :SLEEP
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 잠을 깨웠다!", battler.pbThis, itemName))
      when :POISON
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 독을 치료했다!", battler.pbThis, itemName))
      when :BURN
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 화상을 치료했다!", battler.pbThis, itemName))
      when :PARALYSIS
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 마비를 치료했다!", battler.pbThis, itemName))
      when :FROZEN
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 얼음을 녹였다!", battler.pbThis, itemName))
      end
      if oldConfusion
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 혼란에서 벗어나게 했다!", battler.pbThis, itemName))
      end
    end
    next true
  }
...
Battle::ItemEffects::StatusCure.add(:MENTALHERB,
  proc { |item, battler, battle, forced|
    ...
    if battler.effects[PBEffects::Attract] >= 0
      if forced
        battle.pbDisplay(_INTL("\\j[{1},은,는] 헤롱헤롱 상태에서 벗어났다!", battler.pbThis))
      else
        battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 헤롱헤롱 상태에서 벗어났다!",
           battler.pbThis, itemName))
      end
      battler.pbCureAttract
    end
    battle.pbDisplay(_INTL("\\j[{1},은,는] 도발에서 풀려났다!", battler.pbThis)) if battler.effects[PBEffects::Taunt] > 0
    battler.effects[PBEffects::Taunt] = 0
    battle.pbDisplay(_INTL("{1}의 앙코르 상태가 끝났다!", battler.pbThis)) if battler.effects[PBEffects::Encore] > 0
    battler.effects[PBEffects::Encore]     = 0
    ...
    battle.pbDisplay(_INTL("\\j[{1},은,는] 사슬묶기에서 풀려났다!", battler.pbThis)) if battler.effects[PBEffects::Disable] > 0
    battler.effects[PBEffects::Disable] = 0
    battle.pbDisplay(_INTL("\\j[{1},은,는] 회복봉인에서 풀려났다!", battler.pbThis)) if battler.effects[PBEffects::HealBlock] > 0
    battler.effects[PBEffects::HealBlock] = 0
    next true
  }
)
...
Battle::ItemEffects::StatusCure.add(:PECHABERRY,
  proc { |item, battler, battle, forced|
    ...
    battle.pbCommonAnimation("EatBerry", battler) if !forced
    battler.pbCureStatus(forced)
    battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 중독을 치료했다!", battler.pbThis, itemName)) if !forced
    next true
  }
)
...
Battle::ItemEffects::StatusCure.add(:PERSIMBERRY,
  proc { |item, battler, battle, forced|
    ...
    battler.pbCureConfusion
    if forced
      battle.pbDisplay(_INTL("\\j[{1},은,는] 혼란에서 벗어났다!", battler.pbThis))
    else
      battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 혼란에서 벗어나게 했다!", battler.pbThis,
         itemName))
    end
    next true
  }
...
Battle::ItemEffects::StatusCure.add(:RAWSTBERRY,
  proc { |item, battler, battle, forced|
    ...
    battle.pbCommonAnimation("EatBerry", battler) if !forced
    battler.pbCureStatus(forced)
    battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 화상을 치료했다!", battler.pbThis, itemName)) if !forced
    next true
  }
)
...
Battle::ItemEffects::PriorityBracketUse.add(:CUSTAPBERRY,
  proc { |item, battler, battle|
    ...
    battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 빠르게 행동하게 했다!", battler.pbThis, battler.itemName))
...
Battle::ItemEffects::OnMissingTarget.add(:BLUNDERPOLICY,
  proc { |item, user, target, move, hit_num, battle|
    ...
    battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},을,를] 다 써버렸다...", user.itemName)))
...
Battle::ItemEffects::OnBeingHit.add(:AIRBALLOON,
  proc { |item, user, target, move, battle|
    battle.pbDisplay(JosaProcessor.process(_INTL("{1}의 풍선이 터졌다!", target.pbThis, target.itemName)))
...
Battle::ItemEffects::OnBeingHit.add(:JABOCABERRY,
  proc { |item, user, target, move, battle|
    ...
    battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] \\j[{2},을,를] 사용해 {3}에게 데미지를 입혔다!", target.pbThis,
...
Battle::ItemEffects::OnBeingHit.add(:ROCKYHELMET,
  proc { |item, user, target, move, battle|
    ...
    battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 인해 데미지를 입었다!", user.pbThis, target.itemName)))
  }
)
...
Battle::ItemEffects::OnBeingHit.add(:ROWAPBERRY,
  proc { |item, user, target, move, battle|
    ...
    user.pbReduceHP(amt, false)
    battle.pbDisplay(
      JosaProcessor.process(
        _INTL("\\j[{1},은,는] \\j[{2},을,를] 사용해 {3}에게 데미지를 입혔다!",
...
Battle::ItemEffects::OnBeingHit.add(:STICKYBARB,
  proc { |item, user, target, move, battle|
    ...
    battle.pbDisplay(JosaProcessor.process(_INTL("{1}의 \\j[{2},이,가] {3}에게 옮겨졌다!",
...
Battle::ItemEffects::OnBeingHit.add(:WEAKNESSPOLICY,
  proc { |item, user, target, move, battle|
    ...
    battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},을,를] 다 써버렸다...", target.itemName)))
    target.pbHeldItemTriggered(item)
  }
)
...
Battle::ItemEffects::OnBeingHitPositiveBerry.add(:ENIGMABERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      ...
      battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 체력을 회복했다!", battler.pbThis, itemName)))
    end
    next true
  }
)
...
Battle::ItemEffects::AfterMoveUseFromTarget.add(:EJECTBUTTON,
  proc { |item, battler, user, move, switched_battlers, battle|
    ...
    battle.pbCommonAnimation("UseItem", battler)
    battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는,] \\j[{2},으로,로] 인해 교체되었다!", battler.pbThis, battler.itemName)))
    battler.pbConsumeItem(true, false)
    newPkmn = battle.pbGetReplacementPokemonIndex(battler.index)   # Owner chooses
    next if newPkmn < 0
    battle.pbRecallAndReplace(battler.index, newPkmn)
...
Battle::ItemEffects::AfterMoveUseFromTarget.add(:REDCARD,
  proc { |item, battler, user, move, switched_battlers, battle|
    ...
    next if newPkmn < 0
    battle.pbCommonAnimation("UseItem", battler)
    battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] \\j[{3},을,를] 향해 \\j[{2},을,를] 들어 올렸다!",
       battler.pbThis, battler.itemName, user.pbThis(true))))
    battler.pbConsumeItem
    if user.hasActiveAbility?(:SUCTIONCUPS) && !battle.moldBreaker
      battle.pbShowAbilitySplash(user)
      if Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(JosaProcessor.process(_INTL("\\j[1},은,는] 스스로를 고정시켰다!", user.pbThis)))
      else
        battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 자신을 고정시켰다!", user.pbThis, user.abilityName)))
      end
      battle.pbHideAbilitySplash(user)
      next
    end
    if user.effects[PBEffects::Ingrain]
      battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 뿌리로 자신을 고정시켰다!", user.pbThis)))
      next
    end
    battle.pbRecallAndReplace(user.index, newPkmn, true)
    battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 끌려 나왔다!", user.pbThis)))
...
Battle::ItemEffects::AfterMoveUseFromUser.add(:LIFEORB,
  proc { |item, user, targets, move, numHits, battle|
    ...
    battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 생명의구슬로 인해 HP를 조금 잃었다!", user.pbThis)))
...
Battle::ItemEffects::AfterMoveUseFromUser.add(:SHELLBELL,
  proc { |item, user, targets, move, numHits, battle|
    ...
    battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 약간의 HP를 회복했다!",
...
Battle::ItemEffects::OnEndOfUsingMove.add(:LEPPABERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] {2}의 PP를 회복했다!", battler.pbThis, moveName)))
    else
      battle.pbDisplay(JosaProcessor.process(_INTL("{1}의 \\j[{2},이,가] {3}의 PP를 회복시켰다!", battler.pbThis, itemName, moveName)))
    end
    next true
  }
)
...
Battle::ItemEffects::OnEndOfUsingMoveStatRestore.add(:WHITEHERB,
  proc { |item, battler, battle, forced|
    ...
    battle.pbCommonAnimation("UseItem", battler) if !forced
    if forced
      battle.pbDisplay(_INTL("{1}의 능력 변화가 원래대로 돌아왔다!", battler.pbThis))
    else
      battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] \\j[{2},을,를] 사용해 능력변화를 원래대로 되돌렸다!",
         battler.pbThis, itemName)))
    end
    next true
  }
  ...
  proc { |item, battler, battle|
    if battler.pbHasType?(:POISON)
      ...
      battler.pbTakeEffectDamage(battler.totalhp / 8) do |hp_lost|
        battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 인해 데미지를 받았다!", battler.pbThis, battler.itemName))
      end
    end
  }
)
...
Battle::ItemEffects::EndOfRoundEffect.add(:FLAMEORB,
  proc { |item, battler, battle|
    ...
    battler.pbBurn(nil, _INTL("\\j[{1},은,는] \\j[{2},으로,로] 인해 화상을 입었다!", battler.pbThis, battler.itemName))
  }
)
...
Battle::ItemEffects::EndOfRoundEffect.add(:TOXICORB,
  proc { |item, battler, battle|
    ...
    battler.pbPoison(nil, _INTL("\\j[{1},은,는] \\j[{2},으로,로] 인해 맹독 상태가 되었다!",
...
Battle::ItemEffects::OnSwitchIn.add(:AIRBALLOON,
  proc { |item, battler, battle|
    battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 인해 공중에 떠 있다!",
       battler.pbThis, battler.itemName))
  }
)
