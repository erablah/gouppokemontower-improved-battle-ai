...
class BattleArenaBattle < Battle
  ...
  def pbCanSwitchIn?(idxBattler, _idxParty, partyScene = nil)
    partyScene&.pbDisplay(_INTL("\\j[{1},은,는] 교체할 수 없다!", @battlers[idxBattler].pbThis))
    return false
  end
...
#    PBDebug.log("[Mind: #{@mind.inspect}, Skill: #{@skill.inspect}]")
    ...
    if points[0] == points[1]
      pbDisplay(_INTL("\\j[{1},은,는] 심판 판정으로 상대 \\j[{2},과,와] 비겼다!",
      ...
      pbDisplay(_INTL("\\j[{1},은,는] 심판 판정으로 상대 \\j[{2},을,를] 이겼다!",
                      @battlers[0].name, @battlers[1].name))
      @battlers[1].hp = 0
      @battlers[1].pbFaint(false)
    else
      pbDisplay(_INTL("\\j[{1},은,는] 심판 판정으로 상대 {2}에게 졌다!",
                      @battlers[0].name, @battlers[1].name))
      @battlers[0].hp = 0
      @battlers[0].pbFaint(false)
    end
