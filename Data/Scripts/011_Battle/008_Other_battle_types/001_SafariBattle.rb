...
class Battle::Scene
  ...
  def pbSafariCommandMenu(index)
    pbCommandMenuEx(index,
                    [_INTL("\\j[{1},은,는] 무엇을 던질까?", @battle.pbPlayer.name),
                     _INTL("Ball"),
                     _INTL("Bait"),
                     _INTL("Rock"),
                     _INTL("Run")], 3)
...
class SafariBattle
  ...
  def pbStartBattle
    begin
      ...
      pbDisplayPaused(_INTL("야생 \\j[{1},이,가] 나타났다!", pkmn.name))
      @scene.pbSafariStart
      ...
      loop do
        ...
        when 1   # Bait
          pbDisplayBrief(_INTL("\\j[{1},은,는] {2}에게 먹이를 던졌다!!", self.pbPlayer.name, pkmn.name))
          @scene.pbThrowBait
          catchFactor  /= 2 if pbRandom(100) < 90   # Harder to catch
          escapeFactor /= 2                       # Less likely to escape
        when 2   # Rock
          pbDisplayBrief(_INTL("\\j[{1},은,는] {2}에게 돌을 던졌다!", self.pbPlayer.name, pkmn.name))
          @scene.pbThrowRock
        ...
        if @decision == 0
          if @ballCount <= 0
            ...
            pbSEPlay("Battle flee")
            pbDisplay(_INTL("\\j[{1},은,는] 도망쳤다!", pkmn.name))
            @decision = 3
          elsif cmd == 1   # Bait
            pbDisplay(_INTL("\\j[{1},은,는] 먹이를 먹고 있다!", pkmn.name))
          elsif cmd == 2   # Rock
            pbDisplay(_INTL("\\j[{1},은,는] 화가 났다!", pkmn.name))
          else
            pbDisplay(_INTL("\\j[{1},은,는] 신중하게 살펴보고 있다!", pkmn.name))
          end
          # Weather continues
          weather_data = GameData::BattleWeather.try_get(@weather)
          @scene.pbCommonAnimation(weather_data.animation) if weather_data
