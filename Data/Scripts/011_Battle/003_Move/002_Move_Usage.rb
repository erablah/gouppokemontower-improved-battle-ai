class Battle::Move
  ...
  def pbDisplayUseMessage(user)
    @battle.pbDisplayBrief(_INTL("{1}의 {2}!", user.pbThis, @name))
  end
    ...
    target.allAllies.each do |b|
      ...
      if showMessage
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}의 \\j[{3},으로,로] 인해 영향을 받지 않았다!",
  ...
  def pbHitEffectivenessMessages(user, target, numTargets = 1)
    ...
    if target.damageState.critical
      ...
      if target.damageState.affection_critical
        if numTargets > 1
          @battle.pbDisplay(_INTL("\\j[{1},이,가] 칭찬을 받으려 {2}의 급소에 맞췄다!",
                                  user.pbThis, target.pbThis(true)))
        else
          @battle.pbDisplay(_INTL("\\j[{1},이,가] 칭찬을 받으려 급소에 맞췄다!", user.pbThis))
        end
      elsif numTargets > 1
  ...
  def pbEndureKOMessage(target)
    if target.damageState.disguise
      ...
      target.pbChangeForm(1, _INTL("{1}의 모습이 변했다!", target.pbThis))
      @battle.pbHideAbilitySplash(target)
    elsif target.damageState.endured
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 버텨냈다!", target.pbThis))
    elsif target.damageState.sturdy
      @battle.pbShowAbilitySplash(target)
      if Battle::Scene::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 버텨냈다!", target.pbThis))
      else
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 옹골참으로 버텨냈다!", target.pbThis))
      end
      @battle.pbHideAbilitySplash(target)
    elsif target.damageState.focusSash
      @battle.pbCommonAnimation("UseItem", target)
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 기합의띠로 버텨냈다!", target.pbThis))
      target.pbConsumeItem
    elsif target.damageState.focusBand
      @battle.pbCommonAnimation("UseItem", target)
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 기합의띠로 버텨냈다!", target.pbThis))
    elsif target.damageState.affection_endured
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 당신이 슬프지 않게 버텨냈다!", target.pbThis))
    end
  end
