...
class Battle::Move::SleepTargetIfUserDarkrai < Battle::Move::SleepTarget
  def pbMoveFailed?(user, targets)
    if !user.isSpecies?(:DARKRAI) && user.effects[PBEffects::TransformSpecies] != :DARKRAI
      @battle.pbDisplay(_INTL("하지만 \\j[{1},은,는] 기술을 사용할 수 없다!", user.pbThis))
      return true
    end
    return false
  end
...
class Battle::Move::SleepTargetChangeUserMeloettaForm < Battle::Move::SleepTarget
  def pbEndOfMoveUsageEffect(user, targets, numHits, switchedBattlers)
    ...
    user.pbChangeForm(newForm, _INTL("{1}의 모습이 변했다!", user.pbThis))
  end
end
...
class Battle::Move::SleepTargetNextTurn < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 졸리게 만들었다!", user.pbThis, target.pbThis(true)))
  end
end
...
class Battle::Move::GiveUserStatusToTarget < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    when :SLEEP
      target.pbSleep
      msg = _INTL("\\j[{1},은,는] 잠에서 깼다.", user.pbThis)
    when :POISON
...
class Battle::Move::CureUserBurnPoisonParalysis < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    case old_status
    when :BURN
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 화상을 치료했다!", user.pbThis))
    when :POISON
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 독을 치료했다!", user.pbThis))
    when :PARALYSIS
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 마비를 치료했다!", user.pbThis))
    end
  end
end
...
class Battle::Move::CureUserPartyStatus < Battle::Move
  ...
  def pbAromatherapyHeal(pkmn, battler = nil)
    ...
    case oldStatus
    when :SLEEP
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 잠에서 깨어났다.", curedName))
    when :POISON
...
class Battle::Move::StartUserSideImmunityToInflictedStatus < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 신비한 장막에 휩싸였다!", user.pbTeam))
  end
end
...
class Battle::Move::SetUserTypesBasedOnEnvironment < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] {2} 타입으로 변했다!", user.pbThis, typeName))
  end
end
...
class Battle::Move::SetUserTypesToTargetTypes < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}의 타입이 \\j[{2},과,와] 같아졌다!",
...
class Battle::Move::SetTargetTypesToPsychic < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] {2} 타입으로 변했다!", target.pbThis, typeName))
  end
end
...
class Battle::Move::UserLosesFireType < Battle::Move
  ...
  def pbEffectAfterAllHits(user, target)
    if !user.effects[PBEffects::BurnUp]
      ...
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 모든 불을 소진했다!", user.pbThis))
    end
  end
end
...
class Battle::Move::SetTargetAbilityToSimple < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},이,가] 되었다!", target.pbThis, target.abilityName))
...
class Battle::Move::SetUserAbilityToTargetAbility < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}의 \\j[{3},을,를] 복사했다!",
...
class Battle::Move::UserTargetSwapAbilities < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    if Battle::Scene::USE_ABILITY_SPLASH
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 자신의 \\j[{2},을,를] 상대의 \\j[{3},과,와] 맞바꿨다!", user.pbThis))
    else
...
class Battle::Move::StartUserAirborne < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 전자기력으로 떠올랐다!", user.pbThis))
  end
end
...
class Battle::Move::StartTargetAirborneAndAlwaysHitByMoves < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 공중으로 튕겨 올라갔다!", target.pbThis))
  end
end
...
class Battle::Move::HitsTargetInSkyGroundsTarget < Battle::Move
  ...
  def pbEffectAfterAllHits(user, target)
    ...
    target.effects[PBEffects::Telekinesis] = 0
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 그대로 떨어졌다!", target.pbThis))
  end
end
...
class Battle::Move::StartGravity < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.allBattlers.each do |b|
      ...
      if showMessage
        @battle.pbDisplay(_INTL("중력 탓에 \\j[{1},은,는] 공중에 머물 수 없었다!", b.pbThis))
      end
    end
  end
end
