...
class Battle::Move::UserTakesTargetItem < Battle::Move
  def pbEffectAfterAllHits(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}의 \\j[{3},을,를] 훔쳤다!", user.pbThis, target.pbThis(true), itemName))
...
class Battle::Move::TargetTakesUserItem < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] {3}에게서 \\j[{2},을,를] 받았다!", target.pbThis, itemName, user.pbThis(true)))
...
class Battle::Move::UserTargetSwapItems < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 상대와 도구를 바꿨다!", user.pbThis))
    @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 얻었다.", user.pbThis, oldTargetItemName)) if oldTargetItem
    @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 얻었다.", target.pbThis, oldUserItemName)) if oldUserItem
    user.pbHeldItemTriggerCheck
    target.pbHeldItemTriggerCheck
  end
end
...
class Battle::Move::RestoreUserConsumedItem < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    if itemName.starts_with_vowel?
      @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 발견했다!", user.pbThis, itemName))
    else
      @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 발견했다!", user.pbThis, itemName))
    end
    user.pbHeldItemTriggerCheck
...
class Battle::Move::RemoveTargetItem < Battle::Move
  ...
  def pbEffectAfterAllHits(user, target)
    ...
    itemName = target.itemName
    target.pbRemoveItem(false)
    @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 떨어뜨렸다!", target.pbThis, itemName))
  end
end
...
class Battle::Move::DestroyTargetBerryOrGem < Battle::Move
  def pbEffectWhenDealingDamage(user, target)
    ...
    target.pbRemoveItem
    @battle.pbDisplay(_INTL("{1}의 \\j[{2},은,는] 불타 사라졌다!", target.pbThis, item_name))
  end
end
...
class Battle::Move::CorrodeTargetItem < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}의 \\j[{3},을,를] 부식시켰다!",
...
class Battle::Move::StartTargetCannotUseItem < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 더 이상 도구를 사용할 수 없다!", target.pbThis))
  end
end
...
class Battle::Move::UserConsumeBerryRaiseDefense2 < Battle::Move::StatUpMove
  ...
  def pbCanChooseMove?(user, commandPhase, showMessages)
    ...
    if !item || !item.is_berry? || !user.itemActive?
      if showMessages
        msg = _INTL("\\j[{1},은,는] 열매가 없어서 그 기술을 사용할 수 없다!", user.pbThis)
  ...
  def pbEffectGeneral(user)
    super
    @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 먹었다!!", user.pbThis, user.itemName))
    item = user.item
...
class Battle::Move::UserConsumeTargetBerry < Battle::Move
  ...
  def pbEffectAfterAllHits(user, target)
    ...
    user.setBelched
    target.pbRemoveItem
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 상대의 \\j[{2},을,를] 훔쳐 먹었다!", user.pbThis, itemName))
    user.pbHeldItemTriggerCheck(item.id, false)
    user.pbSymbiosis
  end
end
...
class Battle::Move::ThrowUserItemAtTarget < Battle::Move
  ...
  def pbDisplayUseMessage(user)
    ...
    if !@willFail
      @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 던졌다!!", user.pbThis, user.itemName))
    end
  end
