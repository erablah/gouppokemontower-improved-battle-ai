...
class Battle::Move::HealUserFullyAndFallAsleep < Battle::Move::HealingMove
  ...
  def pbEffectGeneral(user)
    user.pbSleepSelf(_INTL("\\j[{1},은,는] 잠들어 회복했다!", user.pbThis), 3)
    super
  end
end
...
class Battle::Move::HealUserByTargetAttackLowerTargetAttack1 < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    if target.hasActiveAbility?(:LIQUIDOOZE, true)
      ...
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 해감액을 흡수했다!", user.pbThis))
...
class Battle::Move::HealUserByHalfOfDamageDoneIfTargetAsleep < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    if !target.asleep?
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 아무런 영향을 받지 않았다!", target.pbThis)) if show_message
      return true
    end
    return false
  end
...
class Battle::Move::HealUserAndAlliesQuarterOfTotalHPCureStatus < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    if target.status != :NONE
      ...
      case old_status
      when :SLEEP
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 잠에서 깨어났다.", target.pbThis))
      when :POISON
...
class Battle::Move::StartHealUserEachTurn < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 물의 장막으로 몸을 감쌌다!", user.pbThis))
  end
end
...
class Battle::Move::StartHealUserEachTurnTrapUserInBattle < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 뿌리를 내렸다!", user.pbThis))
  end
end
...
class Battle::Move::StartDamageTargetEachTurnIfTargetAsleep < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 악몽을 꾸기 시작했다!", target.pbThis))
  end
end
...
class Battle::Move::StartLeechSeedTarget < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    if target.effects[PBEffects::LeechSeed] >= 0
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 공격을 피했다!", target.pbThis)) if show_message
      return true
    end
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 씨앗이 심겼다!", target.pbThis))
  end
end
...
class Battle::Move::UserLosesHalfOfTotalHP < Battle::Move
  def pbEffectAfterAllHits(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 반동으로 데미지를 입었다!", user.pbThis))
    user.pbItemHPHealCheck
  end
end
    ...
    if !@battle.moldBreaker
      ...
      if bearer
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 사용할 수 없다!!", user.pbThis, @name))
        else
...
class Battle::Move::AttackerFaintsIfUserFaints < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 공격한 상대를 길동무로 끌고 가려 한다!", user.pbThis))
  end
end
...
class Battle::Move::SetAttackerMovePPTo0IfUserFaints < Battle::Move
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 상대에게 원한을 품었다!", user.pbThis))
  end
end
