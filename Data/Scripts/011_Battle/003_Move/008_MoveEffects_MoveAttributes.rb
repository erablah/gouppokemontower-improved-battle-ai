    ...
    if target.hasActiveAbility?(:STURDY) && !@battle.moldBreaker
      if show_message
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          @battle.pbDisplay(_INTL("하지만 {1}의 \\j[{2},으로,로] 인해 효과가 없었다!",
...
class Battle::Move::EnsureNextCriticalHit < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 정신을 집중했다!", user.pbThis))
  end
end
...
class Battle::Move::StartPreventCriticalHitsAgainstUserSide < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("행운의 주문이 \\j[{1},을,를] 급소 공격으로부터 지켰다!", user.pbTeam(true)))
  end
end
...
class Battle::Move::UserEnduresFaintingThisTurn < Battle::Move::ProtectMove
  ...
  def pbProtectMessage(user)
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 버티고 있다!", user.pbThis))
  end
end
...
class Battle::Move::StartWeakenPhysicalDamageAgainstUserSide < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},이,가] {2}의 방어를 올려줬다!", @name, user.pbTeam(true)))
  end
end
...
class Battle::Move::StartWeakenSpecialDamageAgainstUserSide < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},이,가] {2}의 특수방어를 올려줬다!", @name, user.pbTeam(true)))
  end
end
...
class Battle::Move::StartWeakenDamageAgainstUserSideIfHail < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},이,가] {2}의 물리 및 특수 공격에 대한 저항력을 강화했다!",
...
class Battle::Move::RemoveScreens < Battle::Move
  ...
  def pbEffectGeneral(user)
    if user.pbOpposingSide.effects[PBEffects::LightScreen] > 0
      ...
      @battle.pbDisplay(_INTL("{1}의 빛의장막이 사라졌다!", user.pbOpposingTeam))
    end
    if user.pbOpposingSide.effects[PBEffects::Reflect] > 0
      user.pbOpposingSide.effects[PBEffects::Reflect] = 0
      @battle.pbDisplay(_INTL("{1}의 리플렉터가 사라졌다!", user.pbOpposingTeam))
    end
    if user.pbOpposingSide.effects[PBEffects::AuroraVeil] > 0
      user.pbOpposingSide.effects[PBEffects::AuroraVeil] = 0
      @battle.pbDisplay(_INTL("{1}의 오로라 베일이 사라졌다!", user.pbOpposingTeam))
    end
  end
...
class Battle::Move::ProtectUserSideFromDamagingMovesIfUserFirstTurn < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 공격을 막기 위해 바닥을 들어올릴 준비를 한다!", user.pbThis))
  end
end
...
class Battle::Move::ProtectUserSideFromStatusMoves < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("트릭가드가 \\j[{1},을,를] 지켰다!!", user.pbTeam(true)))
  end
end
...
class Battle::Move::HoopaRemoveProtectionsBypassSubstituteLowerUserDef1 < Battle::Move::StatDownMove
  ...
  def pbMoveFailed?(user, targets)
    if !user.isSpecies?(:HOOPA)
      @battle.pbDisplay(_INTL("하지만 \\j[{1},은,는] 이 기술을 사용할 수 없다!", user.pbThis(true)))
      return true
    elsif user.form != 1
      @battle.pbDisplay(_INTL("하지만 현재 상태로는 \\j[{1},은,는] 그 기술을 사용할 수 없다!", user.pbThis(true)))
      return true
    end
    return false
  end
...
class Battle::Move::EnsureNextMoveAlwaysHits < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 조준했다!", user.pbThis, target.pbThis(true)))
  end
end
...
class Battle::Move::TypeDependsOnUserMorpekoFormRaiseUserSpeed1 < Battle::Move::RaiseUserSpeed1
  def pbMoveFailed?(user, targets)
    if !user.isSpecies?(:MORPEKO) && user.effects[PBEffects::TransformSpecies] != :MORPEKO
      @battle.pbDisplay(_INTL("하지만 \\j[{1},은,는] 이 기술을 사용할 수 없다!", user.pbThis))
      return true
    end
    return false
  end
