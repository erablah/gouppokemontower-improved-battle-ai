...
class Battle::Move::RedirectAllMovesToUser < Battle::Move
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 주목의 대상이 되었다!", user.pbThis))
  end
end
...
class Battle::Move::RedirectAllMovesToTarget < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 주목의 대상이 되었다!", target.pbThis))
  end
end
...
class Battle::Move::CurseTargetOrLowerUserSpd1RaiseUserAtkDef1 < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    return if !user.pbHasType?(:GHOST)
    # Ghost effect
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 자신의 체력을 깎고 {2}에게 저주를 걸었다!", user.pbThis, target.pbThis(true)))
...
class Battle::Move::TargetNextFireMoveDamagesTarget < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 분진 가루에 덮였다!", user.pbThis))
  end
end
...
class Battle::Move::PowerUpAllyMove < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 도우려한다!", user.pbThis, target.pbThis(true)))
  end
end
...
class Battle::Move::UserAddStockpileRaiseDefSpDef1 < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    if user.effects[PBEffects::Stockpile] >= 3
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 더 이상 비축할 수 없다!", user.pbThis))
      return true
    end
    return false
  end
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}회 비축했다!",
...
class Battle::Move::PowerDependsOnUserStockpile < Battle::Move
  ...
  def pbEffectAfterAllHits(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},이,가] 비축한 효과가 사라졌다!", user.pbThis))
...
class Battle::Move::UseMoveDependingOnEnvironment < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 변했다!", @name, GameData::Move.get(@npMove).name))
...
class Battle::Move::BounceBackProblemCausingStatusMoves < Battle::Move
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 매직코트를 몸에 감쌌다!", user.pbThis))
  end
end
...
class Battle::Move::StealAndUseBeneficialStatusMove < Battle::Move
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 상대의 행동을 기다리고 있다!", user.pbThis))
  end
end
...
class Battle::Move::ReplaceMoveThisBattleWithTargetLastMoveUsed < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    user.eachMoveWithIndex do |m, i|
      ...
      @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 배웠다!", user.pbThis, newMove.name))
