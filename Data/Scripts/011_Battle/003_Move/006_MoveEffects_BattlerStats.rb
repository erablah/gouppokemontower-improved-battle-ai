...
class Battle::Move::MaxUserAttackLoseHalfOfTotalHP < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    if user.hasActiveAbility?(:CONTRARY)
      ...
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 체력을 깎아 \\j[{2},을,를] 떨어뜨렸다!",
         user.pbThis, GameData::Stat.get(@statUp[0]).name)))
    else
      user.stages[@statUp[0]] = Battle::Battler::STAT_STAGE_MAXIMUM
      user.statsRaisedThisRound = true
      @battle.pbCommonAnimation("StatUp", user)
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 체력을 깎아 \\j[{2},을,를] 최고로 상승시켰다!",
         user.pbThis, GameData::Stat.get(@statUp[0]).name)))
    end
...
class Battle::Move::RaiseUserSpDef1PowerUpElectricMove < Battle::Move::StatUpMove
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 충전하고 있다!", user.pbThis)))
    super
  end
end
...
class Battle::Move::RaiseUserCriticalHitRate2 < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 기운이 넘쳐난다!", user.pbThis)))
  end
end
...
class Battle::Move::RaiseUserMainStats1TrapUserInBattle < Battle::Move::RaiseUserMainStats1
  ...
  def pbEffectGeneral(user)
    ...
    if !user.trappedInBattle?
      ...
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 인해 도망칠 수 없다!", user.pbThis, @name)))
    end
  end
end
    ...
    if target.hasActiveAbility?(:OBLIVIOUS) && !@battle.moldBreaker
      if show_message
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 헤롱헤롱하지 않았다!", target.pbThis, target.abilityName)))
        end
...
class Battle::Move::LowerTargetSpeed1MakeTargetWeakerToFire < Battle::Move::TargetStatDownMove
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    if !target.effects[PBEffects::TarShot]
      ...
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 불꽃 기술에 약해졌다!", target.pbThis)))
    end
  end
end
...
class Battle::Move::LowerTargetEvasion1RemoveSideEffects < Battle::Move::TargetStatDownMove
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    if target.pbOwnSide.effects[PBEffects::AuroraVeil] > 0
      ...
      @battle.pbDisplay(JosaProcessor.process(_INTL("{1}의 오로라 베일이 사라졌다!", target.pbTeam)))
    end
    ...
    if target.pbOwnSide.effects[PBEffects::StealthRock] ||
      ...
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 스텔스록을 날려버렸다!", user.pbThis)))
    end
    if target.pbOwnSide.effects[PBEffects::Spikes] > 0 ||
      ...
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 압정을 날려버렸다!", user.pbThis)))
    end
    if target.pbOwnSide.effects[PBEffects::ToxicSpikes] > 0 ||
      ...
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 독압정을 날려버렸다!", user.pbThis)))
    end
    if target.pbOwnSide.effects[PBEffects::StickyWeb] ||
      ...
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 끈적끈적네트를 날려버렸다!", user.pbThis)))
    end
...
class Battle::Move::UserTargetSwapAtkSpAtkStages < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 상대의 모든 공격과 특수공격 수치 변화를 뒤집었다!", user.pbThis)))
  end
end
...
class Battle::Move::UserTargetSwapDefSpDefStages < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 상대의 모든 방어와 특수방어 수치 변화를 뒤집었다!", user.pbThis)))
  end
end
...
class Battle::Move::UserTargetSwapStatStages < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 상대의 모든 능력치 수치 변화를 뒤집었다!", user.pbThis)))
  end
end
...
class Battle::Move::UserCopyTargetStatStages < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] {2}의 능력치 변화를 복사했다!", user.pbThis, target.pbThis(true))))
  end
end
...
class Battle::Move::UserStealTargetPositiveStatStages < Battle::Move
  ...
  def pbCalcDamage(user, target, numTargets = 1)
    if target.hasRaisedStatStages?
      ...
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 상대의 올라간 능력치를 훔쳤다!", user.pbThis)))
      showAnim = true
      GameData::Stat.each_battle do |s|
...
class Battle::Move::StartUserSideImmunityToStatStageLowering < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 안개에 휩싸였다!", user.pbTeam)))
  end
end
...
class Battle::Move::UserSwapBaseAtkDef < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 공격과 방어를 맞바꿨다!", user.pbThis)))
  end
end
...
class Battle::Move::UserTargetSwapBaseSpeed < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 상대와 스피드를 맞바꿨다!", user.pbThis)))
  end
end
...
class Battle::Move::UserTargetAverageBaseAtkSpAtk < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 자신의 공격력을 상대와 나눴다!", user.pbThis)))
  end
end
...
class Battle::Move::UserTargetAverageBaseDefSpDef < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 자신의 방어력을 상대와 나눴다!", user.pbThis)))
  end
end
