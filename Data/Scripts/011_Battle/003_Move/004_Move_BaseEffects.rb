...
class Battle::Move::TwoTurnMove < Battle::Move
  ...
  def pbQuickChargingMove(user, targets)
    ...
    if @powerHerb
      ...
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 파워허브로 힘이 가득하다!", user.pbThis))
      user.pbConsumeItem
    end
  end
...
class Battle::Move::ProtectMove < Battle::Move
  ...
  def pbProtectMessage(user)
    if @sidedEffect
      @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 막았다!", @name, user.pbTeam(true)))
    else
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 스스로를 지켰다!", user.pbThis))
    end
  end
end
...
class Battle::Move::PledgeMove < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 기다리고 있다...",
  ...
  def pbEffectAfterAllHits(user, target)
    ...
    when :SeaOfFire   # Grass + Fire
      if user.pbOpposingSide.effects[PBEffects::SeaOfFire] == 0
        ...
        msg = _INTL("\\j[{1},은,는] 파도치는 화염이 감싸졌다!", user.pbOpposingTeam(true))
    ...
    when :Rainbow   # Fire + Water
      if user.pbOwnSide.effects[PBEffects::Rainbow] == 0
        ...
        msg = _INTL("\\j[{1},은,는] 무지개에 감싸졌다!", user.pbTeam(true))
    ...
    when :Swamp   # Water + Grass
      if user.pbOpposingSide.effects[PBEffects::Swamp] == 0
        ...
        msg = _INTL("\\j[{1},은,는] 늪지대에 감싸졌다!!", user.pbOpposingTeam(true))
