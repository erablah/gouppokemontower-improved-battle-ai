...
class Battle::Move::FleeFromBattle < Battle::Move
  ...
  def pbEffectGeneral(user)
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 도망쳤다!", user.pbThis))
...
class Battle::Move::SwitchOutUserStatusMove < Battle::Move
  ...
  def pbEndOfMoveUsageEffect(user, targets, numHits, switchedBattlers)
    return if user.wild?
    @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}의 곁으로 돌아갔다!", user.pbThis,
...
class Battle::Move::LowerTargetAtkSpAtk1SwitchOutUser < Battle::Move::TargetMultiStatDownMove
  ...
  def pbEndOfMoveUsageEffect(user, targets, numHits, switchedBattlers)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}의 곁으로 돌아갔다!", switcher.pbThis,
...
class Battle::Move::SwitchOutTargetStatusMove < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    if target.hasActiveAbility?(:SUCTIONCUPS) && !@battle.moldBreaker
      if show_message
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          @battle.pbDisplay(_INTL("\\j[{1},은,는] 스스로를 고정시켰다!", target.pbThis))
        else
          @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 자신을 고정시켰다!", target.pbThis, target.abilityName))
        end
    ...
    if target.effects[PBEffects::Ingrain]
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 뿌리로 자신을 고정시켰다!", target.pbThis)) if show_message
      return true
    end
  ...
  def pbSwitchOutTargetEffect(user, targets, numHits, switched_battlers)
    ...
    targets.each do |b|
      ...
      @battle.pbDisplay(_INTL("\\j[{1},이,가] 끌려 나왔다!", b.pbThis))
...
class Battle::Move::BindTarget < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    # Message
    msg = JosaProcessor.process(_INTL("\\j[{1},은,는] 소용돌이에 갇혔다!", target.pbThis))
    case @id
    when :BIND
      msg = JosaProcessor.process(_INTL("\\j[{1},은,는] {2}에게 조여지고 있다!", target.pbThis, user.pbThis(true)))
    when :CLAMP
      msg = JosaProcessor.process(_INTL("\\j[{1},은,는] \\j[{2},을,를] 꽉 잡고 있다!", user.pbThis, target.pbThis(true)))
    when :FIRESPIN
      msg = JosaProcessor.process(_INTL("\\j[{1},은,는] 불꽃의 소용돌이에 갇혔다!", target.pbThis))
    when :INFESTATION
      msg = JosaProcessor.process(_INTL("\\j[{1},은,는] {2}의 벌레가 들끓는 공격에 당했다!", target.pbThis, user.pbThis(true)))
    when :MAGMASTORM
      msg = JosaProcessor.process(_INTL("\\j[{1},은,는] 마그마스톰에 갇혔다!", target.pbThis))
    when :SANDTOMB
      msg = JosaProcessor.process(_INTL("\\j[{1},은,는] 모래지옥에 갇혔다!", target.pbThis))
    when :WHIRLPOOL
      msg = JosaProcessor.process(_INTL("\\j[{1},은,는] 바다회오리에 빠져있다!", target.pbThis))
    when :WRAP
      msg = JosaProcessor.process(_INTL("\\j[{1},은,는] {2}에게 휘감겨 있다!", target.pbThis, user.pbThis(true)))
    end
...
class Battle::Move::TrapTargetInBattle < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 더 이상 도망칠 수 없다!", target.pbThis))
  end
...
class Battle::Move::TrapTargetInBattleLowerTargetDefSpDef1EachTurn < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] {2} 때문에 도망칠 수 없다!", target.pbThis, @name))
  end
end
...
class Battle::Move::UsedAfterUserTakesPhysicalDamage < Battle::Move
  def pbDisplayChargeMessage(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 트랩쉘을 설치했다!", user.pbThis))
  end
...
class Battle::Move::TargetActsNext < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 친절한 제안을 받아들였다!", target.pbThis))
  end
end
...
class Battle::Move::StartSlowerBattlersActFirst < Battle::Move
  def pbEffectGeneral(user)
    if @battle.field.effects[PBEffects::TrickRoom] > 0
      ...
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 공간을 되돌렸다!", user.pbThis))
    else
      @battle.field.effects[PBEffects::TrickRoom] = 5
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 공간을 뒤틀었다!", user.pbThis))
    end
  end
...
class Battle::Move::DisableTargetLastMoveUsed < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}의 \\j[{2},은,는] 봉인되었다!", target.pbThis,
...
class Battle::Move::DisableTargetUsingSameMoveConsecutively < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 트집에 시달리고 있다!", target.pbThis))
    target.pbItemStatusCureCheck
  end
end
...
#        "UseLastMoveUsedByTarget",                    # Mirror Move   # See above
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 앙코르 상태에 들어갔다!", target.pbThis))
    target.pbItemStatusCureCheck
  end
end
...
class Battle::Move::DisableTargetStatusMoves < Battle::Move
    ...
    if Settings::MECHANICS_GENERATION >= 6 && target.hasActiveAbility?(:OBLIVIOUS) &&
      ...
      if show_message
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          @battle.pbDisplay(_INTL("하지만 {1}의 \\j[{2},으로,로] 인해 실패했다!",
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 도발에 넘어갔다!", target.pbThis))
    target.pbItemStatusCureCheck
  end
end
...
class Battle::Move::DisableTargetHealingMoves < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 회복할 수 없게 되었다!", target.pbThis))
    target.pbItemStatusCureCheck
  end
end
...
class Battle::Move::DisableTargetSoundMoves < Battle::Move
  def pbAdditionalEffect(user, target)
    ...
    if target.effects[PBEffects::ThroatChop] == 0
      @battle.pbDisplay(_INTL("{1}의 효과로 \\j[{2},은,는] 소리 기술을 사용할 수 없다!",
...
class Battle::Move::DisableTargetMovesKnownByUser < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 상대와 공유하는 기술을 봉인했다!", user.pbThis))
  end
end
