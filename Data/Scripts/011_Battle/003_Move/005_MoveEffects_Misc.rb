...
class Battle::Move::DoesNothingCongratulations < Battle::Move
  def pbEffectGeneral(user)
    if user.wild?
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},이,가] 축하한다!!", user.pbThis(true))))
    else
...
class Battle::Move::FailsIfUserNotConsumedBerry < Battle::Move
  def pbCanChooseMove?(user, commandPhase, showMessages)
    if !user.belched?
      if showMessages
        msg = _INTL("\\j[{1},은,는] 열매를 먹지 않아 트름 할 수 없다!", user.pbThis)
...
class Battle::Move::FailsIfTargetHasNoItem < Battle::Move
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] {2}의 공격을 받으려 한다!", target.pbThis, target.itemName)))
    return false
  end
end
...
class Battle::Move::FailsIfUserDamagedThisTurn < Battle::Move
  def pbDisplayChargeMessage(user)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 집중하고 있다!", user.pbThis)))
  end
...
class Battle::Move::CrashDamageIfFailsUnusableInGravity < Battle::Move
  ...
  def pbCrashDamage(user)
    ...
    @battle.pbDisplay(JosaProcessor.process(JosaProcessor.process(_INTL("\\j[{1},은,는] 바닥에 부딪히고 말았다!", user.pbThis))))
...
class Battle::Move::SwapSideEffects < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 서로의 필드를 바꿨다!", user.pbThis)))
  end
end
...
class Battle::Move::UserMakeSubstitute < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    if user.effects[PBEffects::Substitute] > 0
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 이미 대타가 있다!", user.pbThis)))
      return true
    end
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 대타를 집어넣었다!", user.pbThis)))
  end
end
...
class Battle::Move::RemoveUserBindingAndEntryHazards < Battle::Move::StatUpMove
  ...
  def pbEffectAfterAllHits(user, target)
    ...
    if user.effects[PBEffects::Trapping] > 0
      ...
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] {2}의 \\j[{3},으로,로]부터 풀려났다!", user.pbThis, trapUser.pbThis(true), trapMove)))
      user.effects[PBEffects::Trapping]     = 0
      user.effects[PBEffects::TrappingMove] = nil
...
class Battle::Move::AttackTwoTurnsLater < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    if @id == :DOOMDESIRE
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 파멸의 운명을 골랐다!", user.pbThis)))
    else
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 미래의 공격을 예지했다!", user.pbThis)))
    end
  end
...
class Battle::Move::UserSwapsPositionsWithAlly < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    if @battle.pbSwapBattlers(idxA, idxB)
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},과,와] \\j[{2},은,는] 자리를 바꿨다!",
...
class Battle::Move::BurnAttackerBeforeUserActs < Battle::Move
  def pbDisplayChargeMessage(user)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 부리를 가열하기 시작했다!", user.pbThis)))
  end
end
