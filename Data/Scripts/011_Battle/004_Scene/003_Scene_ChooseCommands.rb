class Battle::Scene
  ...
  def pbCommandMenu(idxBattler, firstAction)
    ...
    cmds = [
      JosaProcessor.process(_INTL("무엇을 할까?", @battle.battlers[idxBattler].name)),
      _INTL("Fight"),
      _INTL("Bag"),
      _INTL("Pokémon"),
  ...
  def pbItemMenu(idxBattler, _firstAction)
    ...
    loop do
      ...
      commands[commands.length]          = _INTL("Cancel")
      command = itemScene.pbShowCommands(JosaProcessor.process(_INTL("\\j[{1},을,를] 선택했습니다.", itemName)), commands)
