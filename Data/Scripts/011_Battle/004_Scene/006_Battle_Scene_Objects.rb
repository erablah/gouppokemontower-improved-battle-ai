...
#    pbSetSmallFont(@hpNumbers.bitmap)
  ...
  def draw_gender
    ...
    pbDrawTextPositions(self.bitmap, [[gender_text, @spriteBaseX + 120, 12, :left, base_color, shadow_color]])
  end
...
class Battle::Scene::AbilitySplashBar < Sprite
  ...
  def initialize(side, viewport = nil)
    ...
    # 배경 이미지에서 y 위치와 높이(상하 구분)
    @bgHeight = @bgBitmap.height / 2
    @bgSprite.src_rect = Rect.new(0, (@side == 0) ? 0 : @bgHeight, @bgBitmap.width, @bgHeight)

    # 텍스트 출력용 비트맵
    @contents = Bitmap.new(@bgBitmap.width, @bgHeight)
    self.bitmap = @contents
    pbSetSystemFont(self.bitmap)
...
def refresh
  ...
  pokemon_name = _INTL("{1}'s", @battler.name)
  ability_name = @battler.abilityName

  combined_text = "#{pokemon_name} #{ability_name}"

  textPos = []
  textX = (@side == 0) ? 10 : self.bitmap.width - 10
  align = (@side == 0) ? :left : :right
  
  self.bitmap.font.size = 24

  textPos.push([combined_text, textX, 30, align, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR, :outline])
