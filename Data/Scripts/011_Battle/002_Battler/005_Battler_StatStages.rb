class Battle::Battler
  ...
  def pbCanRaiseStatStage?(stat, user = nil, move = nil, showFailMsg = false, ignoreContrary = false)
    ...
    if statStageAtMax?(stat)
      if showFailMsg
        @battle.pbDisplay(_INTL("{1}의 \\j[{2},은,는] 더 오르지 않는다!",
  ...
  def pbRaiseStatStage(stat, increment, user, showAnim = true, ignoreContrary = false)
    ...
    arrStatTexts = [
      _INTL("{1}의 \\j[{2},이,가] 올랐다!", pbThis, GameData::Stat.get(stat).name),
      _INTL("{1}의 \\j[{2},이,가] 크게 올랐다!", pbThis, GameData::Stat.get(stat).name),
      _INTL("{1}의 \\j[{2},이,가] 매우 크게 올랐다!", pbThis, GameData::Stat.get(stat).name)
    ]
  ...
  def pbRaiseStatStageByCause(stat, increment, user, cause, showAnim = true, ignoreContrary = false)
    ...
    if user.index == @index
      arrStatTexts = [
        _INTL("{1}의 \\j[{2},으로,로] \\j[{3},이,가] 올랐다!", pbThis, cause, GameData::Stat.get(stat).name),
        _INTL("{1}의 \\j[{2},으로,로] \\j[{3},이,가] 크게 올랐다!", pbThis, cause, GameData::Stat.get(stat).name),
        _INTL("{1}의 \\j[{2},으로,로] \\j[{3},이,가] 매우 크게 올랐다!", pbThis, cause, GameData::Stat.get(stat).name)
      ]
    else
      arrStatTexts = [
        _INTL("{1}의 \\j[{2},으로,로] {3}의 \\j[{4},이,가] 올랐다!", user.pbThis, cause, pbThis(true), GameData::Stat.get(stat).name),
        _INTL("{1}의 \\j[{2},으로,로] {3}의 \\j[{4},이,가] 크게 올랐다!", user.pbThis, cause, pbThis(true), GameData::Stat.get(stat).name),
        _INTL("{1}의 \\j[{2},으로,로] {3}의 \\j[{4},이,가] 매우 크게 올랐다!", user.pbThis, cause, pbThis(true), GameData::Stat.get(stat).name)
      ]
    end
    @battle.pbDisplay(arrStatTexts[[increment - 1, 2].min])
    # Trigger abilities upon stat gain
  ...
  def pbCanLowerStatStage?(stat, user = nil, move = nil, showFailMsg = false,
    ...
    if statStageAtMin?(stat)
      if showFailMsg
        @battle.pbDisplay(_INTL("{1}의 \\j[{2},은,는] 더 이상 떨어지지 않는다!",
                                pbThis, GameData::Stat.get(stat).name))
      end
      return false
    end
  ...
  def pbLowerStatStage(stat, increment, user, showAnim = true, ignoreContrary = false,
    ...
    arrStatTexts = [
      _INTL("{1}의 \\j[{2},이,가] 떨어졌다!", pbThis, GameData::Stat.get(stat).name),
      _INTL("{1}의 \\j[{2},이,가] 크게 떨어졌다!", pbThis, GameData::Stat.get(stat).name),
      _INTL("{1}의 \\j[{2},이,가] 매우 크게 떨어졌다!", pbThis, GameData::Stat.get(stat).name)
    ]
    @battle.pbDisplay(arrStatTexts[[increment - 1, 2].min])
  ...
  def pbLowerStatStageByCause(stat, increment, user, cause, showAnim = true,
    ...
    if user.index == @index
      arrStatTexts = [
        _INTL("{1}의 \\j[{2},으로,로] 자신의 \\j[{3},이,가] 떨어졌다!", pbThis, cause, GameData::Stat.get(stat).name),
        _INTL("{1}의 \\j[{2},으로,로] 자신의 \\j[{3},이,가] 크게 떨어졌다!", pbThis, cause, GameData::Stat.get(stat).name),
        _INTL("{1}의 \\j[{2},으로,로] 자신의 \\j[{3},이,가] 매우 크게 떨어졌다!", pbThis, cause, GameData::Stat.get(stat).name)
      ]
    else
      arrStatTexts = [
        _INTL("{1}의 \\j]{2},으로,로] {3}의 \\j[{4},이,가] 떨어졌다!", user.pbThis, cause, pbThis(true), GameData::Stat.get(stat).name),
        _INTL("{1}의 \\j]{2},으로,로] {3}의 \\j[{4},이,가] 크게 떨어졌다!!", user.pbThis, cause, pbThis(true), GameData::Stat.get(stat).name),
        _INTL("{1}의 \\j]{2},으로,로] {3}의 \\j[{4},이,가] 매우 크게 떨어졌다!", user.pbThis, cause, pbThis(true), GameData::Stat.get(stat).name)
      ]
    end
    @battle.pbDisplay(arrStatTexts[[increment - 1, 2].min])
    # Trigger abilities upon stat loss
  ...
  def pbLowerAttackStatStageIntimidate(user)
    ...
    if @effects[PBEffects::Substitute] > 0
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        @battle.pbDisplay(_INTL("{1}의 대타가 {2}의 \\j[{3},을,를] 대신 맞았다!",
    ...
    if Settings::MECHANICS_GENERATION >= 8 && hasActiveAbility?([:OBLIVIOUS, :OWNTEMPO, :INNERFOCUS, :SCRAPPY])
      @battle.pbShowAbilitySplash(self)
      if Battle::Scene::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("{1}의 \\j[{2},은,는] 더 이상 떨어지지 않는다!", pbThis, GameData::Stat.get(:ATTACK).name))
      else
        @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] \\j[{3},이,가] 떨어지는 걸 막았다!", pbThis, abilityName,
    ...
    if !hasActiveAbility?(:CONTRARY)
      if pbOwnSide.effects[PBEffects::Mist] > 0
        @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}의 \\j[{3},을,를] 흰안개로 막았다!",
                                pbThis, user.pbThis(true), user.abilityName))
        return false
      end
      if abilityActive? &&
        ...
        @battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] {3}의 \\j[{4},을,를] 막았다!",
      ...
      allAllies.each do |b|
        ...
        if Battle::AbilityEffects.triggerStatLossImmunityFromAlly(b.ability, b, self, :ATTACK, @battle, false)
          @battle.pbDisplay(_INTL("\\j[{1},은,는] {4}의 \\j[{5},으로,로] 인해 {2}의 \\j[{3},을,를] 막았다!",
