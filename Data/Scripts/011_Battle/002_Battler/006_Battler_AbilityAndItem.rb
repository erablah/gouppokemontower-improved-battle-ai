class Battle::Battler
  ...
  def pbContinualAbilityChecks(onSwitchIn = false)
    ...
    if hasActiveAbility?(:TRACE)
      ...
      if choices.length > 0
        ...
        @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}의 \\j[{3},을,를] 트레이스했다!", pbThis, choice.pbThis(true), choice.abilityName))
  ...
  def pbMoveImmunityStatRaisingAbility(user, move, moveType, immuneType, stat, increment, show_message)
    ...
    if show_message
      ...
      if pbCanRaiseStatStage?(stat, self)
        ...
        @battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] {3}의 효과를 낮췄다!", pbThis, abilityName, move.name))
      end
      @battle.pbHideAbilitySplash(self)
    end
    return true
    ...
    if show_message
      ...
      if canHeal? && pbRecoverHP(@totalhp / 4) > 0
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          @battle.pbDisplay(_INTL("\\j[{1},이,가] {2}의 체력을 회복시켰다.", pbThis, abilityName))
        end
      elsif Battle::Scene::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("It doesn't affect {1}...", pbThis(true)))
      else
  ...
  def pbSymbiosis
    ...
    @battle.pbPriority(true).each do |b|
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("\\j[{1},은,는] {3}에게 \\j[{2},을,를] 공유했다!",
                                b.pbThis, b.itemName, pbThis(true)))
      else
        @battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] {4}에게 \\j[{3},을,를] 공유했다!",
  ...
  def pbConfusionBerry(item_to_use, forced, confuse_stat, confuse_msg)
    ...
    if amt > 0
      if forced
        ...
        @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 체력을 회복했다!", pbThis, used_item_name))
      end
    end
