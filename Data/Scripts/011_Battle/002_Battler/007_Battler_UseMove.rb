class Battle::Battler
  ...
  def pbProcessTurn(choice, tryFlee = true)
    ...
    if tryFlee && wild? &&
      ...
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 도망쳤다!", pbThis))
      @battle.decision = 3
      pbEndTurn(choice)
      return true
    end
    ...
    if choice[0] == :Shift
      ...
      if idxOther >= 0
        ...
        case @battle.pbSideSize(@index)
        when 2
          @battle.pbDisplay(_INTL("\\j[{1},은,는] 자리를 옮겼다!", pbThis))
        when 3
          @battle.pbDisplay(_INTL("\\j[{1},은,는] 센터로 이동했다.", pbThis))
        end
      end
      pbBeginTurn(choice)
      pbCancelMoves
  ...
  def pbCancelMoves(full_cancel = false)
    ...
    if @effects[PBEffects::Outrage] == 1 && pbCanConfuseSelf?(false) && !full_cancel
      pbConfuse(_INTL("\\j[{1},은,는] 지쳐 혼란에 빠졌다!", pbThis))
    end
  ...
  def pbUseMove(choice, specialUsage = false)
    ...
    if !specialUsage && !pbReducePP(move)
      @battle.pbDisplay(_INTL("{1}의 {2}!", pbThis, move.name))
    ...
    if isSpecies?(:AEGISLASH) && self.ability == :STANCECHANGE
      if move.damagingMove?
        pbChangeForm(1, _INTL("\\j[{1},은,는] 블레이드폼이 되었다!", pbThis))
      elsif move.id == :KINGSSHIELD
        pbChangeForm(0, _INTL("\\j[{1},은,는] 실드폼이 되었다!", pbThis))
      end
    end
    ...
    @battle.pbPriority(true).each do |b|
      ...
      if Battle::AbilityEffects.triggerMoveBlocking(b.ability, b, user, targets, move, @battle)
        @battle.pbDisplayBrief(_INTL("{1}의 {2}!", user.pbThis, move.name))
        @battle.pbShowAbilitySplash(b)
        @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},을,를] 사용할 수 없다!", user.pbThis, move.name))
    ...
    if move.snatched
      ...
      @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}의 기술을 가로챘다!", user.pbThis, pbThis(true)))
    end
    ...
    if user.status == :FROZEN && move.thawsUser?
      ...
      @battle.pbDisplay(_INTL("\\j[{1},이,가] 얼음을 녹였다!!", user.pbThis))
    end
    # Powder
    if user.effects[PBEffects::Powder] && move.calcType == :FIRE
      ...
      if ![:Rain, :HeavyRain].include?(user.effectiveWeather) && user.takesIndirectDamage?
        user.pbTakeEffectDamage((user.totalhp / 4.0).round, false) do |hp_lost|
          @battle.pbDisplay(_INTL("\\j[{1},은,는] 분진에 의해 데미지를 입었다!", user.pbThis))
        end
    ...
    if targets.length == 0 && move.pbTarget(user).num_targets > 0 && !move.worksWithNoTargets?
      ...
      targets.each do |b|
        ...
        @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}에게 뛰어내렸다!", b.pbThis, move.name))
      ...
      if magicCoater >= 0 || magicBouncer >= 0
        ...
        if !mc.fainted?
          ...
          @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}에게 뛰어내렸다!", mc.pbThis, move.name))
    ...
    @battle.allBattlers.each do |b|
      ...
      @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}에게 지휘했다!", b.pbThis, user.pbThis(true)))
    ...
    if !@effects[PBEffects::Dancer] && !user.lastMoveFailed && realNumHits > 0 &&
      ...
      while dancers.length > 0
        ...
        if !Battle::Scene::USE_ABILITY_SPLASH
          @battle.pbDisplay(_INTL("\\j[{1},은,는] {2}에게 춤을 추고 있다!",
  ...
  def pbProcessMoveHit(move, user, targets, hitNum, skipAccuracyCheck)
    ...
    if user.effects[PBEffects::GemConsumed] && hitNum == 0
      ...
      @battle.pbDisplay(_INTL("\\j[{1},이,가] \\j[{2},을,를] 강화하였다!",
    ...
    targets.each do |b|
      ...
      @battle.pbDisplay(_INTL("\\j[{1},이,가] {2}의 데미지를 약하게 했다!", b.itemName, b.pbThis(true)))
      b.pbConsumeItem
    end
