class Battle::Battler
  ...
  def pbCanInflictStatus?(newStatus, user, showMessages, move = nil, ignoreStatus = false)
    ...
    if self.status == newStatus && !ignoreStatus
      if showMessages
        msg = ""
        case self.status
        when :SLEEP     then msg = _INTL("\\j[{1},은,는] 이미 자고있다!", pbThis)
        when :POISON    then msg = _INTL("\\j[{1},은,는] 이미 독 상태다!", pbThis)
        when :BURN      then msg = _INTL("\\j[{1},은,는] 이미 화상 상태다!", pbThis)
        when :PARALYSIS then msg = _INTL("\\j[{1},은,는] 이미 마비에 걸려있다!", pbThis)
        when :FROZEN    then msg = _INTL("\\j[{1},은,는] 이미 얼어있다!", pbThis)
        end
    ...
    if affectedByTerrain?
      ...
      when :Electric
        if newStatus == :SLEEP
          if showMessages
            @battle.pbDisplay(_INTL("\\j[{1},이,가] 일렉트릭필드를 펼쳤다!", pbThis(true)))
          end
          return false
        end
      when :Misty
        @battle.pbDisplay(_INTL("\\j[{1},이,가] 미스트필드를 펼쳤다!", pbThis(true))) if showMessages
        return false
      end
    end
    # Uproar immunity
    if newStatus == :SLEEP && !(hasActiveAbility?(:SOUNDPROOF) && !@battle.moldBreaker)
      @battle.allBattlers.each do |b|
        ...
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 소란을 피우고 있어 잠들지 않는다!", pbThis(true))) if showMessages
        return false
      end
    end
    # Type immunities
    ...
    if immuneByAbility
      if showMessages
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          case newStatus
          when :SLEEP     then msg = _INTL("\\j[{1),은,는] 잠들지 않는다!", pbThis)
          when :POISON    then msg = _INTL("\\j[{1},은,는] 독에 걸리지 않는다!", pbThis)
          when :BURN      then msg = _INTL("\\j[{1},은,는] 화상에 걸리지 않는다!", pbThis)
          when :PARALYSIS then msg = _INTL("\\j[{1},은,는] 마비에 걸리지 않는다!", pbThis)
          when :FROZEN    then msg = _INTL("\\j[{1},은,는] 얼어붙지 않는다!", pbThis)
          end
        elsif immAlly
          case newStatus
          when :SLEEP
            msg = _INTL("\\j[{1},은,는] {2}의 \\j[{3},으로,로] 잠들지 않았다!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)
          when :POISON
            msg = _INTL("\\j[{1},은,는] {2}의 \\j[{3},으로,로] 독에 걸리지 않았다!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)
          when :BURN
            msg = _INTL("\\j[{1},은,는] {2}의 \\j[{3},으로,로] 화상에 걸리지 않았다!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)
          when :PARALYSIS
            msg = _INTL("\\j[{1},은,는] {2}의 \\j[{3},으로,로] 마비에 걸리지 않았다!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)
          when :FROZEN
            msg = _INTL("\\j[{1},은,는] {2}의 \\j[{3},으로,로] 얼어붙지 않았다!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)
          end
        else
          case newStatus
          when :SLEEP     then msg = _INTL("\\j[{1},은,는] \\j[{2},으로,로] 잠에 들지 않는다!", pbThis, abilityName)
          when :POISON    then msg = _INTL("{1}의 \\j[{2},이,가] 독에 걸리는 것을 막았다!", pbThis, abilityName)
          when :BURN      then msg = _INTL("{1}의 \\j[{2},이,가] 화상에 걸리는 것을 막았다!", pbThis, abilityName)
          when :PARALYSIS then msg = _INTL("{1}의 \\j[{2},이,가] 마비에 걸리는 것을 막았다!", pbThis, abilityName)
          when :FROZEN    then msg = _INTL("{1}의 \\j[{2},이,가] 얼어붙는 것을 막았다!", pbThis, abilityName)
          end
        end
        @battle.pbDisplay(msg)
    ...
    if pbOwnSide.effects[PBEffects::Safeguard] > 0 && !self_inflicted && move &&
      ...
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 세이프가드로 보호받고 있다!", pbThis)) if showMessages
      return false
    end
    return true
  end
  ...
  def pbInflictStatus(newStatus, newStatusCount = 0, msg = nil, user = nil)
    ...
    if msg && !msg.empty?
      ...
      case newStatus
      when :SLEEP
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 잠에 들었다!", pbThis))
      when :POISON
        if newStatusCount > 0
          @battle.pbDisplay(_INTL("\\j[{1},은,는] 맹독에 걸렸다!", pbThis))
        else
          @battle.pbDisplay(_INTL("\\j[{1},은,는] 독에 걸렸다!", pbThis))
        end
      when :BURN
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 화상을 입었다!", pbThis))
      when :PARALYSIS
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 마비에 걸렸다!", pbThis))
      when :FROZEN
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 얼어붙고 말았다!", pbThis))
      end
    end
  ...
  def pbContinueStatus
    ...
    case self.status
    when :SLEEP
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 깊이 자고 있다.", pbThis)))
    when :POISON
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 독의 데미지를 받았다!", pbThis)))
    when :BURN
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 화상의 데미지를 받았다!", pbThis)))
    when :PARALYSIS
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 몸이 저려 움직일 수 없다!", pbThis)))
    when :FROZEN
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 꽁꽁 얼어붙었다!", pbThis)))
    end
  ...
  def pbCureStatus(showMessages = true)
    ...
    if showMessages
      case oldStatus
      when :SLEEP     then @battle.pbDisplay(_INTL("\\j[{1},은,는] 일어났다!", pbThis))
      ...
      when :FROZEN    then @battle.pbDisplay(_INTL("{1}의 얼음이 녹았다!", pbThis))
      end
    end
  ...
  def pbCanConfuse?(user = nil, showMessages = true, move = nil, selfInflicted = false)
    ...
    if @effects[PBEffects::Confusion] > 0
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 이미 혼란에 빠져있다.", pbThis)) if showMessages
      return false
    end
    if @effects[PBEffects::Substitute] > 0 && !(move && move.ignoresSubstitute?(user)) &&
       !selfInflicted
    ...
    if (selfInflicted || !@battle.moldBreaker) && hasActiveAbility?(:OWNTEMPO)
      if showMessages
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          @battle.pbDisplay(_INTL("\\j[{1},은,는] 혼란에 빠지지 않는다!", pbThis))
        else
          @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 혼란에 걸리지 않았다!", pbThis, abilityName))
        end
  ...
  def pbConfuse(msg = nil)
    ...
    msg = _INTL("\\j[{1},은,는] 혼란에 빠졌다!", pbThis) if nil_or_empty?(msg)
    @battle.pbDisplay(msg)
      ...
      if hasActiveAbility?([:AROMAVEIL, :OBLIVIOUS])
        if showMessages
          ...
          if Battle::Scene::USE_ABILITY_SPLASH
            ...
            @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 헤롱헤롱하지 않았다!", pbThis, abilityName))
          end
          @battle.pbHideAbilitySplash(self)
        end
        return false
        ...
        allAllies.each do |b|
          ...
          if showMessages
            ...
            if Battle::Scene::USE_ABILITY_SPLASH
              ...
              @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 헤롱헤롱하지 않았다!", b.pbThis, b.abilityName))
            end
  ...
  def pbAttract(user, msg = nil)
    ...
    msg = _INTL("\\j[{1},은,는] 사랑에 빠졌다!", pbThis) if nil_or_empty?(msg)
    @battle.pbDisplay(msg)
    # Destiny Knot
    if hasActiveItem?(:DESTINYKNOT) && user.pbCanAttract?(self, false)
      user.pbAttract(self, _INTL("\\j[{1},은,는] {2}에게 헤롱헤롱하다!", user.pbThis(true), itemName))
    end
    # Attraction cures
    pbItemStatusCureCheck
    pbAbilityStatusCureCheck
