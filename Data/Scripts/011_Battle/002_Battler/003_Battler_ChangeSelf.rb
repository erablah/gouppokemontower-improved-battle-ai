class Battle::Battler
  ...
  def pbRecoverHPFromDrain(amt, target, msg = nil)
    if target.hasActiveAbility?(:LIQUIDOOZE, true)
      ...
      pbReduceHP(amt)
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 해감액을 흡수했다!", pbThis))
      @battle.pbHideAbilitySplash(target)
      pbItemHPHealCheck
    else
      msg = _INTL("{1}의 에너지를 흡수했다!", target.pbThis) if nil_or_empty?(msg)
  ...
  def pbFaint(showMessage = true)
    ...
    @battle.pbDisplayBrief(_INTL("\\j[{1},은,는] 쓰러졌다!", pbThis)) if showMessage
  ...
  def pbCheckFormOnMovesetChange
    ...
    if isSpecies?(:KELDEO)
      ...
      pbChangeForm(newForm, _INTL("{1}의 모습이 변했다!", pbThis))
    end
  end
  ...
  def pbCheckForm(endOfRound = false)
    ...
    if isSpecies?(:DARMANITAN) && self.ability == :ZENMODE
      if @hp <= @totalhp / 2
        if @form.even?
          @battle.pbShowAbilitySplash(self, true)
          @battle.pbHideAbilitySplash(self)
          pbChangeForm(@form + 1, _INTL("\\j[{1},이,가] 발동했다!", abilityName))
        end
      elsif @form.odd?
        @battle.pbShowAbilitySplash(self, true)
        @battle.pbHideAbilitySplash(self)
    ...
    if isSpecies?(:MINIOR) && self.ability == :SHIELDSDOWN
      if @hp > @totalhp / 2   # Turn into Meteor form
        ...
        if @form != newForm
          @battle.pbShowAbilitySplash(self, true)
          @battle.pbHideAbilitySplash(self)
          pbChangeForm(newForm, _INTL("\\j[{1},이,가] 비활성화됐다!", abilityName))
        end
      elsif @form < 7   # Turn into Core form
        @battle.pbShowAbilitySplash(self, true)
        @battle.pbHideAbilitySplash(self)
        pbChangeForm(@form + 7, _INTL("\\j[{1},이,가] 활성화됐다!", abilityName))
      end
    end
    ...
    if isSpecies?(:WISHIWASHI) && self.ability == :SCHOOLING
      if @level >= 20 && @hp > @totalhp / 4
        if @form != 1
          @battle.pbShowAbilitySplash(self, true)
          @battle.pbHideAbilitySplash(self)
          pbChangeForm(1, _INTL("\\j[{1},은,는] 몰려들었다!", pbThis))
        end
      elsif @form != 0
        @battle.pbShowAbilitySplash(self, true)
        @battle.pbHideAbilitySplash(self)
        pbChangeForm(0, _INTL("\\j[{1},이,가] 흩어졌다!", pbThis))
      end
    end
    ...
    if isSpecies?(:ZYGARDE) && self.ability == :POWERCONSTRUCT && endOfRound &&
      ...
      @battle.pbShowAbilitySplash(self, true)
      @battle.pbHideAbilitySplash(self)
      pbChangeForm(newForm, _INTL("\\j[{1},은,는] 퍼펙트폼으로 변했다!", pbThis))
    end
  ...
  def pbTransform(target)
    ...
    @battle.scene.pbRefreshOne(@index)
    @battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 변신했다!", pbThis, target.pbThis(true)))
