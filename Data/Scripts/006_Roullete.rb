#===============================================================================
# * Roulette Minigame - by FL (Modified for GIMMIGHOULCOIN and Korean INTL)
#===============================================================================
...
module Roulette
  ...
  TABLE_POSITIONS=[ [1,10, 7,4], [5, 2,11,8], [9, 6, 3,0] ]
  COLUMNS=4
  ROWS=3
  ROUNDS=6

  #===========================================================================
  # Cursor / RouletteObject / Scene / Screen / start 등 기존 FL 스크립트
  # 구조 그대로 둠
  #===========================================================================
  ...
  class Scene
    ...
    def processResults
      ...
      if wins
        if @cursor.multiplier==12
          displayMessage(_INTL(
            "\\me[{1}]잭팟!\\wtnp[50]", Bridge.getAudioName("Slots big win")
          ))
        else
          displayMessage(_INTL(
            "\\me[{1}]당첨!\\wtnp[30]", Bridge.getAudioName("Slots win")
          ))
        end
        addCredits(@wager*@cursor.multiplier, false)
        displayMessage(_INTL("코인을 {1}개 받았다!",@wager*@cursor.multiplier))
      else  
        pbPlayBuzzerSE()
        displayMessage(_INTL("아무 것도 받지 못 했다!"))
      end
      @playedBalls[@result]=true
      if Roulette.count(@playedBalls,true)==ROUNDS # Clear
        if ROUNDS!=1
          displayMessage(_INTL("룰렛이 초기화 되었다."))
        end
        @roulette.clearBalls
        @playedBalls.clear
      ...
      drawMultiplier
      if confirmMessage(_INTL("계속 도전할까요?"))
        if Bridge.coins<@wager
          displayMessage(_INTL("코인이 부족합니다!"))
          @exit=true 
        end
      else  
        @exit=true 
  ...
  def self.start(wager=1, backgroundAndIconColor=nil)
    if !Bridge.hasCoinCase?
      Bridge.message(_INTL("룰렛 게임입니다!"))
    elsif Bridge.confirmMessage(_INTL(
      "최소 베팅 금액은 코인 {1}개 입니다. 게임을 시작하시겠습니까?", wager
    ))
      if Bridge.coins<wager
        Bridge.message(_INTL("코인이 부족합니다!"))
      elsif Bridge.coins==Bridge.maxCoins
        Bridge.message(_INTL("코인이 가득 찼습니다!"))  
      else
        pbFadeOutIn(99999){     
          scene=Scene.new
  ...
  #=============================================================================
  # * Essentials multiversion layer - Bridge
  #=============================================================================
  module Bridge
    @@audioNameHash = nil
    module_function
    ...
    #===========================================================================
    # * Coins Handling - Use Bag :GIMMIGHOULCOIN instead of Coin Case
    #===========================================================================
    def coinHolder
      return $bag
    end
    ...
    def coins
      return $bag.quantity(:GIMMIGHOULCOIN)
    end

    def coins=(value)
      current = $bag.quantity(:GIMMIGHOULCOIN)
      if value > current
        $bag.add(:GIMMIGHOULCOIN, value - current)
      elsif value < current
        $bag.remove(:GIMMIGHOULCOIN, current - value)
      end
    end
