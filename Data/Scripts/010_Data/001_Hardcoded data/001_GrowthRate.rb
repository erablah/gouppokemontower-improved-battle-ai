...
def minimum_exp_for_level(level)
  raise ArgumentError, "Level #{level} is invalid." if !level || level <= 0
  raise ArgumentError, "Level #{level} exceeds max level #{GrowthRate.max_level}" if level > GrowthRate.max_level
...
#===============================================================================
#===============================================================================
# Pokémon Experience Tables (Level 1~300)
#===============================================================================
...
#-------------------------------------------------------------------------------
# Erratic (최대 레벨 300 확장 및 안정화 - ΔE 15,000 시작, 공차 1,000)
#-------------------------------------------------------------------------------
GameData::GrowthRate.register({
  :id          => :Erratic,
  :name        => _INTL("Erratic"),
  ...
  :exp_formula => proc do |level|
    ...
    if n <= 50
      ...
      exp = n**3 * ((1911 - (10 * n)) / 3).floor / 500
    elsif n <= 100
      exp = n**3 * (160 - n) / 100
    else 
      # n >= 101: Lv 100 초과 시 누적 경험치 안정화 (ΔE 15,000에서 시작, 공차 1,000)
      # E_100 = 600,000
      base_exp = 600_000
      
      n_diff = n - 100
      
      # Sum(ΔE) = n_diff * (2*시작항 + (n_diff - 1)*공차) / 2
      # 시작항: 15000, 공차: 1000
      sum_delta_exp = n_diff * (30000 + (n_diff - 1) * 1000) / 2
      
      exp = base_exp + sum_delta_exp
    end
    next exp.to_i 
  end
})
...
#-------------------------------------------------------------------------------
# Fluctuating (최대 레벨 300 확장 및 안정화)
#-------------------------------------------------------------------------------
GameData::GrowthRate.register({
  :id          => :Fluctuating,
  ...
  :exp_formula => proc do |level|
    ...
    if n <= 15
      ...
      # n >= 101: Lv 100 초과 시 누적 경험치 안정화 (ΔE 약 20,000 ~ 50,000)
      # E_100 = 1,640,000 (기존 최종값)
      base_exp = 1_640_000
      
      # ΔE = 20000 + 150 * (n-101) (등차수열)
      # 누적 합 계산: n_diff = n - 100
      n_diff = n - 100
      
      # Sum(ΔE) = n_diff * (2*시작항 + (n_diff - 1)*공차) / 2
      # 시작항: 20000, 공차: 150
      sum_delta_exp = n_diff * (40000 + (n_diff - 1) * 150) / 2
      
      # NOTE: 음수 EXP 방지를 위해 E_100 기준점을 조정할 필요는 없습니다.
      # 이 공식은 현재 레벨에서 바로 다음 레벨 목표치를 계산하므로,
      # 이미 경험치가 높은 포켓몬은 Lv 100까지의 원형 공식으로 인해 생긴 높은 EXP를
      # 따라잡는 데 시간이 걸릴 뿐, 새로운 공식 자체는 음수를 만들지 않습니다.
      exp = base_exp + sum_delta_exp
    end
    next exp.to_i 
  end
