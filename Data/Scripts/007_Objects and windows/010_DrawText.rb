...
def getFormattedText(bitmap, xDst, yDst, widthDst, heightDst, text, lineheight = 32,
  ...
  text = JosaProcessor.process(text)
  textchunks = []
  controls = []
#  oldtext = text
...
#  end
  ...
  while position < textchars.length
    ...
    texty = (lineheight * y) + yDst + yStart + 5   # TEXT OFFSET
...
module JosaProcessor
  HANGUL_BEGIN = 0xAC00
  HANGUL_END = 0xD7A3

  def self.has_jongsung?(word)
    return false if word.nil? || word.empty?
    last_char = word[-1]
    code = last_char.ord
    return false unless (HANGUL_BEGIN..HANGUL_END).include?(code)
    jong = (code - HANGUL_BEGIN) % 28
    jong != 0
  end

  def self.strip_color_codes(word)
    clean = word.gsub(/\\c\[\d+\]/, "")
    clean = clean.gsub(/<[^>]*>/, "")
    clean.strip
  end
  ...
  def self.process(text)
    text.gsub(/\\j\[((?:\\.|[^\]])+?),([^,\]]*?),([^,\]]*?)\]/) do
      ...
      plain_word = strip_color_codes(raw_word)
      josa = has_jongsung?(plain_word) ? josa_with : josa_without
      "#{raw_word}#{josa}"
    end
  end
end
...
module Kernel
  if method_defined?(:pbShowCommands)
    alias_method :__josa_pbShowCommands, :pbShowCommands

    def pbShowCommands(text, commands, default = 0, cancel = -1)
      text = JosaProcessor.process(text)
      __josa_pbShowCommands(text, commands, default, cancel)
    end
  else
    # pbShowCommands가 아직 정의되지 않았을 때 경고 출력
    puts "Warning: pbShowCommands method not found in Kernel."
  end
end

module JosaIntl
  def self.message(text, *args)
    formatted = _INTL(text, *args)
    final = JosaProcessor.process(formatted)
    pbMessage(final)
  end

  def self.show_commands(text, commands, default = 0, cancel = -1)
    formatted = _INTL(text)
    final = JosaProcessor.process(formatted)
    return pbShowCommands(final, commands, default, cancel)
  end
  
  def show_message_and_choices(msg_text, commands)
    processed_msg = JosaProcessor.process(msg_text)
    pbMessage(processed_msg)
