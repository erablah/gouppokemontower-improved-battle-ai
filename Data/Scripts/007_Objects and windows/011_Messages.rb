...
DEFAULT_WINDOWSKIN="speech hgss 1"
...
TEXT_SKIP_BUTTON = Input::JUMPDOWN
# Possible Inputs:
...
def pbChooseNumber(msgwindow, params)
  ...
  cmdwindow.setSkin("Graphics/Windowskins/speech hgss 1") if params.skin
...
def pbDisplayNameWindow(msgwindow,dark,param)
  ...
  if dark==true
   ...
   namewindow.setSkin("Graphics/Windowskins/speech hgss 1")    #namewindow.setSkin(MessageConfig.pbGetSpeechFrame())
  end
...
#=====GELA FACE and NAME 2==================         
 ...
 #==========GELA FACE and NAME 3, ADDING PORTRAIT CATCH, BASICALLY MR, ML   
  ...
  controls.length.times do |i|
    ...
    josa2 = parts[2]
    begin
      result = Josa.get(word, josa1, josa2)
    rescue
      result = "#{word}#{josa1}"  # fallback
    end
    textchunks[i] = result
    controls[i] = nil
    end
...
def pbShowCommands(msgwindow, commands = nil, cmdIfCancel = 0, defaultCmd = 0)
  ...
  # msgwindow가 문자열이라면 조사 처리 적용
  if msgwindow.is_a?(String)
    msgwindow = JosaProcessor.process(msgwindow)
  end
...
class Message
  ...
  def pbShowCommands(msgwindow, commands = nil, cmdIfCancel = 0, defaultCmd = 0)
    # msgwindow가 문자열이면 조사 처리
    if msgwindow.is_a?(String)
      msgwindow = JosaProcessor.process(msgwindow)
    end
    # commands 배열 각 항목에 조사 처리 (필요하면)
    if commands
      commands = commands.map { |cmd| JosaProcessor.process(cmd) }
    end
    __josa_pbShowCommands(msgwindow, commands, cmdIfCancel, defaultCmd)
  end
end