...
class Battle
  ...
  def pbCanTerastallize?(idxBattler)
	...
	return false if battler.item != :TERAPIECE && battler.item != :WELLSPRINGMASK && battler.item != :HEARTHFLAMEMASK && battler.item != :CORNERSTONEMASK && battler.item != :TEALMASK 
  ...
  def pbTerastallize(idxBattler)
    ...
    pbDisplay(_INTL("\\j[{1},은,는] {2}타입으로 테라스탈하였다!", battler.pbThis, GameData::Type.get(battler.tera_type).name))
  ...
  def pbAnimateTerastallization(battler)
    if @scene.pbCommonAnimationExists?("Terastallize")
      ...
      if Settings::SHOW_TERA_ANIM && $PokemonSystem.battlescene == 0
        ...
        if battler.wild?
          pbDisplay(_INTL("\\j[{1},은,는] 테라스탈 에너지가 가득하다!", battler.pbThis))
        else
          trainerName = pbGetOwnerName(battler.index)
          pbDisplay(_INTL("\\j[{1},이,가] {2}의 {3}에 반응하고 있다!", battler.pbThis, trainerName, pbGetTeraOrbName(battler.index)))
        end
