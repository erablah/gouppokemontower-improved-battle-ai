...
MidbattleHandlers.add(:midbattle_global, :wild_ultra_battle,
  proc { |battle, idxBattler, idxTarget, trigger|
    ...
    when "BattlerReachedHPCap_foe"
      ...
      battle.pbDisplayPaused(_INTL("{1}의 울트라버스트가 사라졌다!\n이제 포획이 가능할 것 같다!", foe.pbThis))
    when "BattleEndWin"
...
class Battle
  ...
  def pbUltraBurst(idxBattler)
    ...
    pbDisplay(_INTL("{1}에게서 밝은 빛이 쏟아져 나온다!", battler.pbThis(true)))    
    pbAnimateUltraBurst(battler)
    pbDisplay(_INTL("\\j[{1},은,는] 울트라버스트로 진정한 힘을 내기 시작하였다!", battler.pbThis))    
    side  = battler.idxOwnSide
    owner = pbGetOwnerIndexFromBattlerIndex(idxBattler)
