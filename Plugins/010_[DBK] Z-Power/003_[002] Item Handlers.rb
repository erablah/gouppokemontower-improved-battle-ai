...
ItemHandlers::UseOnPokemon.addIf(:zcrystals,
  ...
  proc { |item, qty, pkmn, scene|
    ...
    if pkmn.shadowPokemon? || pkmn.egg?
      ...
      scene.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 이미 \\j[{2},을,를] 지니고 있다.", pkmn.name, crystal)))
      next false
    elsif !compatible && !scene.pbConfirm(JosaProcessor.process(_INTL("\\j[{1},은,는] 이 Z크리스탈의 힘을 쓰지 못 할 것 같다.. 그래도 줄까?", pkmn.name)))
      next false
    end
    scene.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},이,가] {2}에게 Z파워를 주고 있다!", crystal, pkmn.name)))
    if pkmn.item
      heldItem = GameData::Item.get(pkmn.item)
      prefix = (heldItem.is_zcrystal?) ? "a piece of" : (heldItem.portion_name.starts_with_vowel?) ? "" : ""
      scene.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 이미 {2} \\j[{3},을,를] 지니고 있다.\1", pkmn.name, prefix, heldItem.portion_name)))
      if scene.pbConfirm(_INTL("Would you like to switch the two items?"))
        if !$bag.can_add?(pkmn.item)
          ...
          scene.pbDisplay(JosaProcessor.process(_INTL("{1}의 \\j[{2},을,를] 가져온 뒤, \\j[{3},을,를] 지니게 했다!", pkmn.name, itemname, crystal)))
        end
      else
        next false
      end
    ...
    scene.pbDisplay(JosaProcessor.process(_INTL("{1}에게 \\j[{2},을,를] 지니게 했다!", pkmn.name, crystal)))
    next true
  }
)
