...
def self.item(item, quantity = 1, message = nil)
    ...
    message = _INTL("\\j[{1},이,가] 뭔가를 들고 있는 듯하다...") if nil_or_empty?(message)
    ...
    if $bag.add(item, quantity)    # If item can be picked up
      ...
      if item == :LEFTOVERS
        pbMessage(_INTL("\\me[{1}]\\j[{3},이,가] \\j[\\c[1]{2}\\c[0],을,를] 발견했다!\\wtnp[30]", meName, itemname, pokename))
      elsif item == :DNASPLICERS
        pbMessage(_INTL("\\me[{1}]\\j[{3},이,가] \\j[\\c[1]{2}\\c[0],을,를] 발견했다!\\wtnp[30]", meName, itemname, pokename))
      elsif item.is_machine?    # TM or HM
        pbMessage(_INTL("\\me[{1}]\\j[{4},이,가] \\j[\\c[1]{2} {3}\\c[0],을,를] 발견했다!\\wtnp[30]", meName, itemname, GameData::Move.get(move).name, pokename))
      elsif quantity>1
        pbMessage(_INTL("\\me[{1}]\\j[{4},이,가] \\j[\\c[1]{3}\\c[0],을,를] {2}개 발견했다!\\wtnp[30]", meName, quantity, itemname, pokename))
      elsif itemname.starts_with_vowel?
        pbMessage(_INTL("\\me[{1}]\\j[{3},이,가] \\j[\\c[1]{2}\\c[0],을,를] 발견했다!\\wtnp[30]", meName, itemname, pokename))
      else
        pbMessage(_INTL("\\me[{1}]\\j[{3},이,가] \\j[\\c[1]{2}\\c[0],을,를] 발견했다!\\wtnp[30]", meName, itemname, pokename))
      end
      pbMessage(_INTL("\\j[{1},을,를] 가방의\\n<icon=bagPocket{2}>\\c[1]{3} 주머니\\c[0]에 넣었다.",
    ...
    # Can't add the item
    if item == :LEFTOVERS
      pbMessage(_INTL("\\j[{1},이,가] \\j[\\c[1]{2}\\c[0],을,를] 발견했다!\\wtnp[30]", pokename, itemname))
    elsif item == :DNASPLICERS
      pbMessage(_INTL("\\j[{1},이,가] \\j[\\c[1]{2}\\c[0],을,를] 발견했다!\\wtnp[30]", pokename, itemname))
    elsif item.is_machine?    # TM or HM
      pbMessage(_INTL("\\j[{1},이,가] \\j[\\c[1]{2} {3}\\c[0],을,를] 발견했다!\\wtnp[30]", pokename, itemname, GameData::Move.get(move).name))
    elsif quantity>1
      pbMessage(_INTL("\\j[{1},이,가] \\j[\\c[1]{3}\\c[0],을,를] {2}개 발견했다!\\wtnp[30]", pokename, quantity, itemname))
    elsif itemname.starts_with_vowel?
      pbMessage(_INTL("\\j[{1},이,가] \\j[\\c[1]{2}\\c[0],을,를] 발견했다!\\wtnp[30]", pokename, itemname))
    else
      pbMessage(_INTL("\\j[{1},이,가] \\j[\\c[1]{2}\\c[0],을,를] 발견했다!\\wtnp[30]", pokename, itemname))
    end
    pbMessage(_INTL("하지만 가방이 가득 찼다..."))
    return false
  end
  #-----------------------------------------------------------------------------
