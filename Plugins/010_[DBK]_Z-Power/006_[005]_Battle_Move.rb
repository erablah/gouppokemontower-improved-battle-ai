...
class Battle::Move
  ...
  def pbDisplayUseMessage(user)
    if zMove? && !@specialUseZMove
      ...
      @battle.pbDisplayBrief(_INTL("\\j[{1},은,는] Z파워로 둘러싸여있다!", user.pbThis))
      ...
      @battle.pbDisplayBrief(JosaProcessor.process(_INTL("\\j[{1},이,가] Z파워로 전력을 발휘한다!", user.pbThis)))
    end
  ...
  def pbUseZPowerStatusEffect(user)
    ...
    when "HealUser"
      if user.hp < user.totalhp
        ...
        @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] Z파워로 체력을 회복시켰다!", user.pbThis)))
      end
    #---------------------------------------------------------------------------
    ...
    when "CriticalHit"
      if user.effects[PBEffects::FocusEnergy] < 4
        ...
        @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] Z파워를 통해 상대의 급소를 간파했다!", user.pbThis)))
      end
    #---------------------------------------------------------------------------
    ...
    when "ResetStats"
      if user.hasLoweredStatStages?
        ...
        @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] Z파워로 떨어진 능력치를 복구했다!", user.pbThis)))
      end
    #---------------------------------------------------------------------------
    # Z-Powered effects that cause misdirection.
    when "FollowMe"
      @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] Z파워를 사용해 주목의 대상이 되었다!", user.pbThis)))
        ...
        stats.each do |stat|
          if user.pbCanRaiseStatStage?(stat, user, nil, false, true)
            ...
            if showAnim
              ...
              @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] Z파워로 몸이 둘러싸였다!", user.pbThis, boost)))
              showAnim = false
            end
          end
        end
