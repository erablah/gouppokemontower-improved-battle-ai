...
# will not show up on your tournament selection list.
  ...
  def initialize(viewport)
    ...
    if @modified_party == "notEligible"
      pbMessage(_INTL("죄송하지만 토너먼트에 참가할 수 없는 포켓몬이 있는 것 같네요.\\1"))
      GameData::PWTTournament.get(@tournament_id).call_ban_reason
      pbMessage(_INTL("파티를 조정 후 다시 와주세요!\\1"))
  ...
  def continue
    ...
    case ret
    when "win"
      base_reward = GameData::PWTTournament.get(@tournament_id).points_won * 2000
      total_money = base_reward

      pbMessage(_INTL("오늘 토너먼트의 승리를 축하드립니다!\\1"))

      if PWTSettings::PWT_STREAK_MULT && $stats.pwt_win_streak[@tournament_id] >= 5
        total_money *= streak_multiplier
        pbMessage(_INTL("연승 보너스를 포함하여 총 {1}원을 획득했습니다!\\me[Item get]\\wtnp[80]", total_money.to_s_formatted))
      else
        pbMessage(_INTL("이번 승리로 {1}원을 획득했습니다!\\me[Item get]\\wtnp[80]", total_money.to_s_formatted))
      end

      $player.money += total_money

      pbMessage(_INTL("또 다른 도전을 기다리고 있겠습니다."))
      ...
      self.endTournament
    when "loss"
      pbMessage(_INTL("안타깝게도 이번 토너먼트에선 패배하시고 말았네요.\\1"))
      pbMessage(_INTL("다음 번엔 더 좋은 결과를 내시길 바라겠습니다."))
  ...
  def chooseTournament
    ...
    commands.push("취소")
    cmd = pbMessage(_INTL("어떤 토너먼트에 참가하시겠어요?"),commands,commands.length)
...
def chooseBattle
  ...
  def choosePokemon
    ...
    pbMessage(_INTL("참전할 포켓몬을 선택해주세요."))
    pbFadeOutIn(99999){
  ...
  def cancelEntry
    self.endTournament
    pbMessage(_INTL("다음에 다시 뵙도록 하겠습니다."))
    return false
  end
  ...
  def introduction
    pbMessage(_INTL("안녕하세요! 포켓몬 월드 토너먼트에 오신 걸 환영합니다.\\1"))
    pbMessage(_INTL("이 곳은 전세계의 트레이너들이 모여 배틀하는 곳이랍니다.\\1"))
    pbMessage(_INTL("진행하기 전에 우선 게임을 저장하겠습니다.\\1"))
  end
  
  # Creates a small conversation if no Tournaments are available
  def notAvailable
    pbMessage(_INTL("죄송하지만, 현재 참가 가능한 대회가 없는 것 같아요.\\1"))
    pbMessage(_INTL("나중에 다시 와주세요!\\1"))
  end
  ...
  def startTournament
    ...
    pbMessage(_INTL("아나운서: {1} 토너먼트에 오신 여러분을 환영합니다!\\1",GameData::PWTTournament.get(@tournament_id).name))
    pbMessage(_INTL("아나운서: 오늘은 8명의 도전자들이 '챔피언'의 자리를 놓고 불꽃 튀는 한판 승부를 펼칠 예정인데요!\\1"))
    pbMessage(_INTL("아나운서: 그럼 오늘의 대진표, 함께 살펴보시죠!\\1"))
    trainer = self.generateRound1
    self.displayScoreboard(trainer)
    self.moveSwitch('A')
    pbMessage(_INTL("아나운서: 자, 그럼 말은 여기까지! 첫 번째 경기, 시작합니다!\\1"))
    pbMessage(_INTL("아나운서: 이번 경기는 {1}님과 {2}님의 대결입니다!",$player.name,trainer.name))
    ...
    if pbPWTBattle(trainer,@challenge_rules,@outcome)
      ...
      @beat.push(trainer)
      pbMessage(_INTL("아나운서: 와~ 대단한 첫 경기였습니다!\\1"))
      pbMessage(_INTL("아나운서: 관중석의 열기! 선수들의 기세! 지금 이 곳은 완전 뜨겁습니다!\\1"))
      self.visualRound(trainer,true)
      pbMessage(_INTL("아나운서: 자, 그럼 1라운드의 결과를 확인해보겠습니다!\\1"))
      trainer = self.generateRound2
      self.displayScoreboard(trainer)
      pbMessage(_INTL("아나운서: 다음 경기는 {1}님과 {2}님의 대결이 되겠네요!\\1",$player.name,trainer.name))
      self.visualRound(trainer)
      pbMessage(_INTL("아나운서: 자, 두 번째 배틀... 시작합니다!"))
      pbMessage(trainer.beforebattle) if !trainer.beforebattle.nil?
	  $PokemonGlobal.nextBattleVictoryBGM = "B2W2 213 Winning in the PWT!"
      if pbPWTBattle(trainer,@challenge_rules,@outcome)
        @round = 2
        pbMessage(trainer.afterbattle) if !trainer.afterbattle.nil?
        @beat.push(trainer)
        pbMessage(_INTL("아나운서: 오늘 정말 눈을 뗄 수 없는 경기들이 계속되고 있네요!\\1"))
        pbMessage(_INTL("아나운서: 다들 정말 전력을 다해 싸우고 있습니다!\\1"))
        self.visualRound(trainer,true)
        pbMessage(_INTL("아나운서: 자, 결승전에 올라간 트레이너는 과연 누구일까요?\\1"))
        trainer = self.generateRound3
        self.displayScoreboard(trainer)
        pbMessage(_INTL("아나운서: 자, 그럼 이제 준비가 끝난 것 같군요.\\1"))
        pbMessage(_INTL("아나운서: 그럼 {1}님과 {2}님의 결승전을 시작하겠습니다!\1\\1",$player.name,trainer.name))
        self.visualRound(trainer)
        pbMessage(_INTL("아나운서: 누가 이길까요? 최고의 트레이너에게 영광을!"))
        pbMessage(trainer.beforebattle) if !trainer.beforebattle.nil?
        ...
        if pbPWTBattle(trainer,@challenge_rules,@outcome)
          ...
          pbMessage(_INTL("아나운서: 와! 마지막까지 손에 땀을 쥐게 하는 명승부였습니다!\\1"))
          pbMessage(_INTL("아나운서: 두 분 모두 멋졌지만, 승리의 여신은 결국 {1}님에게 미소를 지었네요!\\1",$player.name))
          pbMessage(_INTL("아나운서: 축하드립니다, {1}님! 오늘의 챔피언은 바로 당신입니다!\\1",$player.name))
          pbMessage(_INTL("아나운서: 오늘의 토너먼트는 여기까지! 시청해주신 여러분, 다음에 또 만나요~"))
          return "win"
        end
      end
    end
...
def pbLoadTrainer(*args)
  ...
  return trainer  # 무조건 그대로 반환하여 순서 유지
end
...
def startPWT
  ...
  # 기존 PWT가 남아있다면 강제로 종료 및 정리
  if !$PWT.nil? && $PWT.respond_to?(:endTournament)
    $PWT.endTournament
  end
