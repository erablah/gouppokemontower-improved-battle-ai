...
class Battle
  ...
  def pbUseItemMessage(item, trainerName, pkmn = nil)
    ...
    if pkmn.is_a?(Battle::Battler) && item_data.battle_use < 4
      pbDisplayBrief(_INTL("\\j[{1},은,는] {3}에게 \\j[{2},을,를] 사용했다.", trainerName, itemName, pkmn.pbThis(true)))
    elsif pkmn.is_a?(Pokemon) && item_data.battle_use < 4
      pbDisplayBrief(_INTL("\\j[{1},은,는] {3}에게 \\j[{2},을,를] 사용했다.", trainerName, itemName, pkmn.name))
    else
      pbDisplayBrief(_INTL("\\j[{1},은,는] \\j[{2},을,를] 사용했다.", trainerName, itemName))
    end
  end
end
