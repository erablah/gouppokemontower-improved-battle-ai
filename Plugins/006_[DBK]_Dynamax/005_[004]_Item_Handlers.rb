...
ItemHandlers::UseOnPokemon.add(:DYNAMAXCANDY, proc { |item, qty, pkmn, scene|
  ...
  if pkmn.dynamax_lvl < 10 && pkmn.dynamax_able? && !pkmn.hasEternamaxForm?
    ...
    if item == :DYNAMAXCANDYXL
      scene.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 다이맥스 레벨이 10으로 올랐다!", pkmn.name)))
      $stats.total_dynamax_lvls_gained += (10 - pkmn.dynamax_lvl)
      pkmn.dynamax_lvl = 10
    else
      scene.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 다이맥스 레벨이 1 올랐다!", pkmn.name)))
...
ItemHandlers::UseOnPokemon.add(:MAXSOUP, proc { |item, qty, pkmn, scene|
  ...
  if pkmn.hasGigantamaxForm?
    if pkmn.gmax_factor?
      ...
      scene.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 거다이맥스의 힘을 잃어버렸다.", pkmn.name)))
    else
      pbSEPlay("Pkmn move learnt")
      pkmn.gmax_factor = true
      $stats.total_gmax_factors_given += 1
      scene.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 거다이맥스의 힘을 얻게 되었다!", pkmn.name)))
    end
    scene.pbHardRefresh
    next true
  else
