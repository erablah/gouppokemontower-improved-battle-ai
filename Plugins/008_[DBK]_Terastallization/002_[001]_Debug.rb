        ...
        if pkmn.terastal_able?
          if !pkmn.getTeraType(true).nil?
            ...
            if newType && newType != pkmn.tera_type
              ...
              screen.pbDisplay(_INTL("{1}의 테라스탈 타입이 \\j[{2},으로,로] 변했다!", pkmn.name, GameData::Type.get(newType).name))
              screen.pbRefreshSingle(pkmnid)
            end
          end
        else
      ...
      when 1   # Set Tera type
        if pkmn.terastal_able?
          if !pkmn.getTeraType(true).nil?
            ...
            if newType && newType != pkmn.tera_type
              pkmn.tera_type = newType
              pbMessage("\\ts[]" + _INTL("{{1}의 테라스탈 타입이 \\j[{2},으로,로] 변했다!", pkmn.name, GameData::Type.get(newType).name))
              battler&.pbUpdate
            end
          end
        else
