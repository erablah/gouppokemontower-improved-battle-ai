...
class Battle::Battler
  ...
  def pbSuccessCheckAgainstTarget(move, user, target, targets)
    if target.hasRaidShield? && move.statusMove?
      @battle.pbDisplay(_INTL("이상한 배리어가 \\j[{1},을,를] 지켜냈다!", target.pbThis(true)))
      return false
    end
  ...
  def startRaidShield(shield_hp = 0)
    ...
    @battle.pbDisplay(_INTL("{1} 주변에 에너지가 모이기 시작했다!", pbThis(true)))
    ...
    @battle.pbDisplay(_INTL("{1} 앞에 이상한 배리어가 나타났다!", pbThis(true)))
    pbCureStatus
  ...
  def setRaidShieldHP(amt, user = nil)
    ...
    return if @battle.pbAllFainted? || @battle.decision > 0
    @battle.pbDisplay(_INTL("이상한 배리어가 사라졌다!"))
    oldhp = @hp
    @hp -= @totalhp / 8
    @hp = 1 if @hp <= 1
