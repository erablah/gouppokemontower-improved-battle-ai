...
def pbGenerateRaidRewards(pokemon, style = :Basic, rank = nil, loot = nil, weather = nil, terrain = nil, environ = nil)
  ...
  [:EXPCANDYXL, :EXPCANDYL, :EXPCANDYM, :EXPCANDYS, :EXPCANDYXS].each_with_index do |candy, i|
    case candy
    when :EXPCANDYXS then next if rank > 5 || rank == 5 && rand(5) < 4
    when :EXPCANDYS  then next if rank > 5 && rand(5) < 2
    when :EXPCANDYM  then next if rank < 3
    when :EXPCANDYL  then next if rank < 4 || rank == 4 && rand(5) > 2
    when :EXPCANDYXL then next if rank < 5 || rank == 5 && rand(5) > 2
    end
    candyQty = (i == 0) ? (qty / 2).round : qty * i
    rewards.push([candy, candyQty / 2])
  end
  rewards.push([:RARECANDY, 1 + rand(-1..1)]) if rank > 3
  rewards.push([:SHINYCANDY, 1 + rand(-1..0)]) if rank > 3
  rewards.push([:JAZZTAPE, 1 + rand(-1..0)]) if rank > 2
  rewards.push([:ADAPTIVELEVELCANDY, 1 + rand(-1..1)]) if rank > 3
  #-----------------------------------------------------------------------------
  # Adds TM/TR reward.
  #-----------------------------------------------------------------------------
  if rank > 2
  ...
  if rank > 2
    ...
    if rand(10) < 4
      training = [:PPUP, :PPMAX, :ABILITYCAPSULE, :BOTTLECAP]
