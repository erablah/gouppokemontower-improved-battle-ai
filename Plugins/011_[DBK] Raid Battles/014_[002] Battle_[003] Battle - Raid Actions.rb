...
def pbRaidResetDrops(battler)
  ...
  pbDisplay(_INTL("\\j[{1},이,가] 자신에게 걸린 나쁜 효과를 제거했다!", battler.pbThis))
...
def pbRaidResetBoosts(battler)
  @scene.pbAnimateExtraAction(battler.index)
  pbDisplay(_INTL("\\j[{1},이,가] 아군에게 걸린 능력치 변화와 특성을 무효화했다!", battler.pbThis))
...
def pbRaidDrainCheer(battler)
  @scene.pbAnimateExtraAction(battler.index)
  pbDisplay(_INTL("\\j[{1},이,가] 응원의 효과를 약화시켰다!", battler.pbThis))
