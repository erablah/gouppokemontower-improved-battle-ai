...
MultipleForms.register(:QUILAVA, {
  "getForm" => proc { |pkmn|
    next 1 if pkmn.hasItem?(:HISUIREGION)
    next 0
  }
})
...
# [Generation 9 Pack] [000] Pokemon.rb (또는 form_count를 정의했던 파일)
module GameData
  class Species
    # 폼을 세는 메서드를 GameData::Species.keys를 사용하여 재정의합니다.
    def self.form_count(species_id)
      return 0 unless species_id
      species_id_s = species_id.to_s
      count = 0
      # 모든 종/폼 ID를 반복하여 해당 종 ID로 시작하는 키를 셉니다.
      GameData::Species.keys.each do |key|
        key_s = key.to_s
        # base species (e.g., 'BULBASAUR') 또는 form keys (e.g., 'BULBASAUR_1')를 모두 포함합니다.
        if key_s == species_id_s || key_s.start_with?("#{species_id_s}_")
          count += 1
        end
      end
      return count
