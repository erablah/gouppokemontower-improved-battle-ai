  ...
  def pbStartWeather(user, newWeather, fixedDuration = false, showAnim = true)
    ...
    if newWeather == :Hail && Settings::HAIL_WEATHER_TYPE > 0
      ...
      if Settings::HAIL_WEATHER_TYPE == 2
        pbDisplay(_INTL("거센 눈보라가 치기 시작했다!"))
      else
        pbDisplay(_INTL("눈이 내리기 시작했다!"))
      end
  ...
  def pbEOREndWeather(priority)
    if @field.weather == :Hail && Settings::HAIL_WEATHER_TYPE > 0
      ...
      if @field.weatherDuration == 0
        if Settings::HAIL_WEATHER_TYPE == 2
          pbDisplay(_INTL("눈보라가 그쳤다."))
        else
          pbDisplay(_INTL("눈이 그쳤다."))
        end
        @field.weather = :None
...
class Battle::Battler
  ...
  def pbCanInflictStatus?(newStatus, user, showMessages, move = nil, ignoreStatus = false)
    ...
    if [:DROWSY, :FROSTBITE].include?(newStatus)
      ...
      if self.status == newStatus && !ignoreStatus
        if showMessages
          @battle.pbDisplay(_INTL("\\j[{1},은,는] 이미 졸고 있다!", pbThis))      if newStatus == :DROWSY
          @battle.pbDisplay(_INTL("\\j[{1},은,는] 이미 동상에 걸려있다!", pbThis)) if newStatus == :FROSTBITE
        end
        return false
      end
      ...
      when :DROWSY
        ...
        if !(hasActiveAbility?(:SOUNDPROOF) && !@battle.moldBreaker)
          @battle.allBattlers.each do |b|
            ...
            @battle.pbDisplay(_INTL("하지만 \\j[{1},은,는] 소란을 피고 있다!", pbThis(true))) if showMessages
            return false
          end
        end
      #-------------------------------------------------------------------------
      ...
      if immuneByAbility
        if showMessages
          ...
          if Battle::Scene::USE_ABILITY_SPLASH
            case originalStatus
            when :SLEEP  then msg = _INTL("\\j[{1},은,는] 잠에 들지 않는다!", pbThis)
            when :FROZEN then msg = _INTL("\\j[{1},은,는] 동상에 걸리지 않는다!", pbThis)
            end
          elsif immAlly
            case originalStatus
            when :SLEEP  then msg = _INTL("\\j[{1},은,는] {2}의 {3} 때문에 잠에 들지 않는다!", pbThis, immAlly.pbThis(true), immAlly.abilityName)
            when :FROZEN then msg = _INTL("\\j[{1},은,는] {2}의 {3} 때문에 동상에 걸리지 않는다!", pbThis, immAlly.pbThis(true), immAlly.abilityName)
            end
          else
            case originalStatus
            when :SLEEP  then msg = _INTL("{1}의 \\j[{2},이,가] 졸음을 막았다!", pbThis, abilityName)
            when :FROZEN then msg = _INTL("{1}의 \\j[{2},이,가] 동상을 막았다!", pbThis, abilityName)
            end
          end
  ...
  def pbInflictStatus(newStatus, newStatusCount = 0, msg = nil, user = nil)
    ...
    if [:DROWSY, :FROSTBITE].include?(newStatus)
      ...
      if msg && !msg.empty?
        ...
        case newStatus
        when :DROWSY    then @battle.pbDisplay(_INTL("\\j[{1},은,는] 졸고 있다!\n너무 졸려 기술을 사용할 수 없다!", pbThis))
        when :FROSTBITE then @battle.pbDisplay(_INTL("\\j[{1},은,는] 동상에 걸렸다!", pbThis))
        end
      end
  ...
  def pbCureStatus(showMessages = true)
    if [:DROWSY, :FROSTBITE].include?(self.status)
      ...
      if showMessages
        case oldStatus
        when :DROWSY    then @battle.pbDisplay(_INTL("\\j[{1},은,는] 졸음을 이겨냈다.", pbThis))
        when :FROSTBITE then @battle.pbDisplay(_INTL("\\j[{1},은,는] 동상이 치유되었다.", pbThis))
        end
      end
  ...
  def pbContinueStatus
    ...
    when :DROWSY
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 동상에 몸을 움직이기 힘들다!", pbThis))
	  PBDebug.log("[Status continues] #{pbThis}'s drowsy count is #{@statusCount}")
    when :FROSTBITE
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 동상에 데미지를 받고 있다!", pbThis))
    end
  end
  ...
  def pbCheckFormOnStatusChange
    ...
    if isSpecies?(:SHAYMIN) && frostbite?
      pbChangeForm(0, _INTL("{1}의 모습이 변했다!", pbThis))
    end
