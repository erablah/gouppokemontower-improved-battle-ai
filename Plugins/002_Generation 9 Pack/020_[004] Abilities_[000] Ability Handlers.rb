...
Battle::AbilityEffects::StatusCure.add(:INSOMNIA,
  proc { |ability, battler|
    ...
    if !Battle::Scene::USE_ABILITY_SPLASH
      ...
      when :DROWSY then msg = _INTL("{1}의 \\j[{2},이,가] 잠을 깨웠다!", battler.pbThis, battler.abilityName)
      end
...
Battle::AbilityEffects::StatusCure.add(:MAGMAARMOR,
  proc { |ability, battler|
    ...
    if !Battle::Scene::USE_ABILITY_SPLASH
      ...
      when :FROSTBITE then msg = _INTL("{1}의 \\j[{2},이,가] 동상을 치료했다!", battler.pbThis, battler.abilityName)
      end
      battler.battle.pbDisplay(msg)
    end
    battler.battle.pbHideAbilitySplash(battler)
  ...
  proc { |ability, battler, battle|
    ...
    battler.allAllies.each do |b|
      ...
      if !Battle::Scene::USE_ABILITY_SPLASH
        ...
        when :DROWSY
          battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 동료의 정신을 차리게 했다!", battler.pbThis, battler.abilityName))
        when :FROSTBITE
          battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 동료의 동상을 치료했다!", battler.pbThis, battler.abilityName))
        end
      end
...
Battle::AbilityEffects::EndOfRoundHealing.add(:HYDRATION,
  proc { |ability, battler, battle|
    ...
    if !Battle::Scene::USE_ABILITY_SPLASH
      ...
      when :FROSTBITE
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 동상을 치료했다!", battler.pbThis, battler.abilityName))
      end
    end
    battle.pbHideAbilitySplash(battler)
  }
  ...
  proc { |ability, battler, user, status|
    ...
    when :DROWSY
      if user.pbCanSynchronizeStatus?(:SLEEP, battler)
        ...
        msg = nil
        if !Battle::Scene::USE_ABILITY_SPLASH
          msg = _INTL("{1}의 \\j[{2},이,가] \\j[{3},을,를] 졸리게 만들었다! 졸려서 기술을 쓸 수 없을 지 모른다!",
             battler.pbThis, battler.abilityName, user.pbThis(true))
        end
    ...
    when :FROSTBITE
      if user.pbCanSynchronizeStatus?(:FROZEN, battler)
        ...
        msg = nil
        if !Battle::Scene::USE_ABILITY_SPLASH
          msg = _INTL("{1}의 \\j[{2},이,가] \\j[{3},을,를] 동상에 걸리게 했다!", battler.pbThis, battler.abilityName, user.pbThis(true))
        end
  ...
  proc { |ability, user, target, move, battle|
    ...
    if user.affectedByContactEffect?(Battle::Scene::USE_ABILITY_SPLASH)
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        when :LINGERINGAROMA
          msg = _INTL("가시지않는향기가 {1}에게 옮겨갔다!", user.pbThis(true))
        end
  ...
  proc { |ability, battler, battle, switch_in|
    ...
    battle.allBattlers.each do |b|
      if b.hasActiveItem?(:ABILITYSHIELD)
        ...
        battle.pbDisplay(_INTL("{1}의 특성은 {2}의 효과에 의해 지켜지고 있다!",b.pbThis,itemname))
        next
      end
...
Battle::AbilityEffects::OnEndOfUsingMove.add(:BATTLEBOND,
  proc { |ability, user, targets, move, battle|
    ...
    battle.pbHideAbilitySplash(user)
    # 폼 변화 추가
    user.pbChangeForm(2, _INTL("\\j[{1},은,는] 유대변화했다!", user.pbThis))
    battle.scene.pbChangePokemon(user.index, user.pokemon) if battle.scene.respond_to?(:pbChangePokemon)
    # 능력치 상승(원한다면)
    showAnim = true
