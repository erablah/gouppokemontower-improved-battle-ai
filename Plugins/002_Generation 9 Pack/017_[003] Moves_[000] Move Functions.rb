  ...
  def pbEffectAgainstTarget(user, target)
    if [:DROWSY, :FROSTBITE].include?(user.status)
      ...
      when :DROWSY
        ...
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 졸음을 이겨냈다.", user.pbThis))
      when :FROSTBITE
        target.pbFreeze
        user.pbCureStatus(false)
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 동상이 치유되었다.", user.pbThis))
      end
    else
...
class Battle::Move::CureUserPartyStatus < Battle::Move
  def pbAromatherapyHeal(pkmn, battler = nil)
    ...
    when :DROWSY
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 졸음을 이겨냈다.", curedName))
    when :FROSTBITE
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 동상이 치유되었다.", curedName))
    end
  end
end
...
class Battle::Move::HealUserAndAlliesQuarterOfTotalHPCureStatus < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    if target.status != :NONE
      ...
      when :DROWSY
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 졸음을 이겨냈다.", target.pbThis))
      when :FROSTBITE
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 동상이 치유되었다.", target.pbThis))
      end
    end
  end
end
  ...
  def pbEffectGeneral(user)
    ...
    @battle.allSameSideBattlers(user).each do |b|
      ...
      if b.hasActiveAbility?(:WINDRIDER) && b.pbCanRaiseStatStage?(:ATTACK, b, self)
        ...
        @battle.pbDisplay(_INTL("불어오는 바람에 \\j[{1],은,는] 충전되었다!", b.pbThis(true)))
...
class Battle::Move::UserTargetSwapAbilities < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if !ret && target.hasActiveItem?(:ABILITYSHIELD)
      @battle.pbDisplay(_INTL("{1}의 특성은 특성가드에 의해 지켜지고 있다!", target.pbThis)) if show_message
      return true
    end
    return ret
  end
...
class Battle::Move::MaxUserAttackLoseHalfOfTotalHP < Battle::Move
  def pbEffectGeneral(user)
    ...
    if user.hasActiveAbility?(:CONTRARY)
      ...
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 자신의 HP를 깎아 공격을 최소로 만들었다!", user.pbThis))
    else
      user.stages[:ATTACK] = 6
      user.addSideStatUps(:ATTACK, 6)
      user.statsRaisedThisRound = true
      @battle.pbCommonAnimation("StatUp", user)
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 자신의 HP를 깎아 공격을 최대로 만들었다!", user.pbThis))
    end
