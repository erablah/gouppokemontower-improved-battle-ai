...
class Battle::Battler
    ...
    if hasActiveAbility?(:TRACE)
      if hasActiveItem?(:ABILITYSHIELD) # Trace failed by its own Ability Shield
        if onSwitchIn
          ...
          @battle.pbDisplay(_INTL("{1}의 특성은 특성가드에 의해 지켜지고 있다!", pbThis))
  ...
  def pbCheckForm(endOfRound = false)
    ...
    if isSpecies?(:TERAPAGOS) && self.ability == :TERASHIFT
      if @form == 0
        ...
        @battle.pbHideAbilitySplash(self)
        pbChangeForm(1, _INTL("{1}의 모습이 변했다!", pbThis))
      end
    end
  ...
  def pbFaint(showMessage = true)
    ...
    if @effects[PBEffects::Commander]
      ...
      if pairedBattler&.effects[PBEffects::Commander]
        ...
        commanderMsg = _INTL("\\j[{1},은,는] {2}의 입 속에서 나왔다!", *order)
  ...
  def pbEffectsAfterMove(user, targets, move, numHits)
    ...
    if move.damagingMove?
      if move.electrocuteUser? && user.status == :DROWSY
        ...
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 충격을 받아 잠에서 깼다!", user.pbThis))
      end
      if user.status == :FROSTBITE && move.thawsUser?
        user.pbCureStatus(false)
        @battle.pbDisplay(_INTL("\\j[{1},은,는] 몸을 녹였다!", user.pbThis))
      end
      targets.each do |b|
  ...
  def pbCanChooseMove?(move, commandPhase, showMessages = true, specialUsage = false)
    if !@effects[PBEffects::Instructed] && @lastMoveUsed == move.id &&
      ...
      if showMessages
        msg = _INTL("\\j[{1},은,는] 이어서 사용할 수 없다!", move.name)
