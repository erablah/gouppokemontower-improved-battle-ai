...
class PokemonLoadScreen
  ...
  def load_save_file(file_path)
    ...
    unless SaveData.valid?(save_data)
      if File.file?(file_path + ".bak")
        pbMessage(_INTL("마지막 세이브가 제대로 이루어지지 않아 백업 세이브를 불러옵니다."))
  ...
  def prompt_save_deletion(file_path)
    pbMessage(_INTL("세이브 파일이 훼손되었거나, 적합한 게임의 세이브가 아닙니다.") + "\1")
    self.delete_save_data(file_path) if pbConfirmMessageSerious(
      _INTL("세이브 파일을 삭제할까요? 게임이 당장 종료되지는 않습니다.")
    )
    exit
  end
  ...
  def delete_save_data(file_path=nil)
    begin
      ...
      pbMessage(_INTL("세이브 파일이 삭제되었습니다."))
    rescue SystemCallError
      pbMessage(_INTL("세이브 파일이 삭제되지 않습니다."))
    end
  end
...
class PokemonSaveScreen
  def doSave(slot)
    if Game.save(slot)
      pbMessage("\\se[]" + _INTL("\\j[{1},은,는] 게임을 저장했다.", $player.name) + "\\me[GUI save game]\\wtnp[20]")
      return true
    else
      pbMessage("\\se[]" + _INTL("저장에 실패했다.") + "\\wtnp[30]")
      return false
    end
  end
  ...
  def pbSaveScreen(exiting=false)
    ...
    if !$player.save_slot
      ...
      choices = [
        _INTL("#{$player.save_slot}에 저장"),
        _INTL("다른 세이브에 저장"),
        exiting ? _INTL("저장하지 않고 종료") : _INTL("취소")
      ]
      opt = pbMessage(_INTL("게임을 저장할까요?"),choices,3)
      if opt == 0
    ...
    loop do
      ...
      if index >= 0
        ...
        if !File.file?(SaveData.get_full_path(slot)) ||
            pbConfirmMessageSerious(_INTL("#{slot}의 세이브를 바꿀까요?")) # If the slot names were changed this grammar might need adjustment.
          pbSEPlay('GUI save choice')
          ret = doSave(slot)
        end
      elsif exiting # Pressed cancel
        next unless pbConfirmMessageSerious(_INTL("정말 저장하지 않고 종료할까요?"))
      end
      break
    end
  ...
  def slotSelectCommands(choices, choice_info, defaultCmd=0)
    msgwindow = Window_AdvancedTextPokemon.new(_INTL("어떤 슬롯에 저장할까요?"))
    msgwindow.z = 99999
  ...
  def getSaveInfoBoxContents(slot)
    ...
    if !File.file?(full_path)
      return _INTL("<ac><c3=3050C8,D0D0C8>(세이브 없음)</c3></ac>")
    end
    ...
    if time
      ...
      datetime_str = _INTL("<ac>(지난 세이브)</ac>")
    end
    ...
    min = totalsec / 60 % 60
    if hour > 0
      elapsed_str = _INTL("플레이<r>{1}시간 {2}분<br>", hour, min)
    else
      elapsed_str = _INTL("플레이<r>{1}분<br>", min)
    end
...
def pbEmergencySave
  oldscene = $scene
  $scene = nil
  pbMessage(_INTL("스크립트 오류가 발생해 게임을 재실행합니다."))
  return if !$player
  ...
  if Game.save
    pbMessage("\\se[]" + _INTL("게임이 저장되었습니다.") + "\\me[GUI save game]\\wtnp[20]")
    pbMessage("\\se[]" + _INTL("이전 세이브가 백업되었습니다.") + "\\wtnp[20]")
  else
    pbMessage("\\se[]" + _INTL("세이브에 실패하였습니다.") + "\\wtnp[30]")
  end
  $scene = oldscene
end
