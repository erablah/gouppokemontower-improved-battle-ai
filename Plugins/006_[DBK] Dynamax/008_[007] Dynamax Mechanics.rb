...
class Battle
  ...
  def pbCanDynamax?(idxBattler)
	...
	return false if battler.item != :MAXMUSHROOM
  ...
  def pbAnimateDynamax(battler)
    if @scene.pbCommonAnimationExists?("Dynamax")
      ...
      if Settings::SHOW_DYNAMAX_ANIM && $PokemonSystem.battlescene == 0
        ...
        if battler.wild?
          pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 스스로를 {2} 에너지로 감쌌다!", battler.pbThis, type)))
          @scene.pbRevertBattlerStart(idxBattler)
          battler.makeDynamax
          @scene.pbRevertBattlerEnd
        else
          trainerName = pbGetOwnerName(idxBattler)
          pbDisplay(JosaProcessor.process(_INTL("\\j[{1],은,는] \\j[{2},을,를] 불러들였다!", trainerName, battler.pbThis(true))))
