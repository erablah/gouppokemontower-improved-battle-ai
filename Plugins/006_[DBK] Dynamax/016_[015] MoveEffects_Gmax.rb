...
class Battle::DynamaxMove::DamageTargetSleepTargetNextTurn < Battle::DynamaxMove::Move
  def pbEffectAfterAllHits(user, target)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\J[{1},은,는] 잠이 오기 시작했다!", target.pbThis)))
  end
end
...
class Battle::DynamaxMove::StartVineLashOnFoeSide < Battle::DynamaxMove::Move
  def pbEffectAfterAllHits(user, target)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},이,가] 뿌리에 휩싸였다!", user.pbOpposingTeam)))
  end
end
...
class Battle::DynamaxMove::StartWildfireOnFoeSide < Battle::DynamaxMove::Move
  def pbEffectAfterAllHits(user, target)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},이,가] 불꽃에 휩싸였다!", user.pbOpposingTeam)))
  end
end
...
class Battle::DynamaxMove::StartCannonadeOnFoeSide < Battle::DynamaxMove::Move
  def pbEffectAfterAllHits(user, target)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},이,가] 강력한 소용돌이에 갇혔다!", user.pbOpposingTeam)))
  end
end
...
class Battle::DynamaxMove::DamageTargetStartWeakenDamageAgainstUserSide < Battle::DynamaxMove::Move
  def pbEffectAfterAllHits(user, target)
    ...
    @battle.pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] 물리와 특수 공격을 약하게 받는다!", user.pbTeam)))
  end
end
...
class Battle::DynamaxMove::BindTargetSideUserCanSwitch < Battle::DynamaxMove::Move
  def pbEffectAfterAllHits(user, target)
    ...
    @battle.allOtherSideBattlers(user).each do |b|
      ...
      when :WHIRLPOOL
        msg = _INTL("\\j[{1},은,는] 강력한 소용돌이에 갇혔다!", b.pbThis)
      else
...
class Battle::DynamaxMove::RemoveSideEffectsAndTerrain < Battle::DynamaxMove::Move
  def pbEffectAfterAllHits(user, target)
    ...
    side_effects = [
      ...
      [PBEffects::Steelsurge,  false, _INTL("\\j[{1},은,는] 뾰족한 철을 날려보냈다!", user.pbThis)],
    ...
    if @battle.field.terrain != :None
      ...
      else                @battle.pbDisplay(_INTL("필드가 원래대로 돌아왔다.")) 
      end
