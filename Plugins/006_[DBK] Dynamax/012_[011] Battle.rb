...
class Battle  
    ...
    2.times do |side|
      ...
      if @sides[side].effects[PBEffects::VineLash] > 0
        ...
        priority.each do |battler|
          ...
          battler.pbTakeEffectDamage(battler.totalhp / 6, false) { |hp_lost|
            pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] {2}의 거대한 줄기에 의해 데미지를 입고 있다!", battler.pbThis, movename)))
          }
        end
      end
      #-------------------------------------------------------------------------
      ...
      if @sides[side].effects[PBEffects::Wildfire] > 0
        ...
        priority.each do |battler|
          ...
          battler.pbTakeEffectDamage(battler.totalhp / 6, false) { |hp_lost|
            pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] {2}의 회오리 불꽃에 데미지를 입고 있다!", battler.pbThis, movename)))
          }
        end
      end
      #-------------------------------------------------------------------------
      ...
      if @sides[side].effects[PBEffects::Cannonade] > 0
        ...
        priority.each do |battler|
          ...
          battler.pbTakeEffectDamage(battler.totalhp / 6, false) { |hp_lost|
            pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] {2}의 소용돌이에 데미지를 입고 있다!", battler.pbThis, movename)))
          }
        end
      end
      #-------------------------------------------------------------------------
      ...
      if @sides[side].effects[PBEffects::Volcalith] > 0
        ...
        priority.each do |battler|
          ...
          battler.pbTakeEffectDamage(battler.totalhp / 6, false) { |hp_lost|
            pbDisplay(JosaProcessor.process(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 인해 던져진 바위에 데미지를 입고있다!", battler.pbThis, movename)))
          }
        end
      end
    end
  ...
  def pbEOREndSideEffects(side, priority)
    ...
    pbEORCountDownSideEffect(side, PBEffects::VineLash,
                             JosaProcessor.process(_INTL("\\j[{1},은,는] {2}의 거대한 줄기에서 풀려났다!", @battlers[side].pbTeam, movename)))
    # Wildfire
    movename = GameData::Move.get(:GMAXWILDFIRE).name
    pbEORCountDownSideEffect(side, PBEffects::Wildfire,
                             JosaProcessor.process(_INTL("\\j[{1},은,는] {2}의 회오리불꽃에서 풀려났다!", @battlers[side].pbTeam, movename)))
    # Cannonade
    movename = GameData::Move.get(:GMAXCANNONADE).name
    pbEORCountDownSideEffect(side, PBEffects::Cannonade,
                             JosaProcessor.process(_INTL("\\j[{1},은,는] {2}의 소용돌이에서 풀려났다!", @battlers[side].pbTeam, movename)))
    # Volcalith
    movename = GameData::Move.get(:GMAXVOLCALITH).name
    pbEORCountDownSideEffect(side, PBEffects::Volcalith,
                             JosaProcessor.process(_INTL("{1}으로 인해 {2} 주위에 던져진 바위가 멈췄다!", movename, @battlers[side].pbTeam(true))))
  end
