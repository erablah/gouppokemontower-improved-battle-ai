...
def self.savePreviousTrainerPartyLevels(key, party)
  $PokemonGlobal.previous_trainer_party_levels ||= {}
  $PokemonGlobal.previous_trainer_party_levels[key] = party.map do |pkmn|
    next nil unless pkmn
    dummy = Pokemon.new(pkmn.species, pkmn.level, pkmn.owner)
    dummy.form = pkmn.form
    dummy.calc_stats
    dummy
  end
  puts "[ALS] 저장 완료: #{key} → 레벨들: " +
       $PokemonGlobal.previous_trainer_party_levels[key].map { |p| p ? p.level : "nil" }.inspect
end

def self.applyPreviousTrainerPartyLevels(trainer)
  $PokemonGlobal.previous_trainer_party_levels ||= {}
  levels = $PokemonGlobal.previous_trainer_party_levels[trainer.key]
  return unless levels
  trainer.party.each_with_index do |pkmn, i|
    next unless levels[i]
    pkmn.level = levels[i]
    pkmn.calc_stats
  end
end
