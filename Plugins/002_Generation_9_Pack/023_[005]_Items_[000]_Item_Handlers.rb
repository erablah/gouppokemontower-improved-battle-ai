...
ItemHandlers::UseOnPokemon.add(:AWAKENING, proc { |item, qty, pkmn, scene|
  ...
  when :DROWSY then msg = _INTL("\\j[{1},은,는] 졸음을 이겨냈다.", pkmn.name)
  end
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(msg)
...
ItemHandlers::UseOnPokemon.add(:ICEHEAL, proc { |item, qty, pkmn, scene|
  ...
  when :FROSTBITE then msg = _INTL("\\j[{1},은,는] 동상이 치유되었다.", pkmn.name)
  end
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(msg)
...
ItemHandlers::BattleUseOnPokemon.add(:AWAKENING, proc { |item, pokemon, battler, choices, scene|
  ...
  when :DROWSY then scene.pbDisplay(_INTL("\\j[{1},은,는] 졸음을 이겨냈다.", name))
  end
})
...
ItemHandlers::BattleUseOnPokemon.add(:ICEHEAL, proc { |item, pokemon, battler, choices, scene|
  ...
  when :FROSTBITE then scene.pbDisplay(_INTL("\\j[{1},은,는] 동상이 치유되었다.", name))
  end
})
...
Battle::ItemEffects::StatusCure.add(:LUMBERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      ...
      when :FROSTBITE
        battle.pbDisplay(_INTL("{1}의 \\j[{2},이,가] 동상을 치료했다!", battler.pbThis, itemName))
      end
      if oldConfusion
...
Battle::ItemEffects::StatusCure.add(:CHESTOBERRY,
  proc { |item, battler, battle, forced|
    ...
    when :DROWSY then msg = _INTL("{1}의 \\j[{2},이,가] 잠을 깨웠다!", battler.pbThis, itemName)
    end
    battler.pbCureStatus(forced)
...
Battle::ItemEffects::StatusCure.add(:ASPEARBERRY,
  proc { |item, battler, battle, forced|
    ...
    when :FROSTBITE then msg = _INTL("{1}의 \\j[{2},이,가] 동상을 치료했다!", battler.pbThis, itemName)
    end
    battler.pbCureStatus(forced)
    battle.pbDisplay(msg) if !forced
    next true
