...
class PokemonSummary_Scene
  ...
  def pbOptions
    ...
    case @page
    when 4
      commands[cmdCheckMoves = commands.length] = _INTL("기술 확인하기") if !@pokemon.moves.empty?
      commands[cmdTeachTMs   = commands.length] = _INTL("기술머신 사용하기")
    else
      if !@pokemon.egg?
    ...
    if cmdGiveItem >= 0 && command == cmdGiveItem
      ...
      nickname = pbEnterPokemonName(_INTL("{1}의 별명은?", @pokemon.name), 0, Pokemon::MAX_NAME_SIZE, "", @pokemon, true)
