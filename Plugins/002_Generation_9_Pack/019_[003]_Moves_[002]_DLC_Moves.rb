...
class Battle::Move::LowerTargetSpeedOverTime < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 끈적한 물엿을 뒤집어 썼다!", target.pbThis))
  end
...
class Battle::Move::TwoTurnAttackOneTurnInRainRaiseUserSpAtk1 < Battle::Move::TwoTurnMove
  ...
  def pbChargingTurnMessage(user, targets)
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 전기를 흡수했다!", user.pbThis))
  end
...
class Battle::Move::RandomlyDealsDoubleDamage < Battle::Move
  def pbOnStartUse(user, targets)
    ...
    if @allOutAttack
      @battle.pbDisplay(_INTL("\\j[{1},은,는] 전력을 다하기 시작했다!", user.pbThis))
    end
  end
...
class Battle::Move::RaiseAlliesCriticalHitRate1DragonTypes2 < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    @battle.pbDisplay(_INTL("\\j[{1},은,는] 이미 사기가 돋아 있다!", target.pbThis)) if show_message
    return true
  end
