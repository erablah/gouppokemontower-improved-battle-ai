...
class Battle
  ...
  def pbEOREndBattlerEffects(priority)
    ...
    pbEORCountDownBattlerEffect(priority, PBEffects::Splinters) { |battler|
      pbDisplay(_INTL("\\j[{1},은,는] 떠다디는 돌에서 벗어났다!", battler.pbThis))
  ...
  def pbEOREndBattlerSelfEffects(battler)
    ...
    if battler.effects[PBEffects::Syrupy] > 0
      ...
      pbDisplay(_INTL("\\j[{1},은,는] 끈적한 물엿에서 벗어났다!", battler.pbThis)) if battler.effects[PBEffects::Syrupy] == 0
    end
  end
  ...
  def pbReviveInParty(idxBattler, canCancel = false)
    ...
    pbDisplay(_INTL("\\j[{1},은,는] 정신을 차리고 다시 싸울 준비가 되었다!", displayname))
  end
end
