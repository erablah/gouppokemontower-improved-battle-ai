...
class PokemonSummary_Scene
  ...
  def drawPageOneEgg
    ...
    if mapname && mapname != ""
      ...
      memo += black_text_tag + _INTL("{1}에게서 받은 포켓몬 알.", mapname) + "\n"
    else
  ...
  def pbOptions
    ...
    options.each do |cmd|
      ...
      when :moves    then commands[cmd] = _INTL("기술 확인하기")   if Settings::MECHANICS_GENERATION >= 9 && !@pokemon.moves.empty?
    ...
    when :forget
      ...
      if ret >= 0
        ...
        pbMessage(_INTL("\\j[{1},은,는] \\j[{2},을,를] 잊었다!", @pokemon.name, old_move_name))
