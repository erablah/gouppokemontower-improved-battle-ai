...
class Battle
  ...
  def pbCallForHelp(caller)
    ...
    if caller.totemBattler
      pbDisplay(_INTL("\\j[{1},이,가] 동료 포켓몬에게 도움을 요청했다!", caller.pbThis))
    else
      pbDisplay(_INTL("\\j[{1},이,가] 도움을 요청했다!", caller.pbThis))
    end
    ...
    if idxNewBattler >= 0
      ...
      pbDisplay(_INTL("\\j[{1},이,가] 나타났다!", battler.name))
      ...
      pbDisplay(_INTL("하지만 아무도 나타나지 않았다!"))
      @scene.pbAnimateSubstitute(caller, :show)
