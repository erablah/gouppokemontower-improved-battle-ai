...
def pbSavingPrompt(pkmn, rules)
    ...
    if $DEBUG && Input.press?(Input::CTRL)
      ...
      this_event.turn_up
      pbMessage(_INTL("눈앞의 \\j[{1},을,를] 들여다보았다...", den_name))
      return true
    end
    if !raid_pkmn
      if pbConfirmMessage(_INTL("새로운 레이드에 진입하려면 게임을 저장해야 한다! 그렇게 할까?"))
        save_game = true
        if SaveData.exists? && $game_temp.begun_new_game
          pbMessage(_INTL("경고!"))
          pbMessage(_INTL("이미 저장된 다른 게임 파일이 존재한다!"))
          pbMessage(_INTL("지금 저장하면, 다른 파일의 아이템과 포켓몬을 포함한 모든 모험 기록이 완전히 사라진다!"))
          if !pbConfirmMessageSerious(_INTL("정말로 지금 저장하여 기존 파일을 덮어쓸까?"))
      ...
      if save_game
        ...
        if Game.save
          pbMessage(_INTL("\\se[]\\j[{1},이,가] 게임을 저장했다!\\me[GUI save game]\\wtnp[30]", $player.name))
        else
          pbMessage(_INTL("\\se[]저장에 실패했다!\\wtnp[30]"))
          @interp.setVariable(nil)
          this_event.turn_up
          save_game = false
        end
  ...
  def pbStartScene(pkmn, rules)
    ...
    party_config = (@rules[:size] > 1) ? _INTL("파티 변경") : _INTL("포켓몬 변경")
    textPos = [
      [raid_name.upcase,     97,  24, :center, BASE, SHADOW, :outline],
      [_INTL("레이드 시작"), 391, 140, :center, BASE, SHADOW, :outline],
      [_INTL("레이드 나가기"), 391, 174, :center, BASE, SHADOW, :outline],
	...
	if showTurns
	  if showKOs
	    battleText = "턴 제한 : #{@rules[:turn_count]} \n기절 제한 : #{@rules[:ko_count]}"
	  else
	    battleText = "턴 제한 : #{@rules[:turn_count]}"
	  end
	elsif showKOs
	  battleText = "배틀 제한 : #{@rules[:ko_count]}"
	else
	  battleText = "포켓몬을 쓰러뜨려라!"
	end
    #---------------------------------------------------------------------------
...
def pbRaidEntry
    ...
    loop do
      ...
      if Input.trigger?(Input::UP)
        ...
        if pbConfirmMessage(_INTL("레이드를 그만둘까?"))
        ...
        when 0 # Begin Raid
          @sprites["cursor"].visible = false
          party_display = (@rules[:size] > 1) ? "파티" : "포켓몬"
          if pbConfirmMessage(_INTL("화면에 표시된 {1}로 레이드에 입장할까?", party_display))
...
def pbRaidRewardsScreen(outcome)
  ...
  if [1, 4].include?(outcome)
    ...
    if !items.empty?
      ...
      textPos.push([_INTL("보기"), 316, 290, :center, BASE, SHADOW, :outline])
    end
  end
  pbDrawImagePositions(@overlay, imagePos)
  #---------------------------------------------------------------------------
  ...
  case outcome
  when 1
    result  = JosaProcessor.process(_INTL("\\j[{1},을,를] 쓰러뜨렸다!", @pkmn.name))
  when 4
    result  = JosaProcessor.process(_INTL("\\j[{1},을,를] 잡았다!", @pkmn.name))
    level   = _INTL("Lv. {1}", @pkmn.level)
    ability = _INTL("특성: {1}", GameData::Ability.get(@pkmn.ability_id).name)
    if @pkmn.male?
    ...
    result = _INTL("{1}에게 패배했다!", @pkmn.name)
    textPos.push([_INTL("획득한 보상이 없다."), 367, 180, :center, BASE, SHADOW])
  end
  textPos.push(
    [result,          377,  24, :center, BASE, SHADOW, :outline],
    [level,           32,  78, :left,   BASE, SHADOW, :outline],
    [ability,         32, 290, :left,   BASE, SHADOW, :outline],
    [_INTL("다음"), 438, 290, :center, BASE, SHADOW, :outline]
  )
  pbDrawTextPositions(@overlay, textPos)
  #---------------------------------------------------------------------------
  # Screen controls
